...
One ``CDPSupervisor`` runs per Artemis ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Artemis logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "artemis-dialog-bridge.invalid"
...
(() => {
  if (window.__artemisDialogBridgeInstalled) return;
  window.__artemisDialogBridgeInstalled = true;
  const ENDPOINT = "http://artemis-dialog-bridge.invalid/";
