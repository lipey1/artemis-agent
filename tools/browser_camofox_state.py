"""Artemis-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Artemis
...
def get_camofox_state_dir() -> Path:
    ...
    return get_artemis_home() / CAMOFOX_STATE_DIR_NAME / CAMOFOX_STATE_SUBDIR


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Artemis-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Artemis profile = same userId).
    ...
    return {
        "user_id": f"artemis_{user_digest}",
