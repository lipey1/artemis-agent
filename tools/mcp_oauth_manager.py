...
`mcp_tool.py`, and `artemis_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ArtemisMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_artemis_provider_class() -> Optional[type]:
    ...
    class ArtemisMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._artemis_server_name = server_name
            self._artemis_home = ""
            ...
            self._artemis_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ArtemisTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ArtemisTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._artemis_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ArtemisTokenStorage
                        if isinstance(storage, ArtemisTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._artemis_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ArtemisTokenStorage
            if not isinstance(storage, ArtemisTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``artemis mcp reauth``.
            ...
            back to ``artemis mcp reauth``.
            """
            try:
                if self._artemis_preregistered:
                    return
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._artemis_server_name,
                    artemis_home=self._artemis_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return ArtemisMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ARTEMIS_PROVIDER_CLS: Optional[type] = _make_artemis_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._artemis_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`ArtemisMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _ARTEMIS_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `artemis mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``artemis mcp remove <name>`` and (indirectly) by
        ``artemis mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, artemis_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, artemis_home=artemis_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, artemis_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, artemis_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, artemis_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(artemis_home) / f"{_safe_filename(server_name)}.json"
            try:
