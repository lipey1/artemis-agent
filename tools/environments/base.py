...
# ARTEMIS_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ARTEMIS_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ARTEMIS_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_artemis_home() / "sandboxes"
...
# the FIRST session's ARTEMIS_SESSION_ID into the snapshot makes every LATER
...
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ARTEMIS_SESSION_|ARTEMIS_UI_SESSION_ID|ARTEMIS_CRON_AUTO_DELIVER_|ARTEMIS_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ARTEMIS_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ARTEMIS_SESSION_*} ${!ARTEMIS_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / ARTEMIS_AGENT are per-command attribution markers
        ...
        "AI_AGENT ARTEMIS_AGENT "
        f"ARTEMIS_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ARTEMIS_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_artemis_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via ARTEMIS_DEBUG_INTERRUPT=1) -------------
