...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "artemis-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.artemis"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.artemis directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.artemis"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="artemis-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        ...
        # paths (e.g. home/user/.artemis/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.artemis".lstrip("/")
        ssh_cmd = self._build_ssh_command()
