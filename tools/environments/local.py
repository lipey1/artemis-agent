...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Hermes was already launched from (for example ``artemis-agent``
    while the process cwd is ``~/.hermes/artemis-agent``), passing it through
    ...
    # Common recovery for config values like ``artemis-agent`` when Hermes was
...
def build_subprocess_env(
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Hermes home override into ``ARTEMIS_HOME`` and apply the
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    """
    try:
        from artemis_constants import get_artemis_home, iter_hermes_node_dirs

        candidates = [*iter_hermes_node_dirs(), get_artemis_home() / "bin"]
...
hermes_subprocess_env = artemis_subprocess_env
