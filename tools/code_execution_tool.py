...
_HERMES_CHILD_ALLOWED = frozenset({
    ...
    "ARTEMIS_CONFIG",
    "ARTEMIS_ENV",
    "ARTEMIS_DELEGATED_CHILD_CONTEXT",
})
...
def _connect():
    ...
    ARTEMIS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ARTEMIS_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ARTEMIS_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"ARTEMIS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"ARTEMIS_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ARTEMIS_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from ARTEMIS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  ARTEMIS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ARTEMIS_RPC_SOCKET"] = rpc_endpoint
        child_env["ARTEMIS_RPC_TOKEN"] = rpc_token
        ...
        child_env.pop("ARTEMIS_TIMEZONE", None)
...
def _load_config() -> dict:
    ...
    ``artemis --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (artemis-agent's python). Reproducible and the
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the artemis root from every
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``artemis tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and artemis-agent's own python.
    if mode == "strict":
        cwd_note = (
