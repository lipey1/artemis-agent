...
409), and is driven by:
  ...
  * the ``artemis sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``artemis_cli/``) so the
...
is toggled via ``artemis sync enable/disable`` (a ``sync`` flag on the skill's
...
# only as the local *intent* the user toggles via ``artemis sync enable`` — it is
...
# namespace from artemis-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the ARTEMIS_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: ARTEMIS_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("ARTEMIS_SYNC_BASE_URL")
...
#
#   ARTEMIS_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ARTEMIS_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ARTEMIS_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    """
    return _sync_config_bool("ARTEMIS_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``ARTEMIS_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``artemis sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "ARTEMIS_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ARTEMIS_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.
    ...
    """
    return _sync_config_bool("ARTEMIS_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``artemis sync device --name`` to set an explicit
    label."""
    ...
    env_name = (os.environ.get("ARTEMIS_SYNC_DEVICE_NAME") or "").strip()
...
def push_skills(
    ...
    *,
    ...
    message: str = "artemis skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (artemis sync / NAS UI)."
            ),
        }
...
def maybe_push_skills(*, message: str = "artemis skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (ARTEMIS_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``artemis sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# `artemis sync propose` is the org sharing surface; proposal is
