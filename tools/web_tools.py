...
Backend is selected during ``artemis tools`` setup (web.backend in config.yaml).
When available, Artemis can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Artemis config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Artemis' config/.env layer (``artemis config set``, ``artemis tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.artemis/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``artemis tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `artemis tools` repaint.
        try:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `artemis plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `artemis tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`artemis plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from artemis_cli.config import get_env_value as _gev
