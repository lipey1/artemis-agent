#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Artemis desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Artemis window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Artemis desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "Artemis desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other Artemis "
        "windows are skipped: the nearest non-Artemis window is reported. "
