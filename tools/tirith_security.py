...
it is automatically downloaded from GitHub releases to $ARTEMIS_HOME/bin/tirith.
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_artemis_home(), ".tirith-install-failed")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ARTEMIS_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ARTEMIS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ARTEMIS_HOME/bin/tirith
