...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_artemis_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `artemis tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    ...
    try:
        ...
        return str(Path(get_artemis_home()) / "bin")
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_artemis_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "switch backends via `artemis tools` → Browser Automation."
        )
