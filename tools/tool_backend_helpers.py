...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `artemis model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``artemis auth add
    ...
    2. The environment / ``~/.hermes/.env``. Under a multiplexed gateway turn
       ...
       ``artemis_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``artemis auth
    ...
    when omitted, ``artemis_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `artemis auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``artemis auth add openai-api``) via
