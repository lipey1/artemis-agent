...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <ARTEMIS_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  ARTEMIS_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        base = Path(get_artemis_home()) / "hook_outputs"
