...
Copies bundled skills from the repo's skills/ directory into ~/.artemis/skills/
...
The manifest lives at ~/.artemis/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from artemis_constants import get_bundled_skills_dir, get_artemis_home, get_optional_skills_dir
...
SKILLS_DIR = ARTEMIS_HOME / "skills"
MANIFEST_FILE = SKILLS_DIR / ".bundled_manifest"

# Marker file written by `artemis profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.artemis profile).
# When present in ARTEMIS_HOME, sync_skills() is a no-op so neither the
# installer, `artemis update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# artemis_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks ARTEMIS_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.artemis/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.artemis/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``artemis skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `artemis skills reset {skill_name} --restore` "
...
def _product_skill_present() -> bool:
    ...
    if not SKILLS_DIR.exists():
        return False
    direct = SKILLS_DIR / "autonomous-ai-agents" / PRODUCT_SKILL_NAME / "SKILL.md"
    if direct.is_file():
        return True
    try:
        for skill_md in SKILLS_DIR.rglob("SKILL.md"):
            if is_excluded_skill_path(skill_md):
                continue
            if skill_md.parent.name == PRODUCT_SKILL_NAME:
                return True
    except OSError:
        return False
    return False
...
def ensure_bundled_skills(quiet: bool = True) -> dict:
    ...
    result["repaired_product_skill"] = False
    result["repaired_missing"] = missing
    return result


def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.artemis/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.artemis) that wrote the
    ...
    if (ARTEMIS_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.artemis/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `artemis update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `artemis skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``ARTEMIS_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.artemis/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``ARTEMIS_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `artemis skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `artemis update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``artemis update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``artemis update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``artemis skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `artemis skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ARTEMIS_HOME/.no-bundled-skills so the
    installer, ``artemis update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``artemis skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ARTEMIS_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `artemis update` (or `artemis skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ARTEMIS_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.artemis/skills/ ...")
    result = ensure_bundled_skills(quiet=False)
    parts = [
