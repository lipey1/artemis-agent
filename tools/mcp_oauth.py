...
This module provides the glue:
    - ``ArtemisTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Artemis Agent"
"""
...
from artemis_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__artemis_user_skipped__"
...
def _get_token_dir(artemis_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses ARTEMIS_HOME so each profile gets its own OAuth tokens.
    Layout: ``ARTEMIS_HOME/mcp-tokens/``
    """
    from artemis_constants import get_artemis_home

    base = Path(artemis_home) if artemis_home is not None else Path(get_artemis_home())
...
def _cached_redirect_port(storage: "ArtemisTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Artemis restarts and chooses a
...
def _cached_redirect_uri(storage: "ArtemisTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``artemis mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `artemis mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# ArtemisTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ArtemisTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ARTEMIS_HOME/mcp-tokens/<server_name>.json         -- tokens
        ARTEMIS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ARTEMIS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, artemis_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._artemis_home = Path(artemis_home) if artemis_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._artemis_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._artemis_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._artemis_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Artemis records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._artemis_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Artemis.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://artemis-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `artemis mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = ArtemisTokenStorage(server_name, artemis_home=artemis_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "ArtemisTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Artemis Agent" / "Artemis" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Artemis Agent")
    scope = cfg.get("scope")
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Artemis auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Artemis is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = ArtemisTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`artemis mcp login {server_name}` interactively first to complete "
