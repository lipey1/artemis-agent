...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.artemis/config.yaml``. Artemis
...
Configuration is loaded from ~/.artemis/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``artemis_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from artemis_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``artemis auth add <provider_id>``).
...
# Base URL now resolved via artemis_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_artemis_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.artemis/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.artemis/config.yaml.
    ...
    try:
        ...
        logger.debug("artemis_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``artemis tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Artemis without any Python code
...
#           output_format: wav
#
# Artemis writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ARTEMIS_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _run_command_tts(
    ...
    scrubbed = artemis_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from artemis the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``artemis_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `artemis setup` to configure, "
    ...
    from artemis_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `artemis model` or set XAI_API_KEY.")
    ...
    # output_format when Artemis actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import artemis_cli as _artemis_cli

            _artemis_version = str(_artemis_cli.__version__)
        except Exception:
            _artemis_version = "0.0.0"
        # Include Artemis client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Artemis caches Piper voice models.

    Resolves to ``~/.artemis/cache/piper-voices/`` under the active
    ARTEMIS_HOME so voice downloads follow profile boundaries.
    """
    from artemis_constants import get_artemis_dir
    root = Path(get_artemis_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("ARTEMIS_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `artemis setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run artemis setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'artemis setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'artemis tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_artemis_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
