...
remediation hint pointing at ``artemis tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (ARTEMIS_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (artemis trace upload / /upload-trace).
    #
    ...
    # presence, the `artemis update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "ARTEMIS_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ARTEMIS_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ARTEMIS_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``artemis update`` lazy-refresh pass.
    """
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``artemis update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``artemis_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = artemis_subprocess_env(inherit_credentials=False)
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # deployment sets ARTEMIS_MANAGED=true *and* ARTEMIS_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from artemis_cli.config import get_managed_system
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``ARTEMIS_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``ARTEMIS_DISABLE_LAZY_INSTALLS=1``) and sets
      ``ARTEMIS_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``artemis update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``artemis update``. Never raises; lazy-install failures
