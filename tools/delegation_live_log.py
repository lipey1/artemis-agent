...
per child under::
    ...
    <artemis_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.artemis)."""
    from artemis_constants import get_artemis_dir

    return get_artemis_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        try:
            ...
            header = [
                "=== Artemis subagent live transcript ===",
