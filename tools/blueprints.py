...
`artemis skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.artemis.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.artemis.blueprint`` is a mapping containing
    ...
    artemis = meta.get("artemis") if isinstance(meta, dict) else None
    blueprint = artemis.get("blueprint") if isinstance(artemis, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.artemis.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.artemis.blueprint`` block) they can hand
    to ``artemis skills publish`` to share. ``body`` is the plain-language
