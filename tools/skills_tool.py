...
# Cache validation (mirrors artemis_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.artemis/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set ARTEMIS_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped ARTEMIS_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ARTEMIS_HOME/.env."""
    env_path = get_artemis_home() / ".env"
    ...
    # the canonical readers in artemis_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ARTEMIS_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ARTEMIS_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ARTEMIS_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ARTEMIS_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_artemis_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.artemis/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ARTEMIS_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ARTEMIS_PLATFORM`` environment variable
    3. ``ARTEMIS_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from artemis_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ARTEMIS_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.artemis/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``artemis skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile ARTEMIS_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from artemis_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: artemis plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.artemis/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `artemis skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.artemis.* first (agentskills.io convention), fall back to top-level
        artemis_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            artemis_meta = metadata.get("artemis", {}) or {}

        tags = _parse_tags(artemis_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            artemis_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `artemis sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
