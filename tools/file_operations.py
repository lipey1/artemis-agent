...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__ARTEMIS_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__ARTEMIS_FENCE_" in line or "\x1b]" in line
...
NOT_REGULAR_SENTINEL = "__artemis_not_regular__"
...
class ShellFileOperations(FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        paths — and Artemis disables MSYS argument conversion for its bash
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.artemis-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".artemis-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.artemis-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.artemis-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str,
        ...
        # clean the temp up on any failure so we never leak a ``.artemis-tmp``
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # nothing converts back — Artemis sets MSYS_NO_PATHCONV for its bash).
