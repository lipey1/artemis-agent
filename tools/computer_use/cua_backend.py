...
etc.) surface as specific blocked checks via `artemis computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/artemis-agent#67052.
    """
...
# fetches the latest release, so a `ARTEMIS_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `ARTEMIS_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "ARTEMIS_CUA_DRIVER_CMD"
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/artemis-agent#47072: instead of hardcoding
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``ARTEMIS_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``ARTEMIS_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `artemis computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  artemis computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `artemis tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/artemis-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/artemis-agent#47072: per-tool
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/artemis-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/artemis-agent#47072: per-snapshot
    ...
    def capture(
        ...
        Maps artemis `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/artemis-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/artemis-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/artemis-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/artemis-agent#47072). cua-driver-rs gained an explicit
