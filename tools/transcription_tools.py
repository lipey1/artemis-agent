...
LOCAL_STT_COMMAND_ENV = "ARTEMIS_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ARTEMIS_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via artemis_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``ARTEMIS_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
#
# The single-env-var ``ARTEMIS_LOCAL_STT_COMMAND`` escape hatch is preserved
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ARTEMIS_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `artemis plugins list` to see "
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-local-stt-") as output_dir:
            ...
            child_env = artemis_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``ARTEMIS_LOCAL_STT_LANGUAGE`` (env). When none is set, Groq
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `artemis model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``artemis_cli.models`` helpers so every DeepInfra surface resolves the
