...
Design contract (matches the Artemis "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    artemis_meta = meta.get("artemis") if isinstance(meta, dict) else None
    if not isinstance(artemis_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.artemis.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in artemis_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.artemis.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("artemis", "agent", "artemis agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Artemis Agent' (proper caps) "
