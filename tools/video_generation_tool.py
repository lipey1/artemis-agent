...
provider** — turn on a backend by enabling a plugin (``artemis plugins
enable video_gen/<name>``) and selecting it in ``artemis tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/artemis-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `artemis tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `artemis plugins list` to see "
            f"installed video gen backends, or `artemis tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `artemis tools` → "
...
# `artemis tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`artemis tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `artemis tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
