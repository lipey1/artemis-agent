...
_SIGN_IN_MESSAGE = (
    ...
    "Ask the user to run `artemis model` and sign in to Nous, then retry."
)
...
def _default_directory():
    ...
    is not: an operator running ARTEMIS_MEDIA_DELIVERY_STRICT=1 delivers only
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return get_artemis_dir("cache/videos", "video_cache")
        except Exception:
