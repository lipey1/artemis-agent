"""Centralized logging setup for Artemis Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.artemis/logs/`` (profile-aware via ``get_artemis_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``artemis_cli.web_server``,
    ``artemis_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Artemis (TUI, gateway, ``hy_memory`` server, MCP
...
from artemis_constants import get_config_path, get_artemis_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_artemis_session_injector", False):
    ...
    _session_record_factory._artemis_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``artemis logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``artemis logs --component gateway``.
    "gateway": ("gateway", "artemis_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("artemis_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "artemis_cli.web_server",
        "artemis_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Artemis logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    artemis_home
        Override for the Artemis home directory.  Falls back to
        ``get_artemis_home()`` (profile-aware).
    log_level
    ...
    home = artemis_home or get_artemis_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_artemis_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._artemis_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the artemis group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from artemis_cli.config import is_managed
...
# the module header): when several Artemis processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._artemis_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="artemis-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_artemis_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse artemis_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when artemis_cli.config isn't importable (bare
        # artemis_logging consumers).
        try:
            from artemis_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
