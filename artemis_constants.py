"""Shared constants for Artemis Agent.
...
_UNSET = object()
_ARTEMIS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ARTEMIS_HOME_OVERRIDE", default=_UNSET
)
...
def set_artemis_home_override(path: str | Path | None) -> Token:
    """Set a context-local Artemis home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ARTEMIS_HOME_OVERRIDE.set(value)


def reset_artemis_home_override(token: Token) -> None:
    """Restore the previous context-local Artemis home override."""
    _ARTEMIS_HOME_OVERRIDE.reset(token)


def get_artemis_home_override() -> str | None:
    """Return the active context-local Artemis home override, if any."""
    override = _ARTEMIS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_artemis_home() -> Path:
    """Return the platform-native default Artemis home path."""
...
def _artemis_home_from_env() -> Path:
    """Resolve Artemis home from the process environment only.

    Prefers ``ARTEMIS_HOME``, then the
    platform-native default.  Deliberately ignores the context-local override
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``ARTEMIS_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_artemis_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[ARTEMIS_HOME fallback] ARTEMIS_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass ARTEMIS_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_artemis_home() -> Path:
    """Return the Artemis home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_artemis_home_override`) → ``ARTEMIS_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``ARTEMIS_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``ARTEMIS_HOME`` explicitly (see the systemd
    template in ``artemis_cli/gateway.py`` and the kanban dispatcher in
    ``artemis_cli/kanban_db.py``).  See https://github.com/NousResearch/artemis-agent/issues/18594.
    """
    override = get_artemis_home_override()
    if override:
        return Path(override)

    if not os.environ.get("ARTEMIS_HOME", "").strip():
        _warn_profile_fallback_once()

    return _artemis_home_from_env()


def get_process_artemis_home() -> Path:
    """Return the Artemis home for the running process, ignoring task overrides.

    Unlike :func:`get_artemis_home`, this never follows the context-local
    override set by :func:`set_artemis_home_override`.  It resolves only the
    process ``ARTEMIS_HOME`` env var (falling back to the platform default),
...
def get_default_artemis_root() -> Path:
    """Return the root Artemis directory for profile-level operations.

    In standard deployments this is the platform-native Artemis home
    (``~/.artemis`` on POSIX, ``%LOCALAPPDATA%\\artemis`` on native Windows).

    In Docker or custom deployments where ``ARTEMIS_HOME`` points outside
    ``~/.artemis`` (e.g. ``/opt/data``), returns ``ARTEMIS_HOME`` directly
    — that IS the root.

    In profile mode where ``ARTEMIS_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.artemis/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_artemis_home()
    env_home = os.environ.get("ARTEMIS_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # ARTEMIS_HOME is under ~/.artemis (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ARTEMIS_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ARTEMIS_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ARTEMIS_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_artemis_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``ARTEMIS_OPTIONAL_MCPS``.
    """
    override = os.getenv("ARTEMIS_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_artemis_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ARTEMIS_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<ARTEMIS_HOME>/skills`` last-resort
    """
    override = os.getenv("ARTEMIS_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_artemis_home() / "skills"
...
def get_artemis_dir(
    ...
    *,
    ...
    """Resolve a Artemis subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to ARTEMIS_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ARTEMIS_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Artemis home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local ARTEMIS_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_artemis_home()
...
def iter_artemis_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Artemis-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\artemis\\node``. POSIX installs use
    ``$ARTEMIS_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_artemis_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with artemisManagedNodePathEntries() in
...
_ARTEMIS_NODE_TARGET_MAJOR = int(os.environ.get("ARTEMIS_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Artemis-managed Node trees live under ``$ARTEMIS_HOME/node`` (or a profile's
    ``ARTEMIS_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_artemis_node_executable``
    used to trust file presence alone, so ``artemis update`` would pick that
...
def artemis_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Artemis-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_artemis_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%ARTEMIS_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_ARTEMIS_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_ARTEMIS_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$ARTEMIS_HOME/node`` on POSIX.
    ...
    ``ARTEMIS_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_artemis_managed_node() -> str | None:
    """Install a Artemis-managed Node tree and return its npm path.
    ...
    Artemis never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$ARTEMIS_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_artemis_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_artemis_node_dirs():
...
def heal_artemis_managed_node() -> bool:
    """Redownload Artemis-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not artemis_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ARTEMIS_HOME": str(get_artemis_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_artemis_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_ARTEMIS_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_artemis_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _ARTEMIS_NODE_TARGET_MAJOR
    return False


def find_artemis_node_executable(command: str) -> str | None:
    """Return a Artemis-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_ARTEMIS_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_artemis_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Artemis-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Artemis-managed installs.

    This is for Artemis-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_artemis_node_executable(command)
    if managed:
        return managed
    if artemis_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_artemis_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Artemis-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_artemis_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``artemis update`` — leaving a **dangling symlink** that ``which`` still
...
def display_artemis_home() -> str:
    """Return a user-friendly display string for the current ARTEMIS_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.artemis``
        profile:  ``~/.artemis/profiles/coder``
        custom:   ``/opt/artemis-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.artemis``.  For code that needs a real ``Path``, use
    :func:`get_artemis_home` instead.
    """
    home = get_artemis_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ARTEMIS_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/artemis-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ARTEMIS_HOME}/home`` when the profile-home directory exists."""
    artemis_home = get_artemis_home_override() or (env or {}).get("ARTEMIS_HOME") or os.getenv("ARTEMIS_HOME")
    if not artemis_home:
        return None
    profile_home = os.path.join(artemis_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ARTEMIS_REAL_HOME") or os.getenv("ARTEMIS_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Artemis profile HOME.

    ``ARTEMIS_HOME`` scopes Artemis state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ARTEMIS_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ARTEMIS_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ARTEMIS_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Artemis' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ARTEMIS_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Artemis itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/artemis-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ARTEMIS_HOME.

    Replaces the ``get_artemis_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, artemis_logging.py, artemis_time.py, etc.).
    """
    return get_artemis_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ARTEMIS_HOME."""
    return get_artemis_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ARTEMIS_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_artemis_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._artemis_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``artemis_cli`` modules using three different Windows
    ...
    A few sites outside ``artemis_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``artemis_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Artemis itself. An ImportError
...
# `artemis_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Artemis.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("artemis_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `artemis update` is exactly
...
display_hermes_home = display_artemis_home
iter_hermes_node_dirs = iter_artemis_node_dirs
with_hermes_node_path = with_artemis_node_path
find_hermes_node_executable = find_artemis_node_executable
heal_hermes_managed_node = heal_artemis_managed_node
bootstrap_hermes_managed_node = bootstrap_artemis_managed_node
get_process_hermes_home = get_process_artemis_home
