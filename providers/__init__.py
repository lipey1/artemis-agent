...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with artemis-agent)
2. User plugins: ``$ARTEMIS_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ARTEMIS_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ARTEMIS_HOME/plugins/model-providers/`` if it exists."""
    try:
        from artemis_constants import get_artemis_home

        d = get_artemis_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ARTEMIS_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ARTEMIS_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with artemis-agent.
    ...
    # 2. User plugins — under $ARTEMIS_HOME/plugins/model-providers/<name>/.
