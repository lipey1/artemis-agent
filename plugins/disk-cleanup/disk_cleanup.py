...
Rules:
  ...
  - empty dirs    → always delete (under ARTEMIS_HOME)
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ARTEMIS_HOME/logs/``."""
    return get_artemis_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ARTEMIS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(artemis_home)
        return True
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "artemis-agent", "backups", "profiles", ".worktrees",
...
# Paths under $ARTEMIS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ARTEMIS_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        artemis_home = get_artemis_home()
        for parent in ("cron", "cronjobs"):
            base = artemis_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ARTEMIS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    try:
        for top in artemis_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    artemis_home = get_artemis_home()
    try:
        rel = path.resolve().relative_to(artemis_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "artemis-agent",
