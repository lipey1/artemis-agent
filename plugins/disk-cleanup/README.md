...
Auto-tracks and cleans up ephemeral files created during Artemis Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ARTEMIS_HOME` and `/tmp/artemis-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ARTEMIS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ARTEMIS_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ARTEMIS_HOME` or `/tmp/artemis-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ARTEMIS_HOME/disk-cleanup/` is itself excluded
- `$ARTEMIS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
