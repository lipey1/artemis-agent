...
three virtual model IDs so the ``artemis tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `artemis tools` → Image "
                    "Generation → OpenAI to configure, or `artemis setup` "
                    "to add the key."
                ),
