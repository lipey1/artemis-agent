...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=artemis``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`artemis_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``artemis tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from artemis the next time the catalog is fetched, no patch required.
...
def _live_models() -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        from artemis_cli.models import _fetch_deepinfra_models_by_tag
    except Exception as exc:
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `artemis tools` → Image "
                    "Generation → DeepInfra to configure, or `artemis setup` "
                    "to add the key."
                ),
        ...
        from artemis_cli.models import deepinfra_base_url
