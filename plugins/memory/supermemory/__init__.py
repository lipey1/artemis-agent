...
_DEFAULT_CONTAINER_TAG = "artemis"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=artemis"
...
def _load_supermemory_config(artemis_home: str) -> dict:
    config = _default_config()
    config_path = Path(artemis_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, artemis_home: str) -> None:
    config_path = Path(artemis_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "artemis"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Artemis writes into the "Artemis" Space in the Supermemory
        ...
        merged = {"sm_source": "artemis", **(metadata or {})}
...
def _resolve_container_tag_for_setup(artemis_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(artemis_home)
...
def _probe_supermemory_connection(api_key: str, artemis_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(artemis_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(artemis_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `artemis memory setup`.
        # All other options are documented for $ARTEMIS_HOME/supermemory.json
    ...
    def save_config(self, values, artemis_home):
        ...
        _save_supermemory_config(sanitized, artemis_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, artemis_home)
    ...
    def post_setup(self, artemis_home: str, config: dict) -> None:
        ...
        from artemis_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(artemis_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from artemis_constants import get_artemis_home
        self._artemis_home = kwargs.get("artemis_home") or str(get_artemis_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._artemis_home)
