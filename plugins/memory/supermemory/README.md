...
- Hosted: API key from [app.supermemory.ai/integrations?connect=artemis](http://app.supermemory.ai/integrations?connect=artemis)
...
```bash
artemis memory setup    # select "supermemory"
```

Or manually:

```bash
artemis config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.artemis/.env
```
...
Before running `artemis memory setup`, add the local endpoint to
`$ARTEMIS_HOME/supermemory.json`:
...
Then run `artemis memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ARTEMIS_HOME/supermemory.json`
...
| `container_tag` | `artemis` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `artemis-{identity}` → `artemis-coder`). |
...
`https://api.supermemory.ai`. Artemis resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: artemis`, and document writes stamp
`metadata.sm_source: artemis`. This is a **functional routing key, not telemetry**:
it groups Artemis-written memories into a dedicated "Artemis" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Artemis profile:

```json
{
  "container_tag": "artemis-{identity}"
}
```

For a profile named `coder`, this resolves to `artemis-coder`. The default profile resolves to `artemis-default`. Without `{identity}`, all profiles share the same container.
