...
Configuration
-------------
Secret (lives in $ARTEMIS_HOME/.env or the environment):
...
Behavioral settings (live in $ARTEMIS_HOME/mem0.json, set via `artemis memory
setup`):
  ...
  agent_id           — Agent identifier (default: artemis)
...
def _load_config() -> dict:
    """Load config from env vars, with $ARTEMIS_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "artemis"),
        "oss": {},
    }
    ...
    config_path = get_artemis_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, artemis_home):
        """Write config to $ARTEMIS_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, artemis_home: str, config: dict) -> None:
        ...
        post_setup(artemis_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $ARTEMIS_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "artemis")
