...
```bash
artemis memory setup    # select "mem0"
```

Or manually:
```bash
artemis config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.artemis/.env
```

## Config

Behavioral settings live in `$ARTEMIS_HOME/mem0.json` (set them via `artemis memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.artemis/.env`.
...
| `user_id` | `artemis-user` | User identifier on Mem0 |
| `agent_id` | `artemis` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   artemis memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   artemis memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.artemis/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.artemis/.env
   ```
   or in `$ARTEMIS_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Artemis session and call `mem0_search` — it connects to your server.
...
```bash
artemis memory setup mem0 --mode oss \
...
```bash
artemis memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ARTEMIS_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.artemis/mem0_qdrant"}}
  }
}
```
...
```bash
artemis memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
artemis memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$ARTEMIS_HOME/mem0.json`).
