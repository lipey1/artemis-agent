...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # artemis_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(artemis_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(artemis_home) / "mem0.json"
    existing = {}
...
def _setup_platform(artemis_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider.save_config(provider_config, artemis_home)
...
def _setup_selfhosted(artemis_home: str, config: dict, flags: dict[str, str]) -> None:
...
def _setup_oss(artemis_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(artemis_home, config)
        return
...
def _prompt_api_key(label: str, env_var: str, artemis_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # artemis_cli/config.py; a Notepad BOM on the first line would
...
def _setup_oss_interactive(artemis_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], artemis_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], artemis_home)
        if key:
    ...
    if env_writes:
        _write_env(Path(artemis_home) / ".env", env_writes)
    _save_mem0_json(artemis_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(artemis_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(artemis_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(artemis_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(artemis_home, config, flags)
        return
