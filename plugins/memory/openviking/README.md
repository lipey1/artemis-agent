...
- OpenViking server running and reachable from Artemis

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Artemis can identify older servers that expose the legacy status-only health
...
Then configure Artemis:

```bash
artemis memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Artemis, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
artemis config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.artemis/.env`; for a named profile use
`~/.artemis/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=artemis
```

## Config

OpenViking's server config is separate from Artemis:
...
Artemis-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `artemis` | Artemis peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Artemis lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Artemis sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Artemis built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Artemis action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Artemis
...
file URIs, such as
`viking://user/peers/artemis/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/artemis/memories/preferences/mem_abc123.md`. Files
