...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Artemis peer ID in OpenViking (default: artemis)
...
_DEFAULT_AGENT = "artemis"
_AGENT_PROMPT_LABEL = "Artemis peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ARTEMIS_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Artemis slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Artemis to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Artemis refused the connection."
        ) from exc
...
def _load_artemis_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ARTEMIS_HOME", "")).expanduser() if os.environ.get("ARTEMIS_HOME") else Path.home() / ".artemis"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Artemis did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Artemis will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Artemis session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Artemis only", "write values only to Artemis .env"),
    ...
    _save_artemis_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Artemis .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See artemis-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_artemis_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Artemis peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], artemis_home: str) -> None:
    ...
    def post_setup(self, artemis_home: str, config: dict) -> None:
        ...
        from artemis_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        artemis_home_path = Path(artemis_home)
        env_path = artemis_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_artemis_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        artemis_home = str(kwargs.get("artemis_home") or "").strip()
        if not artemis_home:
            try:
                from artemis_constants import get_artemis_home
                artemis_home = str(get_artemis_home())
            except Exception:
                artemis_home = str(Path.home() / ".artemis")
        self._artemis_home = artemis_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Artemis will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.artemis/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts artemis (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Artemis will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._artemis_home:
            return None
        return Path(self._artemis_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._artemis_home:
            return None
        return Path(self._artemis_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_artemis_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Artemis' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Artemis canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See artemis-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/artemis/.overview.md).
