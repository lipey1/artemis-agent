...
```bash
artemis memory setup    # select "byterover"
```

Or manually:
```bash
artemis config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.artemis/.env
```
...
Working directory: `$ARTEMIS_HOME/byterover/` (profile-scoped).
