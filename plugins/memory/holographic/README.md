...
```bash
artemis memory setup    # select "holographic"
```

Or manually:
```bash
artemis config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.artemis-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ARTEMIS_HOME/memory_store.db` | SQLite database path |
