...
``artemis://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Artemis OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "artemis-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``artemis-desktop`` / ``artemis-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Artemis.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ARTEMIS_HOME override can't reach.
