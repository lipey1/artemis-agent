...
Config: Uses the existing Honcho config chain:
  1. $ARTEMIS_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, artemis_home):
        """Write config to $ARTEMIS_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(artemis_home) / "honcho.json"
        existing = {}
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_artemis_home() / "memories")
