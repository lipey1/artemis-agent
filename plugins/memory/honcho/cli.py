...
Handles: artemis honcho setup | status | sessions | map | peer
"""
...
from artemis_cli.config import cfg_get
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "artemis" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Artemis profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'artemis honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'artemis honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `artemis update` -- no output, no exceptions.
    """
    try:
        from artemis_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Artemis profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ARTEMIS_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_artemis_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        artemis_host.get("pinUserPeer"),
        artemis_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in artemis_host:
        aliases_src = artemis_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in artemis_host:
        prefix_src = artemis_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(artemis_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom artemis_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    artemis_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        artemis_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(artemis_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = artemis_host.get("userPeerAliases")
    prefix = artemis_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(artemis_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(artemis_host)
    artemis_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        artemis_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        artemis_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/artemis (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Artemis persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = artemis_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            artemis_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(artemis_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'artemis honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'artemis honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'artemis honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(artemis_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'artemis honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = artemis_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        artemis_host["peerName"] = new_peer

    current_ai = artemis_host.get("aiPeer") or cfg.get("aiPeer", "artemis")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        artemis_host["aiPeer"] = new_ai

    current_workspace = artemis_host.get("workspace") or cfg.get("workspace", "artemis")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        artemis_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Artemis GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, artemis_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Artemis gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = artemis_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(artemis_host)
            artemis_host["pinUserPeer"] = False
            if prior_aliases:
                artemis_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                artemis_host, current_prefix, prefix_from_root,
            ...
            if merged:
                artemis_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                artemis_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                artemis_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = artemis_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        artemis_host["observationMode"] = new_obs
    else:
        artemis_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(artemis_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        artemis_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        artemis_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = artemis_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        artemis_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = artemis_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        artemis_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                artemis_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(artemis_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            artemis_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        artemis_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        artemis_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        artemis_host["dialecticReasoningLevel"] = new_reasoning
    else:
        artemis_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = artemis_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        artemis_host["sessionStrategy"] = new_strat

    artemis_host["enabled"] = True
    artemis_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        artemis_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(artemis_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: artemis config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    artemis honcho status     -- show full config")
    print("    artemis honcho mode       -- change recall/observation mode")
    print("    artemis honcho tokens     -- tune context and dialectic budgets")
    print("    artemis honcho peer       -- update peer names")
    print("    artemis honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if artemis_cli internals move."""
    try:
        from artemis_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(artemis_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    artemis_host["apiKey"] = cred.access_token
    artemis_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        artemis_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Artemis profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: artemis honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'artemis honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'artemis honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: artemis honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        artemis = hosts.get(_host_key(), {})
        user = artemis.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = artemis.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = artemis.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = artemis.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Artemis' identity in Honcho. Seed with 'artemis honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: artemis honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: artemis honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = artemis.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = artemis.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = artemis.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Artemis asks Honcho's AI peer a question")
        ...
        print("\n  Set with: artemis honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'artemis honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'artemis honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    artemis honcho identity --show        — show both peer representations")
        print("    artemis honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Artemis + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Artemis\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Artemis persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  artemis honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'artemis honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'artemis honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  artemis honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'artemis'")
        print("  with Honcho configured and no prior session history.")
        print("  (Artemis calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    artemis honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'artemis honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Artemis, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'artemis honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    artemis honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  artemis honcho identity <path/to/SOUL.md>")
    ...
    print("    Artemis:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Artemis:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Artemis:   Honcho observes every message and updates representations")
    ...
    print("    Artemis:   per-session by default — each run gets its own session")
    print("              Map a custom name:  artemis honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. artemis honcho setup              — configure API key (required)")
        print("  2. artemis honcho migrate            — re-run this walkthrough")
    else:
        print("  1. artemis honcho status             — verify Honcho connection")
        print("  2. artemis                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. artemis honcho identity --show    — verify AI peer representation")
        print("  4. artemis honcho tokens             — tune context and dialectic budgets")
        print("  5. artemis honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'artemis memory setup'...\n")
        from artemis_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``artemis honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``artemis honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to artemis memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Artemis Honcho",
    )
