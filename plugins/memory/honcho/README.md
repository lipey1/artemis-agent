...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/artemis>
...
```bash
artemis memory setup honcho   # configure Honcho directly (works on a fresh install)
artemis memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
artemis config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.artemis/.env
```

> `artemis honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `artemis memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `artemis model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ARTEMIS_HOME/honcho.json` | Profile-local (isolated Artemis instances) |
| 2 | `~/.artemis/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Artemis profile: `artemis` (default) or `artemis_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `artemis honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Artemis session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`artemis-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`artemis`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-artemis-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening artemis in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Artemis session ID (timestamp + hex). Every `artemis` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Artemis profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`artemis`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Artemis profile: `artemis` (default) or `artemis_<profile>` (e.g. `artemis -p coder` -> host key `artemis_coder`). Older `artemis.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ARTEMIS_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `artemis-agent`) |
...
|---------|-------------|
| `artemis memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `artemis honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `artemis memory setup`) |
| `artemis honcho status` | Show resolved config for active profile |
| `artemis honcho enable` / `disable` | Toggle Honcho for active profile |
| `artemis honcho mode <mode>` | Change recall or observation mode |
| `artemis honcho peer --user <name>` | Update user peer name |
| `artemis honcho peer --ai <name>` | Update AI peer name |
| `artemis honcho tokens --context <N>` | Set context token budget |
| `artemis honcho tokens --dialectic <N>` | Set dialectic max chars |
| `artemis honcho map <name>` | Map current directory to a session name |
| `artemis honcho sync` | Create host blocks for all Artemis profiles |
