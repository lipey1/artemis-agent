...
This plugin connects Artemis Agent to iMessage (and other Spectrum
...
TypeScript-only, Artemis runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Artemis channel,
...
Run `artemis photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.artemis/.env` (the same place every other
...
Management metadata lives in `~/.artemis/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `artemis photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Artemis
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Artemis coalesces
...
- **Native polls are supported.** Artemis posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `artemis send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<artemis-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `artemis photon status`, a DM and a group roundtrip,
