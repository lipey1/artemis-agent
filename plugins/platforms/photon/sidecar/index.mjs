// Artemis Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Artemis): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Artemis -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Artemis-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `artemis photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Artemis can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "artemis-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Artemis'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Artemis allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Artemis, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-artemis-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Artemis supplied them so a known-good
