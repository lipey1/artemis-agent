...
Small Node helper that bridges Artemis Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Artemis is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Artemis-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Artemis can receive inbound messages
...
The Artemis plugin's `artemis photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Artemis is Python, so replies go through this sidecar
