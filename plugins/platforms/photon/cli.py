"""
``artemis photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Artemis
...
# Writable sidecar runtime dir (mirrors to ARTEMIS_HOME on immutable
...
# wiring imports this module for `artemis --help`).
...
# when called later (gateway start, artemis status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `artemis photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Artemis Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``artemis photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Artemis Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.artemis/.env,
    ...
    try:
        from artemis_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  artemis gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `artemis photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`artemis photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.artemis/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `artemis photon telemetry on` / `artemis photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.artemis/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  artemis gateway restart")
    return 0
...
#
# `artemis gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``artemis photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `artemis gateway setup` wizard."""
