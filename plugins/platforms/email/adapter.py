"""
Email platform adapter for the Artemis gateway.

Allows users to interact with Artemis by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "artemis-agent" "version" "{_artemis_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `artemis config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `artemis gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Artemis Agent")
        ...
        msg_id = f"<artemis-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# artemis_cli/gateway.py, and the _send_email dispatch in
