"""ntfy platform adapter (Artemis plugin).
...
a Artemis dependency.

This adapter ships as a Artemis platform plugin under
``plugins/platforms/ntfy/``. The Artemis plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "artemis-in"              # subscribe topic (incoming)
          publish_topic: "artemis-out"     # optional — defaults to topic
...
_ECHO_TAG = "artemis-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``artemis cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Artemis dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `artemis gateway status` without
