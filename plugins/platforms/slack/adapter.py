...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_artemis_home() / "slack_tokens.json"
        ...
        try:
            ...
            from artemis_cli.commands import slack_native_slashes
            import re as _re
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"hermes", ""}:
            ...
            from artemis_cli.commands import slack_subcommand_map
...
# ``artemis_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_artemis_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``artemis_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ARTEMIS_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from artemis_cli.slack_cli import _build_full_manifest
            from artemis_constants import get_artemis_home
            import json as _json
    ...
    print()
    print_info("   Full guide: https://artemis-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``artemis_cli.gateway.get_env_value`` at call time (not via the
