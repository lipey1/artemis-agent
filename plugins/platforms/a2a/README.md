# A2A — Agent-to-Agent protocol for Artemis
...
peer (another Artemis, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
artemis gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Artemis serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.artemis/a2a_audit.jsonl`.
- Conversations persist to `~/.artemis/a2a_conversations/` — they survive context
