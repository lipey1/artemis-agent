"""SimpleX Chat platform adapter (Artemis plugin).
...
This adapter ships as a Artemis platform plugin under
``plugins/platforms/simplex/``. The Artemis plugin loader scans the
...
Optional environment variables:
    ...
    ARTEMIS_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``artemis setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Artemis — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ARTEMIS_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``artemis setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.artemis/.env`` via
    ``artemis_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "artemis_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.artemis/.env"
        )
        return
