"""
LINE Messaging API platform adapter for Artemis Agent.
...
to Artemis Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ARTEMIS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            artemis_home = Path(get_artemis_home()).resolve()
        except Exception:
            artemis_home = Path.home().joinpath(".artemis").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``artemis status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``artemis status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``artemis setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.artemis/.env`` via ``artemis_cli.config``.
    """
    print()
    ...
    try:
        from artemis_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("artemis_cli.config not available; set LINE_* vars manually in ~/.artemis/.env")
        return
