...
# ``model_tools`` import time, so ``artemis status``, ``artemis chat``, etc.
...
# ``artemis_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Artemis is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a artemis MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "artemis-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ARTEMIS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _artemis_home = _get_artemis_home()
        except (ModuleNotFoundError, ImportError):
            _artemis_home = _Path.home() / ".artemis"
        self._thread_count_store = _ThreadCountStore(
            _artemis_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Artemis is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ARTEMIS_HOME", str(_Path.home() / ".artemis"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ARTEMIS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a artemis MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Artemis is thinking…' marker message.
        ...
        "Artemis is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Artemis is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Artemis is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Artemis is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``artemis setup``.

    The setup wizard at ``artemis_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.artemis/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. artemis-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.artemis/.env")
    print_info("Restart the gateway: artemis gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``artemis cron`` running as a
    separate process from ``artemis gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Artemis plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `artemis setup` to install Google Chat support.",
        ...
        # the "Artemis is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Artemis is thinking…' marker message "
