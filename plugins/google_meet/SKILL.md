...
```bash
artemis plugins enable google_meet
artemis meet install                 # pip deps + Chromium (transcribe only)
artemis meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
artemis meet auth                    # optional; skips guest-lobby wait
artemis meet setup                   # preflight checks
```

`artemis meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ARTEMIS_MEET_REALTIME_KEY in ~/.artemis/.env
```
...
pip install playwright websockets && python -m playwright install chromium
artemis plugins enable google_meet
artemis meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
artemis meet node approve my-mac ws://<mac-ip>:18789 <token>
artemis meet node ping my-mac                   # confirm reachable
```

Run `artemis meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Artemis agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `artemis meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ARTEMIS_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `artemis_meet_src`). |
...
Local:
```
$ARTEMIS_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ARTEMIS_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `artemis meet node approve`.
