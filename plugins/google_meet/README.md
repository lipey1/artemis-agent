...
Let the artemis agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where artemis runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `artemis meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `artemis meet` CLI |
...
| `cli.py` | `artemis meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ARTEMIS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `artemis meet node {run,list,approve,remove,status,ping}` |
...
artemis meet install                                      # pip + Chromium
artemis meet setup                                        # preflight
artemis meet auth                                         # optional
artemis meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
artemis meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.artemis/.env
artemis meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
artemis meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
artemis meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.artemis/.env
artemis meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, artemis will **not** switch your system audio input automatically — the
...
artemis plugins enable google_meet
artemis meet node run --display-name my-mac --host 0.0.0.0 --port 18789
