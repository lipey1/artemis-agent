"""`artemis meet node ...` subcommand tree.

Wired into the existing ``artemis meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``artemis meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="artemis-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``artemis meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             artemis meet node approve <name> ws://<host>:{args.port} {token}")
        try:
