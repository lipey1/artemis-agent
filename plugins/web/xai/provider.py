...
``{title, url, description, position}`` rows every other Artemis web
provider produces.
...
prefers Artemis-managed xAI Grok OAuth (via ``artemis auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.artemis/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``artemis tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `artemis auth` to sign in with "
