#!/usr/bin/env python3
"""
SQLite State Store for Artemis Agent.
...
from artemis_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from artemis_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from artemis_state_portability import SessionPortabilityMixin
from artemis_state_schema import SessionSchemaMixin
from artemis_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``artemis -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_artemis_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.artemis`` even when a test fixture later
    redirects ``ARTEMIS_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_artemis_home()`` fresh so a runtime
       ``ARTEMIS_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_artemis_home() / "state.db"
...
# ~/.artemis/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects ARTEMIS_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without ARTEMIS_HOME, a stale worktree
# without the re-pin, or a developer shell that exports ARTEMIS_HOME to the
...
#: ``~/.artemis``).  The test conftest injects the pre-sandbox production
#: root here so custom-``ARTEMIS_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Artemis root for the guard.

    Deliberately avoids ``Path.home()`` / ``artemis_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``artemis_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "artemis"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "artemis"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".artemis"
        return root.resolve()
    except Exception:
        return None
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Artemis home *root*.
    ...
    under ``~/.artemis/artemis-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``ARTEMIS_HOME``) paths.
    """
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Artemis root {root}). "
                "Tests must run against a temporary ARTEMIS_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ARTEMIS_HOME. If this test genuinely needs the live "
...
# artemis_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``artemis_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``artemis update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Artemis-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Artemis"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Artemis-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `artemis doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see artemis_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with artemis_cli.config.
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``artemis_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Artemis home tree (``get_artemis_home()``) — the safe repair scope:
      Artemis owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``artemis_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_artemis_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Artemis needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (artemis_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_artemis_state.py:600-632)
        ...
        probe_session_id = f"_artemis_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.artemis/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``artemis sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ARTEMIS_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_artemis_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ARTEMIS_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship artemis_state without artemis_cli). A
    ...
    try:
        ...
        logger.debug(
            "artemis_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch artemis_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``artemis_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from artemis_cli.backup import is_zeroed_sqlite_file
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `artemis snapshot list` / "
                "`artemis snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (artemis doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple artemis processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Artemis
    ...
    # `artemis update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `artemis snapshot list` / "
                    f"`artemis snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`artemis db optimize`),
            ...
            # ``artemis_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `artemis update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `artemis sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Artemis process held the "
    ...
    #
    # `optimize_fts_storage()` (the `artemis sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`artemis sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``artemis sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``artemis --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: artemis_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``artemis -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``artemis sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``ARTEMIS_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Artemis, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Artemis session.

        A Artemis session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Artemis session is already bound to any Telegram DM topic.
