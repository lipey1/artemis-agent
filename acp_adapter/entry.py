...
def _load_env() -> None:
    ...
    loaded = load_artemis_dotenv(artemis_home=artemis_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", artemis_home / ".env"
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting artemis-agent ACP adapter")
    ...
    if os.environ.get("ARTEMIS_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
