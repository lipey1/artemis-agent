"""ACP permission bridging for Artemis dangerous-command approvals."""
...
# Maps ACP permission option ids to Artemis approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match Artemis approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping Artemis semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_artemis(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Artemis approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_ARTEMIS.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Artemis-compatible approval callback that bridges to ACP.
    ...
    def _callback(
        ...
        **_: object,
        ...
        return _map_outcome_to_artemis(
            response.outcome,
