"""ACP auth helpers — detect and advertise Artemis authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "artemis-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Artemis runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Artemis can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Artemis.
    ...
    not have Artemis provider credentials configured yet, so Artemis always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Artemis using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Artemis provider",
            description=(
                "Open Artemis' interactive model/provider setup in a terminal. "
                "Use this when Artemis has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
