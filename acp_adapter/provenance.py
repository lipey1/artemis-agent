...
This is an additive Artemis extension surfaced under ACP ``_meta.artemis`` so
...
compression rotates the internal Artemis head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.artemis.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_artemis_session_id: The live internal Artemis DB session id
            (``state.agent.session_id``).
        previous_artemis_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"artemis": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_artemis_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_artemis_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_artemis_session_id}
    ...
    rotated = bool(
        ...
        and previous_artemis_session_id != current_artemis_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentArtemisSessionId": current_artemis_session_id,
        "rootArtemisSessionId": root_id,
        "parentArtemisSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_artemis_session_id:
        provenance["previousArtemisSessionId"] = previous_artemis_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"artemis": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"artemis": {"sessionProvenance": prov}}
