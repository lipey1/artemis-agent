"""Windows UTF-8 bootstrap for Artemis entry points.
...
should be imported at the very top of every Artemis entry point
(``artemis``, ``artemis-agent``, ``artemis-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``artemis_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``artemis_bootstrap``, never ``artemis_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Artemis modules.

    Artemis ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Artemis source root
    ...
    """
    root = src_root or os.environ.get("ARTEMIS_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ARTEMIS_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ARTEMIS_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import artemis_bootstrap``
# (or ``from artemis_bootstrap import apply_windows_utf8_bootstrap``) at
