...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``artemis://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``artemis://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"artemis://blueprint/{quote(blueprint.key)}{qs}"
