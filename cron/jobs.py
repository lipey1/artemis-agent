...
# `_get_artemis_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
ARTEMIS_DIR = get_artemis_home().resolve()
...
CRON_DIR = ARTEMIS_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``artemis cron status`` process share it
...
# threshold in `artemis cron status` (artemis_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_artemis_home()
       (context-local override, then the ARTEMIS_HOME env var) — so a test
       or embedder that re-points ARTEMIS_HOME after this module was
...
# the cron inactivity timeout is disabled (ARTEMIS_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. ARTEMIS_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``ARTEMIS_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec artemis artemis cron create ...`` — ``docker exec``
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall artemis-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `artemis cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``artemis cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `artemis cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``artemis cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``artemis cron
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_artemis_home() / "config.yaml"
...
def create_job(
    ...
    # covered, not just `artemis cron create`.
...
def _machine_id() -> str:
    ...
    Uses ``ARTEMIS_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("ARTEMIS_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from ARTEMIS_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one ARTEMIS_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from ARTEMIS_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`artemis_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``artemis_cli.backup._prune_quick_snapshots``:
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``ARTEMIS_HOME/skills`` or an
