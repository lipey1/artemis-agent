...
``artemis cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `artemis gateway restart|stop` — the canonical foot-gun.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_artemis_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_artemis_home() falls back to Path.home(), which raises when
        # neither ARTEMIS_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "artemis gateway restart"
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `artemis gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `artemis gateway restart` from a shell outside "
