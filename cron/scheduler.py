...
Uses a file-based lock (~/.artemis/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `artemis update` reloads
# the module) fail with ModuleNotFoundError for artemis_time et al.
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy artemis_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from artemis_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``artemis tools`` config for the ``cron`` platform.
    ...
    try:
        from artemis_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
# (ARTEMIS_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse ARTEMIS_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("ARTEMIS_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid ARTEMIS_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_artemis_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_artemis_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.artemis/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``artemis update`` / ``artemis gateway stop`` / systemd restart, or an
...
_artemis_home: Path | None = None


def _get_artemis_home() -> Path:
    """Resolve Artemis home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active ARTEMIS_HOME at call time
    ...
    """
    return _artemis_home or get_artemis_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    artemis_home = _get_artemis_home()
    lock_dir = artemis_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Artemis — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ARTEMIS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ARTEMIS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within ARTEMIS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Artemis-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ARTEMIS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_artemis_home() / "scripts"
    ...
    # escape — scripts MUST reside within ARTEMIS_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("ARTEMIS_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `artemis setup`), or pin a "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`artemis setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _artemis_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _artemis_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    #
    # Bounded with its own timeout (separate from ARTEMIS_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("ARTEMIS_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ARTEMIS_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_artemis_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_artemis_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed ARTEMIS_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ARTEMIS_SESSION_PLATFORM
    #     and ARTEMIS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ARTEMIS_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ARTEMIS_SESSION_PLATFORM == "telegram".
    ...
    #     ARTEMIS_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ARTEMIS_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ARTEMIS_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ARTEMIS_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # ARTEMIS_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ ARTEMIS_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "ARTEMIS_CRON_AUTO_DELIVER_PLATFORM",
        "ARTEMIS_CRON_AUTO_DELIVER_CHAT_ID",
        "ARTEMIS_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["ARTEMIS_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `artemis chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with ARTEMIS_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $ARTEMIS_KANBAN_TASK -- letting
        ...
        # load_artemis_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this ARTEMIS_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_artemis_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from artemis_cli.env_loader import (
            load_artemis_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["ARTEMIS_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ARTEMIS_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ARTEMIS_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > ARTEMIS_MODEL env > config.yaml ``model:``
        ...
        try:
            ...
            _cfg_path = str(_get_artemis_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"ARTEMIS_MODEL={os.getenv('ARTEMIS_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `artemis model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from artemis_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("ARTEMIS_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_artemis_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject ARTEMIS_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ARTEMIS_HOME. When a workdir is configured, also inject project
        ...
        # override via ARTEMIS_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from artemis_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_artemis_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_artemis_now().strftime('%b %d %H:%M')}"
...
def run_one_job(
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_artemis_home())
        )
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`artemis pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `artemis resume`. Logged once per
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _artemis_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _artemis_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ARTEMIS_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ARTEMIS_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ARTEMIS_CRON_MAX_PARALLEL value; defaulting to unbounded")
