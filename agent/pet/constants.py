...
row taxonomy is inferred from the atlas shape so Artemis can render both legacy
...
# rows (1536x1872), while older Artemis/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Artemis' activity state names. They are not always identical to the
...
# Legacy Artemis/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Artemis activity names -> accepted row-name aliases in descending
