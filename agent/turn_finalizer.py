...
decomposition campaign (``~/.artemis/plans/god-file-decomposition.md``, Phase 1
...
def finalize_turn(
    ...
    *,
    ...
    if iteration_limit_fallback:
        ...
        _kanban_task = os.environ.get("ARTEMIS_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    result = {
        ...
        # billing audits by callers like `artemis -z --usage-file`.
        "service_tier": (
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        result["error"] = final_response or (
            ...
            "health (`artemis doctor`), then send your message again"
        )
