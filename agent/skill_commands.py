...
#
# When a user invokes a /skill (or /bundle), Artemis expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_artemis_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``ARTEMIS_PLATFORM`` env var and
    ``ARTEMIS_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("ARTEMIS_PLATFORM")
            or get_session_env("ARTEMIS_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("ARTEMIS_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.artemis.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_artemis_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.artemis/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core Artemis slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Artemis command; skipping "
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.artemis/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``artemis -s <skill>`` or
    a deployment's ``ARTEMIS_TUI_SKILLS`` env var could force-load a skill an
