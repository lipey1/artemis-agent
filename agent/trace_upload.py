"""Upload a Artemis session transcript to Hugging Face as an agent trace.

Artemis stores sessions in its own SQLite store (``artemis_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``artemis trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Artemis' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "artemis-traces"
_ARTEMIS_VERSION = "artemis-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Artemis OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Artemis' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Artemis conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.artemis/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `artemis trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/artemis-traces`` dataset. Returns a
