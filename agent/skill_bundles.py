...
Storage
-------
Bundles live in ``~/.artemis/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ARTEMIS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``artemis bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ARTEMIS_HOME.

    Honors ``ARTEMIS_BUNDLES_DIR`` for tests; falls back to
    ``<ARTEMIS_HOME>/skill-bundles``.
    """
    override = os.environ.get("ARTEMIS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_artemis_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `artemis bundles` CLI subcommand.
# ---------------------------------------------------------------------------
