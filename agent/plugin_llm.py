...
Plugins built on Artemis Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Artemis
...
:class:`~artemis_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Artemis' main config
...
handles every provider, fallback chain, and per-task override Artemis
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`artemis_cli.plugins.PluginContext`
