"""Profile-scoped NeMo Relay runtimes owned by the Artemis agent core."""
...
SESSION_SCOPE = "artemis.session"
TURN_SCOPE = "artemis.turn"
LOGICAL_LLM_SCOPE = "artemis.logical_llm_call"
RUNTIME_SCHEMA_KEY = "artemis.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "artemis.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "artemis.relay.runtime_instance"
_PROFILE_KEY_CACHE: dict[str, str] = {}


@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Artemis session."""
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Artemis-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Artemis Relay session without creating one."""
        with self._sessions_lock:
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Artemis session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Artemis Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Artemis Relay session context is unavailable")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Artemis session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before Artemis authorizes a tool call."""
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Artemis Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Artemis Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Artemis profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Artemis Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one Artemis turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "artemis_relay_turn", default=None
)

# Depth of managed Relay callbacks executing on the current logical call path.
# Set >0 while the native Relay pipeline is mid-dispatch of a Artemis callback
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "artemis_relay_managed_callback_depth", default=0
)
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Artemis core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Artemis Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"artemis.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "Artemis Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Artemis Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Artemis turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Artemis turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
            ...
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "artemis.execution_surface": lease.platform or "unknown",
                    },
                )
            except Exception:
                logger.warning("Artemis Relay turn initialization failed", exc_info=True)
                ...
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        try:
                            ...
                            logger.warning(
                                "Artemis Relay turn finalization failed", exc_info=True
                            )
            finally:
                try:
                    ...
                    logger.warning(
                        "Artemis Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            try:
                ...
                logger.warning(
                    "Artemis Relay logical LLM finalization failed",
                    exc_info=True,
                )
                break
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a Artemis tool). Nested managed execution here is structurally
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Artemis session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Artemis Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at Artemis's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Artemis core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Artemis Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Artemis session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Artemis Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Artemis Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Artemis session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Artemis Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active Artemis profile."""
