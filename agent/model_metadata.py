...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Artemis also honours ARTEMIS_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ARTEMIS_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_artemis_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from artemis_constants import get_artemis_home
    return get_artemis_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Artemis Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Artemis from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Artemis' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Artemis minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from artemis_constants import get_artemis_home
    return get_artemis_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Artemis grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from artemis_cli.models import get_copilot_model_context
    ...
    # ``Artemis-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Artemis sends to providers:
