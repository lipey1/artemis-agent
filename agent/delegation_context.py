...
The parent Artemis process may itself be a Kanban dispatcher worker with
ARTEMIS_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's ARTEMIS_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "artemis_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "ARTEMIS_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "ARTEMIS_KANBAN_TASK",
    "ARTEMIS_KANBAN_RUN_ID",
    "ARTEMIS_KANBAN_WORKSPACE",
    "ARTEMIS_KANBAN_WORKSPACES_ROOT",
    "ARTEMIS_KANBAN_CLAIM_LOCK",
    "ARTEMIS_KANBAN_BOARD",
    "ARTEMIS_KANBAN_DB",
)
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``ARTEMIS_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$ARTEMIS_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``ARTEMIS_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``ARTEMIS_KANBAN_*`` vars while losing the ContextVar in
