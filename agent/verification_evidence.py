...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("artemis-verify-", "artemis-ad-hoc-")
...
def _db_path() -> Path:
    return get_artemis_home() / "verification_evidence.db"
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "artemis verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``artemis verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``artemis verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="artemis verify",
        kind="verify",
