...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Artemis starts and finds ``~/.openclaw/``.

    Points users at ``artemis claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``artemis claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Artemis, run "
        "`artemis claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `artemis claw cleanup` (renames it to ~/.openclaw.pre-migration — "
