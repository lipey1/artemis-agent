...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Artemis with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.artemis/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Artemis
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``artemis tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``artemis setup`` for
