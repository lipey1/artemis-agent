...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ARTEMIS_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Artemis runtime / deployment
    "ARTEMIS_HOME", "ARTEMIS_PROFILE", "ARTEMIS_GATEWAY_LOCK_DIR",
    "ARTEMIS_MAX_ITERATIONS", "ARTEMIS_MAX_TOKENS", "ARTEMIS_API_TIMEOUT",
    "ARTEMIS_REDACT_SECRETS", "ARTEMIS_NOUS_TIMEOUT_SECONDS",
    "_ARTEMIS_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ARTEMIS_KANBAN_DB", "ARTEMIS_KANBAN_WORKSPACES_ROOT", "ARTEMIS_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ARTEMIS_KANBAN_",
    "ARTEMIS_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Artemis writes
    ...
    ``artemis_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Artemis parser: save_env_value
    ...
    from artemis_cli.config import _parse_env_value
...
def build_profile_secret_scope(artemis_home: Path) -> Dict[str, str]:
    ...
    try:
        from artemis_cli.env_loader import get_secret_source_values
