...
as ``kind: backend``) or ``~/.artemis/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``artemis tools``
...
def _images_cache_dir() -> Path:
    """Return ``$ARTEMIS_HOME/cache/images/``, creating parents as needed."""
    from artemis_constants import get_artemis_home

    path = get_artemis_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ARTEMIS_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$ARTEMIS_HOME/cache/images/``.
