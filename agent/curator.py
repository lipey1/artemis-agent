...
def _state_file() -> Path:
    return get_artemis_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.artemis/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `artemis update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``artemis curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``artemis update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``artemis curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `artemis curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Artemis' background skill CURATOR. This is an "
    ...
    "into ~/.artemis/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: artemis-config-*, artemis-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.artemis/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.artemis/logs/curator/``)
    ...
    authored skill data in ``~/.artemis/skills/``.

    ``ensure_artemis_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_artemis_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every artemis install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: artemis curator status
        keep an umbrella stable: artemis curator pin document-tools
    ...
    lines.append("full report: artemis curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: artemis curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.artemis/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.artemis/skills/.archive/` for "
            "safety and can be restored via `artemis curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.artemis/skills/.archive/`. "
            "Restore any via `artemis curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `artemis curator restore <name>`")
    lines.append("- All archives live under `~/.artemis/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``artemis curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `artemis curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `artemis update` only if "
        ...
        # recorded in state so `artemis curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``artemis model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `artemis model` → auxiliary
