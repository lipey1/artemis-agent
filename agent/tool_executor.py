...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Artemis process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("ARTEMIS_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid ARTEMIS_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Artemis process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Artemis policy and dispatch exactly once."""
    from agent import relay_tools
    from artemis_cli.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "Artemis tool execution callback invoked more than once"
                )
    ...
    def _artemis_pipeline(relay_args: dict[str, Any]) -> Any:
