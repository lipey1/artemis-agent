...
``auth.json``.  They intentionally have no dependency on ``artemis_cli.auth`` so
...
# Sources Artemis owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Artemis-owned OAuth/device-code state)
