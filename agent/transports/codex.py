...
# Wire-name used when Artemis keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Artemis's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "artemis_web_search"
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if (params.get("provider") or "").strip().lower() == "actual":
            ...
            # line 1 column 1"). Clamp Artemis' wider set to the supported one.
            _effort_clamp.update({"xhigh": "high", "ultra": "max"})
        reasoning_effort = _effort_clamp.get(reasoning_effort, reasoning_effort)

        response_tools = _responses_tools(tools)

        # xAI server-side web search vs Artemis web providers.
        #
        ...
        #    keep Artemis dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``artemis_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Artemis dispatches
