...
Owns one Codex thread per Artemis session. Drives `turn/start`, consumes
...
# Artemis' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ARTEMIS_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Artemis/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Artemis session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ARTEMIS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ARTEMIS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="artemis",
            client_title="Artemis Agent",
            client_version=_get_artemis_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Artemis' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Artemis' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own artemis-tools callback we
            # auto-accept — the user already approved Artemis' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Artemis
        approval-mode/timeout logic. The Artemis-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Artemis approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Artemis approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Artemis returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Artemis approval mode/timeout resolution
    ...
    # "prompt expired"; the Artemis-side messaging already distinguishes them.
    return "decline"
...
def _get_artemis_version() -> str:
    """Best-effort Artemis version string for codex's userAgent line."""
    try:
