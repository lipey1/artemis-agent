...
projection into Artemis' display, approval bridging, transcript projection into
...
"codex_app_server"`. Artemis' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Artemis secret — gateway
        ...
        spawn_env = artemis_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ARTEMIS_KANBAN_TASK"):
            kanban_db = spawn_env.get("ARTEMIS_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ARTEMIS_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ARTEMIS_HOME", os.path.expanduser("~/.artemis")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "artemis",
        client_title: str = "Artemis Agent",
