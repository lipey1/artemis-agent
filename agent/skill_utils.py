...
from artemis_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.artemis/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    ARTEMIS_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ARTEMIS_KANBAN_TASK`` / ``ARTEMIS_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ARTEMIS_KANBAN_TASK") or os.getenv("ARTEMIS_KANBAN_BOARD"):
        ...
        # The Artemis Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``artemis_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ARTEMIS_PLATFORM`` or
            ``ARTEMIS_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("ARTEMIS_PLATFORM")
        or get_session_env("ARTEMIS_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``artemis`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.artemis/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``artemis`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ARTEMIS_HOME, not cwd
        if not p.is_absolute():
            p = (artemis_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.artemis/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.artemis/skills/`` (including via symlinks) or configured
    ...
    # ~/.artemis/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Artemis can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    artemis = metadata.get("artemis") or {}
    if not isinstance(artemis, dict):
        artemis = {}
    return {
        "fallback_for_toolsets": artemis.get("fallback_for_toolsets", []),
        "requires_toolsets": artemis.get("requires_toolsets", []),
        "fallback_for_tools": artemis.get("fallback_for_tools", []),
        "requires_tools": artemis.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    artemis = metadata.get("artemis")
    if not isinstance(artemis, dict):
        return []
    raw = artemis.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Artemis metadata, VCS, virtualenv/dependency, cache, and skill
