...
Every Artemis request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Artemis Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=artemis-agent",
        "client=artemis-client-v<__version__>",
    ]

The version is sourced live from ``artemis_cli.__version__`` so it auto-aligns
...
``artemis_cli.__version__`` is the canonical source of truth.
"""
...
def _artemis_version() -> str:
    """Return the current Artemis release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``artemis_cli`` cannot be imported (should
...
def artemis_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=artemis-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=artemis-client-v{_artemis_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Artemis session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Artemis conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.artemis/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Artemis
    ...
    """
    tags = ["product=artemis-agent", artemis_client_tag()]
