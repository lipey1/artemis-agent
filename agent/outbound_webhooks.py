...
(``gateway/platforms/webhook.py``): inbound wakes Artemis when the world
changes; outbound tells the world when Artemis does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Artemis-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``ARTEMIS_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.artemis/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/artemis-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: ARTEMIS_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `artemis hooks list`
...
Headers::
    ...
    User-Agent:              Artemis-Agent-Outbound-Webhook
    X-Artemis-Event:          <hook event name>
    X-Artemis-Delivery:       <delivery_id>
    X-Artemis-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("ARTEMIS_SAFE_MODE"):
        logger.info("ARTEMIS_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Artemis-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Artemis-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
