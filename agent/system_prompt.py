...
upstream prefix cache warm.  See ``artemis-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ARTEMIS_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ARTEMIS_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Artemis never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ARTEMIS_HOME persona while keeping
    ...
    # Pointer to the artemis-agent skill + docs for user questions about Artemis itself.
    stable_parts.append(ARTEMIS_AGENT_HELP_GUIDANCE)
    ...
    # ARTEMIS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Artemis, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Artemis profile the agent is running
    # under so it doesn't conflate ~/.artemis/skills/ (default profile) with
    # ~/.artemis/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Artemis profile: default. Other profiles (if any) live "
            "under " + str(get_artemis_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active Artemis profile: {active_profile}. This session reads "
            f"and writes {get_artemis_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_artemis_home()}/skills/, {get_artemis_home()}/plugins/, "
            f"{get_artemis_home()}/cron/, {get_artemis_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Artemis). Every other surface (desktop chat panel,
