...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ARTEMIS_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Artemis expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``artemis_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_artemis_home()`` themselves.
        """
        if "artemis_home" not in kwargs:
            from artemis_constants import get_artemis_home
            kwargs["artemis_home"] = str(get_artemis_home())
        for provider in self._providers:
            try:
