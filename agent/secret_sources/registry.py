...
``artemis_cli.env_loader._apply_external_secret_sources()``.
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``artemis`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's ARTEMIS_HOME is ``~/.artemis/profiles/<name>``; the
    default profile (``~/.artemis``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("ARTEMIS_PROFILE_NAME", "ARTEMIS_PROFILE"):
