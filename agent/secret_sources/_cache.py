...
cache problem must never block Artemis startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Artemis home used for cache paths.

    ``home_path`` is whatever ``load_artemis_dotenv()`` already resolved;
    falling back to ``$ARTEMIS_HOME`` / ``~/.artemis`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from artemis_constants import get_artemis_home

        home_path = get_artemis_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<artemis_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ARTEMIS_HOME is ever made traversable.
            try:
