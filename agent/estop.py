...
``artemis pause`` writes a sentinel file at ``$ARTEMIS_HOME/ESTOP``;
``artemis resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Artemis is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.artemis/ESTOP``.
...
def _artemis_home() -> Path:
    """Resolve the active ARTEMIS_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active ARTEMIS_HOME."""
    return _artemis_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on ARTEMIS_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Artemis is paused ({reason}). New work is on hold; "
            "run `artemis resume` to pick things back up."
        )
    return (
        "⏸️ Artemis is paused. New work is on hold; "
        "run `artemis resume` to pick things back up."
    )
