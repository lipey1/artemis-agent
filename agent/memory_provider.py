...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `artemis backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - artemis_home (str): The active ARTEMIS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.artemis``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "artemis").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'artemis memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], artemis_home: str) -> None:
        ...
        Called by 'artemis memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``artemis_home`` is the active ARTEMIS_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ARTEMIS_HOME.

        ``artemis backup`` only walks ARTEMIS_HOME, so any provider state kept
        ...
        of the archive, and ``artemis import`` restores them to their original
