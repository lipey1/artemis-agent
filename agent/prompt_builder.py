...
from artemis_constants import get_artemis_home, get_skills_dir, is_wsl
...
_ARTEMIS_MD_NAMES = (".artemis.md", "ARTEMIS.md")


def _find_artemis_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.artemis.md`` or ``ARTEMIS.md``.
    ...
    # could pick up a .artemis.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ARTEMIS_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Artemis, an intelligent AI assistant. "
    "You are helpful, knowledgeable, and direct. You assist users with a wide "
    "range of tasks including answering questions, writing and editing code, "
    "analyzing information, creative work, and executing actions via your tools. "
    "When asked who you are, introduce yourself as Artemis. Your name and identity are Artemis. "
    "Communicate clearly, admit uncertainty when appropriate, and prioritize "
    "being genuinely useful over being verbose unless otherwise directed below. "
    "Be targeted and efficient in your exploration and investigations."
)

ARTEMIS_AGENT_HELP_GUIDANCE = (
    "You run on Artemis. When the user needs help with "
    "Artemis itself - configuring, setting up, using, extending, or troubleshooting "
    "it - or when you need to understand your own features, tools, or capabilities, "
    "load the `artemis-agent` skill with skill_view(name='artemis-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.artemis/kanban.db`. Your task id is in "
    "`$ARTEMIS_KANBAN_TASK`; your workspace is `$ARTEMIS_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ARTEMIS_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ARTEMIS_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ARTEMIS_KANBAN_BRANCH:-wt/$ARTEMIS_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ARTEMIS_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`artemis profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `artemis kanban <verb>` for board operations. Use "
...
#
# The artemis-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to artemis-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Artemis has none; explicit Artemis YOLO uses a private unrestricted "
        ...
        "run `artemis computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Artemis "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Artemis floating over another application, so the
    user is rarely asking about Artemis — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Artemis "
        ...
        "rather than anything inside Artemis. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Artemis terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Artemis desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Artemis WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Artemis itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Artemis runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Artemis itself is running. The host OS, home, and cwd "
                f"of the Artemis process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Artemis "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Artemis
    ...
    extra = (os.getenv("ARTEMIS_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_artemis_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ARTEMIS_PLATFORM") or os.environ.get("ARTEMIS_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.artemis/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Artemis Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `artemis-agent` skill "
            "first. It has the actual commands (e.g. `artemis config set …`, `artemis tools`, "
            "`artemis setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: artemis setup, artemis setup tools, artemis setup terminal, artemis status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ARTEMIS_HOME and return its content, or None.
    ...
    """
    try:
        from artemis_cli.config import ensure_artemis_home
        ensure_artemis_home()
    except Exception as e:
        logger.debug("Could not ensure ARTEMIS_HOME before loading SOUL.md: %s", e)

    soul_path = get_artemis_home() / "SOUL.md"
...
def _load_artemis_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.artemis.md / ARTEMIS.md — walk to git root."""
    artemis_md_path = _find_artemis_md(cwd_path)
    if not artemis_md_path:
        return ""
    try:
        content = artemis_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = artemis_md_path.name
        try:
            rel = str(artemis_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".artemis.md", context_length=context_length,
            read_path=str(artemis_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", artemis_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .artemis.md / ARTEMIS.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ARTEMIS_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Artemis install/source
    ...
    # explicitly configured cwd is honored verbatim — the Artemis tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Artemis in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Artemis install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_artemis_md(cwd_path, context_length)
    ...
    # SOUL.md from ARTEMIS_HOME only — skip when already loaded as identity
    if not skip_soul:
