"""Anthropic Messages API adapter for Artemis Agent.

Translates between Artemis's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in artemis_cli/runtime_provider.py.
    """
...
# Artemis effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to artemis's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to artemis's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + ArtemisAgent User-Agent.
    ...
    # from ``ANTHROPIC_API_KEY`` (Artemis loads that into the process env from
    # ``~/.artemis/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Artemis notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Artemis historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``artemis models``) never mutates ``~/.artemis/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Artemis)
      ...
      5. Anthropic credential_pool OAuth entry (~/.artemis/auth.json)
    ...
    # 1. Artemis-managed OAuth/setup token env var
    ...
    # 5. Artemis credential_pool OAuth entry.
...
# ── Artemis-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.artemis/.anthropic_oauth.json (our own file).
...
def _get_artemis_oauth_file() -> Path:
    return get_artemis_home() / ".anthropic_oauth.json"
...
def run_artemis_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Artemis-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Artemis with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from artemis_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_artemis_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Artemis-managed OAuth credentials from ~/.artemis/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Artemis OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See artemis-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See artemis-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See artemis-agent#13848 (Kimi) and
    artemis-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Artemis Agent", "Claude Code")
                text = text.replace("Artemis agent", "Claude Code")
                text = text.replace("artemis-agent", "claude-code")
        ...
        #      a) bare Artemis-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Artemis surfaces in its CLI. We
