...
Data resolution order:
  ...
  2. Disk cache (~/.artemis/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Artemis ↔ models.dev
# ---------------------------------------------------------------------------

# Artemis provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Artemis (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_artemis_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``artemis config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Artemis provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from artemis_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Artemis agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Artemis provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Artemis ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Artemis or models.dev provider ID.  Tries exact match then
