...
Extracted verbatim from artemis_state.py so the SessionDB mixin modules
(artemis_state_search / artemis_state_schema / artemis_state_portability) can
reference them without importing artemis_state (which would be a cycle).
artemis_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``artemis sessions optimize-storage``. A legacy DB sits at
...
# `artemis sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `artemis db optimize`. This is
