...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$ArtemisHome = $(if ($env:ARTEMIS_HOME) { $env:ARTEMIS_HOME } else { "$env:LOCALAPPDATA\artemis" }),
    [string]$InstallDir = $(if ($env:ARTEMIS_HOME) { "$env:ARTEMIS_HOME\artemis-agent" } else { "$env:LOCALAPPDATA\artemis\artemis-agent" }),
    ...
    # builds apps/desktop into a launchable Artemis.exe.
    #
    # Why opt-in:
    #   * Artemis-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Artemis.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Artemis.exe
    ...
    #     `artemis desktop` already builds on demand.
...
# ARTEMIS_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[artemis] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'ArtemisInstall.LongPath').Type) {
            Add-Type -Namespace 'ArtemisInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [ArtemisInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -ArtemisHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('ArtemisHome')) {
    $ArtemisHome = ConvertTo-LongPath $ArtemisHome
} else {
    $ArtemisHome = ConvertTo-LongPath $(
        if ($env:ARTEMIS_HOME) { $env:ARTEMIS_HOME } else { "$env:LOCALAPPDATA\artemis" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ARTEMIS_HOME) { "$env:ARTEMIS_HOME\artemis-agent" } else { "$env:LOCALAPPDATA\artemis\artemis-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: ArtemisHome=$ArtemisHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * Artemis Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ArtemisHome)) {
        New-Item -ItemType Directory -Force -Path $ArtemisHome | Out-Null
    }
    $envFile = Join-Path $ArtemisHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ArtemisHome "node"
...
function Install-Uv {
    # Artemis owns its own uv at $ArtemisHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (artemis_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `artemis update` stay in sync.
    $managedUv = Join-Path $ArtemisHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ArtemisHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ArtemisHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ArtemisHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ArtemisHome "bin"
...
# Put the Artemis-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone artemis-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Artemis-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    $tmpCwd = Join-Path $env:TEMP ("artemis-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ARTEMIS_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Artemis-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Artemis setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Artemis uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\artemis\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Artemis needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Artemis copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\artemis\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ARTEMIS_GIT_BASH_PATH`` (User scope) so Artemis can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Artemis-managed PortableGit install instead..."
    }

    # Download PortableGit into $ArtemisHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ArtemisHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Artemis features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Artemis needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ARTEMIS_GIT_BASH_PATH`` (User env scope) so Artemis can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ArtemisHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ArtemisHome\git\usr\bin\bash.exe
    $candidates += "$ArtemisHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ArtemisHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ARTEMIS_GIT_BASH_PATH", $candidate, "User")
            $env:ARTEMIS_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ARTEMIS_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Artemis may not find Git Bash."
    Write-Info "If needed, set ARTEMIS_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Artemis requires Node >=26)"
    }

    # Prefer a Artemis-managed Node from a previous run over a too-old system one.
    $managedNode = "$ArtemisHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ArtemisHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$ArtemisHome\node"
        Write-Success "Node.js $version found (Artemis-managed)"
        ...
        Update-ManagedNpm "$ArtemisHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Artemis-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ArtemisHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ArtemisHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\artemis-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ArtemisHome\node") { Remove-Item -Recurse -Force "$ArtemisHome\node" }
                Move-Item $extractedDir.FullName "$ArtemisHome\node"
                ...
                $version = & "$ArtemisHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ArtemisHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\artemis-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `artemis
                    # update` path (#4735).
                    ...
                    $stashName = "artemis-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. artemis-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``artemis update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Artemis behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running artemis processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\artemis-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `artemis update` (see the notes at the shared clone-path
    ...
    # next `artemis update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Artemis-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running artemis process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running artemis processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Artemis_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (artemis.exe) plus its child tree.
            & taskkill /F /T /IM artemis.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM artemis.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m artemis_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not artemis.exe.
            ...
            # Artemis children re-exec through .artemis-runtime, so killing only
            ...
            # user (see artemis_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `artemis update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `artemis.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'artemis-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install artemis-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and artemis.exe broken with
    ...
    # here, not 30 seconds later when the user runs `artemis`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register artemis.exe in dist-info/RECORD but fail to
        ...
        # `artemis` command while artemis-agent.exe / artemis-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m artemis_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `artemis dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\artemis_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `artemis dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `artemis dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: artemis_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up artemis command..."
    
    if ($NoVenv) {
        $artemisBin = "$InstallDir"
    } else {
        # Expose ONLY the artemis launchers on PATH -- never the whole
        ...
        # (#83797): unrelated projects start resolving python to Artemis'
        # runtime interpreter. A dedicated bin dir with copies of the
        # launcher exes keeps `artemis` globally available without
        # shadowing anything. (Launcher exes embed the venv interpreter
        # path, so they work from any location and survive updates.)
        $artemisBin = "$InstallDir\bin"
        New-Item -ItemType Directory -Force -Path $artemisBin | Out-Null
        foreach ($launcher in @("artemis.exe", "artemis-acp.exe")) {
            $src = "$InstallDir\venv\Scripts\$launcher"
            if (Test-Path $src) {
                Copy-Item -Force $src "$artemisBin\$launcher"
            }
        }
    }
    ...
    if ((-not $NoVenv) -and $currentPath -like "*$legacyBin*") {
        ...
        $currentPath = $cleaned
        Write-Info "Removed legacy venv\Scripts from user PATH (kept artemis via $artemisBin)"
    }
    
    if ($currentPath -notlike "*$artemisBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$artemisBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $artemisBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ARTEMIS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\artemis instead
    # of the Unix default ~/.artemis
    $currentArtemisHome = [Environment]::GetEnvironmentVariable("ARTEMIS_HOME", "User")
    if (-not $currentArtemisHome -or $currentArtemisHome -ne $ArtemisHome) {
        [Environment]::SetEnvironmentVariable("ARTEMIS_HOME", $ArtemisHome, "User")
        Write-Success "Set ARTEMIS_HOME=$ArtemisHome"
    }
    $env:ARTEMIS_HOME = $ArtemisHome
    
    # Update current session
    $env:Path = "$artemisBin;$env:Path"
    
    Write-Success "artemis command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.artemis-bootstrap-complete which tells the Artemis
    ...
    # Artemis-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `artemis update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".artemis-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ARTEMIS_HOME directory structure ($ArtemisHome, default %LOCALAPPDATA%\artemis)
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArtemisHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ArtemisHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ArtemisHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Artemis's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ArtemisHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in artemis_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ArtemisHome"
    
    # Seed bundled skills into $ArtemisHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ArtemisHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $ArtemisHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ArtemisHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ArtemisHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Artemis-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'artemis setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\artemis-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\artemis-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\artemis-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `artemis tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $ArtemisHome "bin"
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Artemis' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'artemis tools')"
        }
    } catch {
...
# the final `electron` -> `Artemis` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Artemis.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_artemis_desktop command
    # resolves apps/desktop/release/win-unpacked/Artemis.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Artemis-managed
    ...
    # The --dir mode produces an unpacked Artemis.exe in
    ...
    # are NOT trying to work around). The Artemis icon + product name are
    # stamped onto Artemis.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\artemis-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Artemis.exe",
        "$desktopDir\release\win-arm64-unpacked\Artemis.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Artemis.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Artemis icon + identity are stamped onto Artemis.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, artemis-agent#38216).
    ...
    #    Artemis.exe. We deliberately do NOT point them at `artemis desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Artemis.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Artemis.lnk')
        )
        ...
        # with the Artemis icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.artemis/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'artemis setup'."
        return
    }
    ...
    # Run artemis setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m artemis_cli.main setup
    } else {
        python -m artemis_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $artemisCmd = "$InstallDir\venv\Scripts\artemis.exe"
    if (-not (Test-Path $artemisCmd)) {
        $artemisCmd = "artemis"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ArtemisHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'artemis whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: artemis gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ArtemisHome\logs\gateway.log"
            Start-Process -FilePath $artemisCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ArtemisHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: artemis gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: artemis gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ArtemisHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArtemisHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArtemisHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArtemisHome\artemis-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   artemis              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   artemis setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   artemis config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   artemis config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   artemis gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   artemis update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Artemis repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Artemis-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Artemis to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
