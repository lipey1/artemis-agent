...
# Python-driven tool session (e.g. Artemis terminal tool). A pre-set PYTHONPATH
...
#   code at /usr/local/lib/artemis-agent, command at /usr/local/bin/artemis,
#   data still at /root/.artemis (ARTEMIS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ARTEMIS_HOME/.no-bundled-skills so future"
            echo "                   'artemis update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Artemis.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.artemis/artemis-agent"
            echo "                   default (root, Linux): /usr/local/lib/artemis-agent"
            echo "  --artemis-home PATH  Data directory (default: ~/.artemis, or \$ARTEMIS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Artemis installs the code under"
            echo "  /usr/local/lib/artemis-agent and links the command into"
            echo "  /usr/local/bin/artemis (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ARTEMIS_HOME"
            echo "  (default /root/.artemis).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ARTEMIS_HOME/artemis-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `artemis update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Artemis-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Artemis","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install artemis command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `artemis` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ARTEMIS_HOME/artemis-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ARTEMIS_HOME/artemis-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/artemis-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ARTEMIS_HOME/artemis-agent — then preserve it)
#
# Always no-op when the user set --dir or $ARTEMIS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ARTEMIS_HOME.
    if is_termux; then
        INSTALL_DIR="$ARTEMIS_HOME/artemis-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ARTEMIS_HOME/artemis-agent/.git" ]; then
            INSTALL_DIR="$ARTEMIS_HOME/artemis-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/artemis-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/artemis-agent"
        ...
        # /usr/local/bin/artemis wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/artemis"
        log_info "  Data:    $ARTEMIS_HOME (unchanged)"
...
# Point a Artemis-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ARTEMIS_HOME/node/bin — which is NOT on
...
# Artemis's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Artemis-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ARTEMIS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ARTEMIS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ARTEMIS_HOME/node/etc/npmrc"
}

get_artemis_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/artemis" ]; then
        echo "$link_dir/artemis"
    else
        echo "artemis"
    fi
}
...
install_uv() {
    ...
    # Artemis owns its own uv at $ARTEMIS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (artemis_cli/managed_uv.py) looks in the same
    # place, so install.sh and `artemis update` stay in sync.
    local _managed_uv="$ARTEMIS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ARTEMIS_HOME/bin ..."
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/artemis-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/artemis-uv-installer.$$.sh")"
    ...
    # directly into $ARTEMIS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ARTEMIS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Artemis-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Artemis-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing Artemis-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Artemis-managed Node from a previous run over a too-old system one.
    if [ -x "$ARTEMIS_HOME/node/bin/node" ] && node_satisfies_build "$("$ARTEMIS_HOME/node/bin/node" --version)"; then
        export PATH="$ARTEMIS_HOME/node/bin:$PATH"
        log_success "Node.js $("$ARTEMIS_HOME/node/bin/node" --version) found (Artemis-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Artemis requires Node >=26) — installing Artemis-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.artemis/node/..."
    ...
    # Place into ~/.artemis/node/ and symlink binaries into the same bin dir
    # the artemis command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ARTEMIS_HOME/node"
    mkdir -p "$ARTEMIS_HOME"
    mv "$extracted_dir" "$ARTEMIS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ARTEMIS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ARTEMIS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ARTEMIS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ARTEMIS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.artemis/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Artemis install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Artemis itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `artemis update` path
                # (#4735).
                ...
                local stash_name
                stash_name="artemis-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``artemis update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Artemis behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up artemis command..."

    if [ "$USE_VENV" = true ]; then
        ARTEMIS_BIN="$INSTALL_DIR/venv/bin/python"
        ARTEMIS_ENTRYPOINT="$INSTALL_DIR/artemis"
    else
        ARTEMIS_BIN="$(which artemis 2>/dev/null || echo "")"
        if [ -z "$ARTEMIS_BIN" ]; then
            log_warn "artemis not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$ARTEMIS_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ARTEMIS_ENTRYPOINT" ]; }; then
        log_warn "Artemis launcher prerequisites not found"
    ...
    # Create a user-facing shim for the artemis command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ARTEMIS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ARTEMIS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/artemis"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/artemis" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARTEMIS_BIN" "$ARTEMIS_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/artemis" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/artemis"
    log_success "Installed artemis launcher → $command_link_display_dir/artemis"

    # Also expose `artemis-agent`. The `artemis-agent` console script declared in
    ...
    rm -f "$command_link_dir/artemis-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/artemis-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARTEMIS_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/artemis-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARTEMIS_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/artemis-agent"
    log_success "Installed artemis-agent launcher → $command_link_display_dir/artemis-agent"

    # Also expose `artemis-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `artemis-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Artemis as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/artemis-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/artemis-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARTEMIS_BIN" "$ARTEMIS_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/artemis-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARTEMIS_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/artemis-acp"
    log_success "Installed artemis-acp launcher → $command_link_display_dir/artemis-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "artemis command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v artemis' \
                >/dev/null 2>&1; then
        ...
        log_info "artemis not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Artemis — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Artemis — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Artemis — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so artemis works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.artemis directory structure (config at top level, code in subdir)
    mkdir -p "$ARTEMIS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.artemis/.env (top level, easy to find)
    if [ ! -f "$ARTEMIS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ARTEMIS_HOME/.env"
            log_success "Created ~/.artemis/.env from template"
        else
            touch "$ARTEMIS_HOME/.env"
            log_success "Created ~/.artemis/.env"
        fi
    else
        log_info "~/.artemis/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ARTEMIS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.artemis/config.yaml (top level, easy to find)
    if [ ! -f "$ARTEMIS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ARTEMIS_HOME/config.yaml"
            log_success "Created ~/.artemis/config.yaml from template"
        fi
    else
        log_info "~/.artemis/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in artemis_cli/default_soul.py — the
    ...
    if [ ! -f "$ARTEMIS_HOME/SOUL.md" ]; then
        cat > "$ARTEMIS_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.artemis/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.artemis/"

    # Seed bundled skills into ~/.artemis/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `artemis update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'artemis update'." \
            > "$ARTEMIS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ARTEMIS_HOME/.no-bundled-skills"
        log_info "  Future 'artemis update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.artemis/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.artemis/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ARTEMIS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ARTEMIS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.artemis/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$ARTEMIS_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Artemis browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Artemis will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Artemis browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (artemis --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `artemis update` doesn't autostash every run.
...
install_browser_use_cli() {
    ...
    # `artemis tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if command -v browser-use >/dev/null 2>&1 || [ -x "$ARTEMIS_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Artemis' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$ARTEMIS_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'artemis tools')"
    fi
}
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'artemis setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m artemis_cli.main setup < /dev/tty
    else
        python -m artemis_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ARTEMIS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Artemis to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ARTEMIS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'artemis whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ARTEMIS_CMD="$(get_artemis_command_path)"
                $ARTEMIS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'artemis whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'artemis gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ARTEMIS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ARTEMIS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: artemis gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: artemis gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ARTEMIS_CMD gateway > "$ARTEMIS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.artemis/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: artemis gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: artemis gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.artemis-bootstrap-complete, which tells the Artemis
    ...
    local marker_path="$INSTALL_DIR/.artemis-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ARTEMIS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ARTEMIS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ARTEMIS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}artemis${NC}              Start chatting"
    echo -e "   ${GREEN}artemis setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}artemis config${NC}       View/edit configuration"
    echo -e "   ${GREEN}artemis config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}artemis gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}artemis update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'artemis' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'artemis' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'artemis' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ARTEMIS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ARTEMIS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ARTEMIS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ARTEMIS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ARTEMIS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Artemis` rename
...
# _purge_electron_build_cache() used by `artemis desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Artemis-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `artemis desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Artemis" ]; then
            app="$desktop_dir/release/linux-unpacked/Artemis"
        elif [ -x "$desktop_dir/release/linux-unpacked/artemis" ]; then
            app="$desktop_dir/release/linux-unpacked/artemis"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Artemis.app" \
            "$desktop_dir/release/mac/Artemis.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `artemis desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ARTEMIS_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ARTEMIS_HOME="$ARTEMIS_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from artemis_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `artemis update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Artemis-managed Node
            # provisioned during prerequisites/node-deps (at $ARTEMIS_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ARTEMIS_HOME.
    # $ARTEMIS_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'artemis update' on this host install.
