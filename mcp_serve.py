"""
Artemis MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Artemis-specific extra)

Usage:
    artemis mcp serve
    artemis mcp serve --verbose
...
logger = logging.getLogger("artemis.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ARTEMIS_HOME."""
    try:
        from artemis_constants import get_artemis_home
        return get_artemis_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ARTEMIS_HOME", Path.home() / ".artemis")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from artemis_constants import get_artemis_home
        directory_file = get_artemis_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ARTEMIS_HOME", Path.home() / ".artemis")
...
class EventBridge:
    ...
    This is the Artemis equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from artemis_constants import get_artemis_home
            db_file = get_artemis_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ARTEMIS_HOME", Path.home() / ".artemis")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Artemis MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "artemis",
        instructions=(
            "Artemis Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Artemis MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
