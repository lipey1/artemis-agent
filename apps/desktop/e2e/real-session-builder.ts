 ...
 * AIAgent loop, using the E2E mock provider configured in `artemisHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(artemisHome: string) {
  ...
  static async start(artemisHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(artemisHome)
