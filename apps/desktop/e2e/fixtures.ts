/**
 * Shared E2E fixtures for the Artemis desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → artemis serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `artemis desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `artemis-e2e-${prefix}-${Math.random()}`))
  const artemisHome = path.join(root, 'artemis-home')
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(artemisHome, 'config.yaml')
 ...
 */
export function writeEnvFile(artemisHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(artemisHome, '.env')
 ...
 */
function writeEmptyConfig(artemisHome: string): void {
  const configPath = path.join(artemisHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ARTEMIS_HOME → sandbox artemis-home (isolated config/sessions)
 *  - ARTEMIS_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ARTEMIS_DESKTOP_IGNORE_EXISTING=1 → don't pick up `artemis` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ARTEMIS_DESKTOP_ARTEMIS_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ARTEMIS_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ARTEMIS_HOME: sandbox.artemisHome,
    ARTEMIS_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ARTEMIS_DESKTOP_IGNORE_EXISTING: '1',
    ARTEMIS_DESKTOP_ARTEMIS_ROOT: REPO_ROOT,
    ARTEMIS_DESKTOP_APP_NAME: `ArtemisE2E-${Date.now()}`,
    ...
    ARTEMIS_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ARTEMIS_HOME + userData
 * @param env      - the process environment (already has ARTEMIS_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.artemisHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.artemisHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ARTEMIS_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`artemis serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.artemisHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ARTEMIS_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Artemis backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Artemis.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Artemis.app', 'Contents', 'MacOS', 'Artemis')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'artemis')
}
 ...
 * progress without spawning a real Artemis backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ARTEMIS_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ARTEMIS_DESKTOP_BOOT_FAKE: '1',
    ARTEMIS_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + artemis-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ARTEMIS_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ARTEMIS_DESKTOP_ARTEMIS
  delete (env as Record<string, string | undefined>).ARTEMIS_DESKTOP_ARTEMIS_ROOT
