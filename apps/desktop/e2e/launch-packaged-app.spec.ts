...
/**
 * E2E smoke tests for the packaged Artemis desktop app.
 *
 ...
 * (credential stripping, isolated ARTEMIS_HOME + userData, unique app name).
 *
...
test('window opens with the Artemis title', async () => {
  const title = await fixture!.page.title()
  expect(title).toContain('Artemis')
})
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      artemisDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).artemisDesktop?.hud?.open({ sessionId: null })
  )
