...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Artemis E2E'], { cwd: repo })
...
function configureRepoCwd(artemisHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(artemisHome, mockUrl)
  fs.appendFileSync(path.join(artemisHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(artemisHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.artemisHome, mock.url, repo)
