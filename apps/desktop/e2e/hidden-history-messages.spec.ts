...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.artemisHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.artemisHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.artemisHome)
  const builder = await RealSessionBuilder.start(sandbox.artemisHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.artemisHome, mock.url)
  fs.appendFileSync(path.join(sandbox.artemisHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.artemisHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
