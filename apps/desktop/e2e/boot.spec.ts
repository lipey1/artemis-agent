 ...
 * packaged binary) with a real `artemis serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → artemis serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Artemis title', async () => {
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → artemis serve is spawned → WS connects → config
