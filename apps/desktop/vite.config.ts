...
const nm = (rel: string): string =>
  real(path.resolve(__dirname, 'node_modules', rel)) ??
  real(path.resolve(__dirname, '../../node_modules', rel)) ??
...
export default defineConfig(({ command }) => ({
  ...
  css: {
    ...
    // `C:\Users\<name>\AppData\Local\artemis\artemis-agent\apps\desktop`), so an
  ...
  resolve: {
    alias: {
      ...
      '@artemis/plugin-sdk': path.resolve(__dirname, './src/sdk/index.ts'),
      '@artemis/shared/billing': path.resolve(__dirname, '../shared/src/billing-types.ts'),
      '@artemis/shared': path.resolve(__dirname, '../shared/src'),
      react: nm('react'),
