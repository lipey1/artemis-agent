...
// no longer bundles the Artemis Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Artemis.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Artemis'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Artemis.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Artemis'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ARTEMIS_HOME for non-sandboxed runs -- matches main.ts's
// resolveArtemisHome(). On Windows it's %LOCALAPPDATA%\artemis; elsewhere
// it's ~/.artemis. The fresh-install sandbox launchFresh() sets its own
// ARTEMIS_HOME and never touches this.
const DEFAULT_ARTEMIS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'artemis')
  }
  return path.join(os.homedir(), '.artemis')
})()
const VENV_ROOT = path.join(DEFAULT_ARTEMIS_HOME, 'artemis-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'artemis-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ARTEMIS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Artemis-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Artemis-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Artemis-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Artemis-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ARTEMIS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ARTEMIS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const artemisHome = path.join(sandbox, 'artemis-home')
  ...
  env.ARTEMIS_DESKTOP_CWD = cwd
  env.ARTEMIS_DESKTOP_IGNORE_EXISTING = '1'
  env.ARTEMIS_DESKTOP_TEST_MODE = 'fresh-install'
  env.ARTEMIS_DESKTOP_USER_DATA_DIR = userDataDir
  env.ARTEMIS_HOME = artemisHome
  delete env.ARTEMIS_DESKTOP_ARTEMIS
  delete env.ARTEMIS_DESKTOP_ARTEMIS_ROOT
  ...
  console.log(`  ARTEMIS_HOME: ${artemisHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(artemisHome, 'artemis-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Artemis Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of artemis_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'artemis-agent', 'artemis_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Artemis
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ARTEMIS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ARTEMIS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
