...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `artemis-dev-mock-${Date.now()}`))
  ...
  return { root, artemisHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(artemisHome, mockUrl) {
  fs.writeFileSync(
    path.join(artemisHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(artemisHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.artemisHome, mock.url)
  console.log(`  ARTEMIS_HOME: ${sandbox.artemisHome}`)
  ...
  const env = {
    ...
    ARTEMIS_DESKTOP_APP_NAME: `ArtemisDevMock-${Date.now()}`,
  }
