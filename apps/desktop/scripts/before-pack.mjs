 ...
 * product name (`Artemis`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Artemis'
 *
 * This is a hard failure with no obvious cause for the user — `artemis desktop`
 ...
 * (the mac-unpacked Artemis.app bundle) and Windows (win-unpacked), so we
 ...
 * Artemis.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `artemis desktop --build-only` (artemis_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Artemis.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Artemis'}.exe`
