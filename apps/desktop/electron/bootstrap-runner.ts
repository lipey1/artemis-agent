 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Artemis Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.artemis-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(artemisHome) {
  return path.join(artemisHome, 'bootstrap-cache')
}
...
function cachedScriptPath(artemisHome, commit) {
  return path.join(bootstrapCacheDir(artemisHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const scriptName = installScriptName()
  const url = `https://raw.githubusercontent.com/NousResearch/artemis-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(artemisHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(artemisHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, artemisHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ARTEMIS_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ARTEMIS_HOME: artemisHome || process.env.ARTEMIS_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, artemisHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, artemisHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--artemis-home', artemisHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, artemisHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, artemisHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, artemisHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(artemisHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, artemisHome, emit })
