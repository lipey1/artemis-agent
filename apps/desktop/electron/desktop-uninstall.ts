 ...
 * the three user-facing uninstall modes to the `artemis uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m artemis_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `artemis_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'artemis_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Artemis.app/Contents/MacOS/Artemis  → …/Artemis.app
 *   Windows: …\Artemis\Artemis.exe                 → …\Artemis  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Artemis.app/Contents/MacOS/Artemis → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Artemis.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Artemis.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Artemis$/i.test(dir) || /[\\/]artemis-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/artemis
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import artemis_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, artemisHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export ARTEMIS_HOME=${q(artemisHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import artemis_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Artemis install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ARTEMIS_HOME=${String(artemisHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
