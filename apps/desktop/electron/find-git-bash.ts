 ...
 *   1. ARTEMIS_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\artemis\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ARTEMIS_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ARTEMIS_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'artemis', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'artemis', 'git', 'usr', 'bin', 'bash.exe'))
  }
