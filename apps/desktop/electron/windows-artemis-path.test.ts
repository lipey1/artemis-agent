// Unit tests for the pure Windows `artemis` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvArtemisCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `artemis` shim shadows
//      the real artemis.cmd/artemis.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the artemis.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvArtemisCommand() — must probe the venv python via
//      canImportArtemisCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvArtemisCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportArtemisCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    artemisHome: '/fake/artemis-home',
...
test('resolveVenvArtemisCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvArtemisCommand('/root/venv/Scripts/artemis.exe', [], deps), null)
})

test('resolveVenvArtemisCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvArtemisCommand('/root/venv/Scripts/artemis.cmd', [], deps), null)
})

test('resolveVenvArtemisCommand: returns null when the basename is not artemis/artemis.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvArtemisCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvArtemisCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvArtemisCommand('/root/venv/bin/artemis.exe', [], deps), null)
})

test('resolveVenvArtemisCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvArtemisCommand('/root/venv/Scripts/artemis.exe', [], deps), null)
})

test('resolveVenvArtemisCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportArtemisCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvArtemisCommand('/root/venv/Scripts/artemis.exe', ['serve'], deps)
...
test('resolveVenvArtemisCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvArtemisCommand('/root/venv/Scripts/artemis.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'artemis_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvArtemisCommand: is case-insensitive on artemis.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvArtemisCommand('/root/venv/Scripts/ARTEMIS.EXE', [], deps))
  assert.ok(resolveVenvArtemisCommand('/root/venv/SCRIPTS/artemis.exe', [], deps))
})
