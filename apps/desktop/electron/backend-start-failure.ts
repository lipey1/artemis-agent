 ...
 * startArtemis() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startArtemis() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startArtemis`, re-emits
