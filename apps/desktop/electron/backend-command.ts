// Backend subcommand routing for the desktop-managed Artemis process.
//
// The desktop app launches its own headless backend via `artemis serve` — it
...
// app hasn't updated yet, or an older `artemis` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Artemis profile to pin via `--profile`.
 */
 ...
 * `-m artemis_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `artemis_cli/subcommands/dashboard.py` source registers
