...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'artemis-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/artemis-runner-test',
    sourceRepoRoot: null,
    artemisHome: '/tmp/artemis-runner-test',
    logRoot: '/tmp/artemis-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'artemis-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'artemis-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/artemis-agent',
      artemisHome: '/tmp/artemis'
    }),
    ['--dir', '/tmp/artemis-agent', '--artemis-home', '/tmp/artemis', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/artemis-agent', '--artemis-home', '/tmp/artemis', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/artemis',
      artemisHome: '/tmp/home'
    }),
    ['--dir', '/tmp/artemis', '--artemis-home', '/tmp/home', '--branch', 'main']
  )
})
