 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Artemis dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Artemis install on the remote (login-shell probe),
 ...
 *     `ARTEMIS_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForArtemis in.
 *
 * The minted ARTEMIS_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ARTEMIS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.artemis/desktop-ssh'
...
// Resolve the remote artemis executable. An EXPLICIT path is honored strictly
...
async function locateArtemis(ssh, remoteArtemisPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The artemis binary or wrapper script
    ...
    //     the Artemis version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/NousResearch/artemis-agent/issues/74411
    return candidate
  }
  ...
  if (remoteArtemisPath) {
    if (await isExecutable(remoteArtemisPath)) {
      return resolveLauncher(remoteArtemisPath)
    }

    const err: any = new Error(
      `The Artemis path you set is not an executable on the remote host: "${remoteArtemisPath}". ` +
        'Check the path (it must be the full path to the `artemis` binary on the remote, e.g. ' +
        '~/artemis-agent/.venv/bin/artemis), or clear it to auto-detect.'
    )

    err.kind = 'artemis-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v artemis')}`)).trim()
  ...
  candidates.push('~/.local/bin/artemis')
  candidates.push('/usr/local/bin/artemis')
  candidates.push('~/.artemis/artemis-agent/venv/bin/artemis')
  ...
  const err: any = new Error(
    'Artemis is not installed on the remote host (could not find a `artemis` executable). ' +
      'Install it on the remote with:  curl -fsSL https://artemis-agent.nousresearch.com/install.sh | sh  ' +
...
// Probe the resolved binary's version string (first line of `<artemis> --version`,
// e.g. "Artemis Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH artemis a
// connection uses, so a stale/unexpected install is visible.
async function probeArtemisVersion(ssh, artemisPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(artemisPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Artemis Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ARTEMIS_HOME the remote dashboard will use (explicit env wins, else
// ~/.artemis). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteArtemisHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ARTEMIS_HOME:-$HOME/.artemis}"')).trim().split('\n').pop()

    return out || '~/.artemis'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Artemis home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'artemisPath', 'artemisHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, artemisPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !artemisPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(artemisPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.artemisPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(artemisPath, profile, opts: any = {}) {
  const artemis = expandRemotePath(artemisPath)
  ...
  const dashCmd =
    ...
    `exec env ARTEMIS_DESKTOP=1 ${artemis} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, artemisPath) {
  const artemis = expandRemotePath(artemisPath)

  const out = await ssh.exec(
    `help="$(${artemis} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { artemisPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, artemisPath))) {
    const err: any = new Error(
      'The remote Artemis install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Artemis on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(artemisPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForArtemis, a token-gated
...
async function connect(deps) {
  ...
  const artemisPath = await locateArtemis(ssh, remoteArtemisPath)
  log(`located artemis at ${artemisPath}`)
  const artemisVersion = await probeArtemisVersion(ssh, artemisPath)

  if (artemisVersion) {
    log(`remote artemis version: ${artemisVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const artemisHome = await probeRemoteArtemisHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.artemisPath))
    ...
    const reusable =
      ...
      lock.artemisPath === artemisPath &&
      lock.artemisHome === artemisHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForArtemis(baseUrl, spawnToken)
    assertNotAborted(signal)
