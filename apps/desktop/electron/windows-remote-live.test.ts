...
//   ARTEMIS_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ARTEMIS_WIN_SSH_USER   remote user
//   ARTEMIS_WIN_SSH_ARTEMIS absolute path to the remote artemis.exe under test
const liveHost = process.env.ARTEMIS_WIN_SSH_HOST || ''
const liveUser = process.env.ARTEMIS_WIN_SSH_USER || ''
const configuredArtemis = process.env.ARTEMIS_WIN_SSH_ARTEMIS || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Artemis-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredArtemis)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredArtemis.replace('artemis.exe', 'python.exe')}' -m artemis_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredArtemis.replace('artemis.exe', 'python.exe')
        const terminate = `& '${python}' -m artemis_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.artemisPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m artemis_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m artemis_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
