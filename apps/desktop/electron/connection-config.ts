 ...
 *     `X-Artemis-Session-Token` header; WS uses `?token=`.
...
// artemis_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - artemis_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - artemis_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / artemis #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-artemis_session_at', '__Secure-artemis_session_at', 'artemis_session_at']
const RT_COOKIE_VARIANTS = ['__Host-artemis_session_rt', '__Secure-artemis_session_rt', 'artemis_session_rt']

// The Nous portal (NAS) does NOT use Artemis gateway session cookies — it is a
...
// Keep this aligned with artemis_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['artemis', 'test', 'tmp', 'root', 'sudo'])
...
// Artemis Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteArtemisPath = String(entry.remoteArtemisPath || '').trim()

  if (remoteArtemisPath) {
    out.remoteArtemisPath = remoteArtemisPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Artemis installation. Preserve an explicit mapping
  // when it is a valid Artemis profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `ARTEMIS_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://artemis.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a artemis session ACCESS-token cookie
 ...
 * the Artemis gateway session cookies, so cloud sign-in / discovery liveness
