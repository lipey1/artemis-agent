 ...
 * Background
 * ----------
 * After `artemis update` + `artemis desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ARTEMIS_DESKTOP_* is preserved
// plus ARTEMIS_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ARTEMIS_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ARTEMIS_DESKTOP_']
