...
export function handoffResultPath(artemisHome: string): string {
  return path.join(artemisHome, '.artemis-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(artemisHome)
  let raw: string
