/**
 * windows-artemis-path.ts
 *
 * Pure, dependency-injected pieces of Windows `artemis` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvArtemisCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `artemis` shim shadowed the real
 *      artemis.cmd/artemis.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\artemis.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvArtemisCommand() — unwrapWindowsVenvArtemisCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `artemis`) must
 * not shadow `artemis.cmd`/`artemis.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv artemis shim, bootstrap-complete
 * marker) — gating on just the artemis.exe console-script shim alone is the
...
export interface ResolveVenvArtemisCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportArtemisCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `artemis`/`artemis.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/artemis(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m artemis_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportArtemisCli(). A
 * venv whose update died mid-`pip install` still has python.exe + artemis.exe
 ...
 *
 * Returns null when `command` is not a venv artemis shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvArtemisCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvArtemisCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^artemis(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Artemis at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Artemis Python at ${python}`,
    command: python,
