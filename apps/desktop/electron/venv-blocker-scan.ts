...
const SCAN_MODULE = 'artemis_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Artemis process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Artemis installation is free.\n' +
    '\n' +
    'Close other Artemis windows and terminals, then retry.  If the problem\n' +
    'persists, run `artemis update` in a terminal for detailed diagnostics.'
  )
}
