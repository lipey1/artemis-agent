...
test('locateArtemis prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/artemis/, 'OK']])
  assert.equal(await locateArtemis(ssh, '/opt/artemis'), '/opt/artemis')
})

test('locateArtemis throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong artemis" bug.
  const ssh = fakeSsh([
    [/command -v artemis/, '/home/u/.local/bin/artemis\n'],
    [/\[ -x .*\.local\/bin\/artemis/, 'OK']
  ])

  await assert.rejects(
    () => locateArtemis(ssh, '/bad/path/artemis'),
    (err: any) => {
      assert.equal(err.kind, 'artemis-not-found')
      assert.match(err.message, /\/bad\/path\/artemis/)
...
test('locateArtemis falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v artemis/, '/home/u/.local/bin/artemis\n'],
    [/\[ -x .*\.local\/bin\/artemis/, 'OK']
  ])

  assert.equal(await locateArtemis(ssh, ''), '/home/u/.local/bin/artemis')
})

test('locateArtemis preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ARTEMIS_BIN" "$ARTEMIS_ENTRYPOINT" "$@",
  // where $ARTEMIS_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.artemis/artemis-agent/venv/bin/python\n']
  ])

  assert.equal(await locateArtemis(ssh, ''), '/home/u/.local/bin/artemis')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateArtemis must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateArtemis returns an explicit remoteArtemisPath unchanged', async () => {
  // The override half of #74411: an explicit remoteArtemisPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/artemis changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/artemis/, 'OK'],
    [/python3 -c/, '/home/u/.artemis/artemis-agent/venv/bin/python\n']
  ])

  assert.equal(await locateArtemis(ssh, '~/.local/bin/artemis'), '~/.local/bin/artemis')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteArtemisPath must never be rewritten')
})

test('locateArtemis falls back to ~/.local/bin/artemis when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateArtemis(ssh, ''), '~/.local/bin/artemis')
})

test('locateArtemis tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/artemis/, 'OK']])
  assert.equal(await locateArtemis(ssh, ''), '~/.artemis/artemis-agent/venv/bin/artemis')
})

test('locateArtemis throws a artemis-not-found error with an install hint', async () => {
...
test('locateArtemis uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v artemis/, '/x/artemis'],
    [/\[ -x/, 'OK']
  ])

  await locateArtemis(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.artemis/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.artemis/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.artemis/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/artemis'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/artemis serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/artemis'), true)
  assert.equal(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/artemis', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ARTEMIS_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { artemisPath: '/x/artemis', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ARTEMIS_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ARTEMIS_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { artemisPath: '/x/artemis', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nARTEMIS_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.artemis\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/print\("OWNED"/, 'OWNED\n'],
    [/kill 333/, ''],
    [/--version/, 'Artemis Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile artemisPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ artemisPath: '/old/stale/artemis', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteArtemisPath: '/new/artemis', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes artemisHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ARTEMIS_HOME/, '/home/alice/.artemis\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"artemisHome":"\/home\/alice\/\.artemis"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ARTEMIS_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/artemis'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.artemis/artemis-agent/venv/bin/artemis'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/artemis'))
  assert.doesNotThrow(() => validateRemotePath('~/.artemis/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/artemis'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('artemis'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/artemis'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/artemis\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/artemis\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/artemis\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/artemis', '/usr/`id`/artemis', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.artemis/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/artemis')
  assert.ok(result.includes('/usr/local/bin/artemis'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/artemis')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ARTEMIS_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/artemis', 'work', {
    tokenFilePath: `~/.artemis/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.artemis\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/artemis', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Artemis', () => {
  const cmd = buildSpawnCommand('/x/artemis', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ARTEMIS_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { artemisPath: '/x/artemis', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { artemisPath: '/x/artemis', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.artemis/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.artemis/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/artemis serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/artemis serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/artemis'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/artemis'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/artemis'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/artemis'), false)
})
