 ...
 *
 * The staged Tauri `artemis-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `artemis update` refreshes the code that drives the
 ...
 *
 * The Tauri installer self-copies into ARTEMIS_HOME on *every* platform
 * (`artemis-setup.exe` on Windows, `artemis-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_artemis_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `artemis update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(artemisHome, 'artemis-setup.exe')
 ...
 *
 * `copy_self_to_artemis_home` deliberately no-ops during `--update`
 ...
 * "Another Artemis update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `artemis update`, which
 ...
 * the installer is written to ARTEMIS_HOME at install/repair time, making mtime
