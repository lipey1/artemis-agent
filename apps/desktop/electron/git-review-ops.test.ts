...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'artemis-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'artemis-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Artemis Test'], { cwd: dir })
