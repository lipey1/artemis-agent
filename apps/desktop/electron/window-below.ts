// window-below.ts — which OS window sits directly underneath a Artemis window.
//
...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session Artemis is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run Artemis under XWayland with DISPLAY set.'
    )
  }
 ...
 *
 * Walks past every window owned by our own process (all Artemis windows share
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which Artemis does not request for this.'
  }
