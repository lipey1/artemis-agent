// Profile-delete routing logic for the `artemis:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_artemis_home() and
...
/**
 * Parse a `artemis:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `artemis:api` request away from the primary/window backend
 ...
 * ensure_artemis_home() recreates the directory the delete just removed.
 */
