...
export function markerPath(artemisHome) {
  return path.join(artemisHome, '.artemis-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(artemisHome)
  let raw
 ...
 *
 * The Tauri-based artemis-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(artemisHome, pid, { now = Date.now } = {}) {
  const file = markerPath(artemisHome)
 ...
 * Artemis to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(artemisHome, opts)
