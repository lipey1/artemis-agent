...
export const ZOOM_STORAGE_KEY = 'artemis:desktop:zoomLevel'
 ...
 * but forgot to emit 'artemis:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('artemis:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
