...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ARTEMIS_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Artemis-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.artemis/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'artemis-desktop-ssh')
  }

  return path.join(os.homedir(), '.artemis', 'desktop-ssh')
}
