...
export interface ArtemisReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForArtemisReady`).
   */
...
export async function waitForArtemisReady(baseUrl: string, options: ArtemisReadyOptions): Promise<void> {
  ...
  throw new Error(`Artemis backend did not become ready: ${detail}`)
}
