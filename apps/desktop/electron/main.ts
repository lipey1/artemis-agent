...
import {
  ...
  resolveVenvArtemisCommand
} from './windows-artemis-path'
import {
...
const USER_DATA_OVERRIDE = process.env.ARTEMIS_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ARTEMIS_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with ARTEMIS_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[artemis] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[artemis] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ARTEMIS_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[artemis] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[artemis] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `artemis desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Artemis sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[artemis] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[artemis] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[artemis] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[artemis] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[artemis] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('artemis:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Artemis burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[artemis] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[artemis] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[artemis] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[artemis] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ARTEMIS_HOME — the user-facing root for everything Artemis-related. Mirrors
// scripts/install.ps1's $ArtemisHome and scripts/install.sh's $ARTEMIS_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\artemis (matches install.ps1)
//   macOS / Linux: ~/.artemis (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.artemis directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\artemis yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ARTEMIS_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ARTEMIS_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.artemis / %LOCALAPPDATA%\artemis.
function resolveArtemisHome() {
  ...
  if (IS_WINDOWS) {
    ...
    // at login, so a ARTEMIS_HOME set via `setx` AFTER login is invisible in
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localArtemis = path.join(process.env.LOCALAPPDATA, 'artemis')
    const legacyDotArtemis = path.join(app.getPath('home'), '.artemis')

    // Artemis product home. Prefer %LOCALAPPDATA%\artemis; keep reading legacy
    // artemis homes only when artemis is absent so we do not orphan state.
    if (directoryExists(localArtemis)) {
      return localArtemis
    }
    if (directoryExists(legacyDotArtemis)) {
      return legacyDotArtemis
    }

    return localArtemis
  }

  const artemisHome = path.join(app.getPath('home'), '.artemis')
  if (directoryExists(artemisHome)) return artemisHome
  return artemisHome
}

const ARTEMIS_HOME = resolveArtemisHome()

function pathWithArtemisManagedNode(...entries) {
  const managed = artemisManagedNodePathEntries(ARTEMIS_HOME).filter(directoryExists)
...
const VENV_ROOT = path.join(ACTIVE_ARTEMIS_ROOT, 'venv')
...
// boots, going straight to `resolveArtemisBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ARTEMIS_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ARTEMIS_ROOT, '.artemis-bootstrap-complete')
...
// active-profile.json records which Artemis profile the desktop launches its
// local backend as. When set, startArtemis() passes `artemis --profile <name>
// dashboard …`, which deterministically pins ARTEMIS_HOME (see
// _apply_profile_override in artemis_cli/main.py) and bypasses the sticky
// ~/.artemis/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors artemis_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// artemisDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ARTEMIS_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by artemis_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ARTEMIS_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (artemis_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ARTEMIS_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ARTEMIS_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ARTEMIS_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ARTEMIS_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ARTEMIS_DESKTOP_APP_NAME || process.env.ARTEMIS_DESKTOP_APP_NAME || 'Artemis'
...
// 'artemis:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.lipey1.artemis) -
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.lipey1.artemis')
}

// Seed the native About panel with the live Artemis version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `artemis update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'artemis-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(artemis-media://…)`.
...
// startArtemis(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ARTEMIS_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ARTEMIS_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startArtemis() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning artemis serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Artemis backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[artemis] ${line}`)
  artemisLog.push(...lines)

  if (artemisLog.length > 300) {
    artemisLog.splice(0, artemisLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-artemis-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('artemis:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the artemis:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('artemis:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ARTEMIS_ROOT
  })
}
...
// The Tauri updater writes ARTEMIS_HOME/.artemis-update-in-progress for the whole
...
// (`force_kill_other_artemis`, taskkill /IM artemis.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ARTEMIS_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Artemis will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ARTEMIS_HOME)
    ...
    if (result && result.ok) {
      ...
      dialog.showErrorBox(
        'Artemis update did not finish',
        `${result.message}\n\nDetails: ${path.join(ARTEMIS_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `artemis` — must not shadow `artemis.cmd`/`artemis.exe`.
...
function unwrapWindowsVenvArtemisCommand(command, backendArgs) {
  return resolveVenvArtemisCommand(command, backendArgs, {
...
// spawns `artemis serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `artemis` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'artemis_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `artemis --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ARTEMIS_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveArtemisBackend step 4); execFileSync of a .cmd without
...
function findPythonForRoot(root) {
  const override = process.env.ARTEMIS_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Artemis deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the artemis-import
...
// findGitBash — locate bash.exe on Windows. Resolves ARTEMIS_GIT_BASH_PATH
...
// normal ARTEMIS_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\artemis\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'artemis', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'artemis', 'git', 'bin', 'git.exe'))
  }
...
function recentArtemisLog() {
  return artemisLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's artemis root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ARTEMIS_ROOT.
// ARTEMIS_DESKTOP_ARTEMIS_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ARTEMIS_DESKTOP_ARTEMIS_ROOT && path.resolve(process.env.ARTEMIS_DESKTOP_ARTEMIS_ROOT),
    !IS_PACKAGED && isArtemisSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isArtemisSourceRoot(ACTIVE_ARTEMIS_ROOT) ? ACTIVE_ARTEMIS_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ARTEMIS_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('artemis:updates:progress', merged)
  }
}
  ...
  try {
    ...
    if (!res.ok) {
      return {
        ...
        branch: 'main',
        error: 'fetch-failed',
        message: `Artemis releases API HTTP ${res.status}`,
        artemisRoot: updateRoot,
        fetchedAt: Date.now()
      }
    }
    const payload = await res.json()
    const tag = String(payload.tag_name || '').replace(/^v/, '')
    const parseVer = (value) => String(value || '')
      .replace(/^v/, '')
      .split('.')
      .map((chunk) => {
        const digits = chunk.match(/^\d+/)
...
// Windows that binary owns ALL repo mutation — running `artemis update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ARTEMIS_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `artemis update` can write
// fresh entry points. On Windows this is the file the running backend
// `artemis.exe` holds open; on POSIX it's never mandatory-locked.
function venvArtemisShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'artemis.exe')
    : path.join(updateRoot, 'venv', 'bin', 'artemis')
}
...
// only signals the direct child, so on Windows a backend `artemis.exe` that
// spawned its own grandchildren (a `artemis` REPL, a pty terminal session, the
...
// `artemisProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `artemis.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a artemis
// REPL, a pty terminal, the gateway) keep `artemis.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const artemisProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(artemisProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvArtemisShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentArtemisProcess = backendConnectionState.getProcess()

    if (currentArtemisProcess && Number.isInteger(currentArtemisProcess.pid)) {
      stragglers.push(currentArtemisProcess.pid)
    }
...
// `artemis update`). Instead we spawn the staged Artemis-Setup binary with
// --update and quit, so it can run `artemis update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged artemis-setup or not — the resolver
      ...
      // `artemis update` (backend) + `artemis desktop --build-only` (OS-aware GUI
      ...
      if (!scriptHandoff && !pythonHandoff) {
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `artemis update --branch ${branch}`
            }
          }
        } catch {
    ...
    preflightStateDb(ARTEMIS_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // artemis.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running artemis, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Artemis install open ' +
        '(a second Artemis window or a terminal running artemis?). Close it and retry.'
    ...
    // `artemis update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `artemis update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      // The `artemis update` child adopts the SCRIPT's claim via
      ...
      // abort with "Another Artemis update is already running (PID <itself>)" —
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvArtemis = path.join(venvBin, IS_WINDOWS ? 'artemis.exe' : 'artemis')
  ...
  // not just the `artemis.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvArtemis) || fileExists(path.join(updateRoot, '.artemis-bootstrap-complete'))
...
// Resolve the artemis CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `artemis` on PATH.
function resolveArtemisCliBinary(updateRoot) {
  const venvArtemis = path.join(updateRoot, 'venv', 'bin', 'artemis')

  if (fileExists(venvArtemis)) {
    return venvArtemis
  }

  return findOnPath('artemis') || null
}
...
// Python-level pre-update snapshot inside `artemis update`.
function preflightStateDb(artemisHome, rememberLog) {
  const stateDbPath = path.join(artemisHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(artemisHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(artemisHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(artemisHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`artemis update`) + OS-aware GUI rebuild
// (`artemis desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  if (!artemis) {
    emitUpdateProgress({ stage: 'manual', message: 'artemis update', percent: null })

    return { ok: true, manual: true, command: 'artemis update', artemisRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(ARTEMIS_HOME, rememberLog)

  // Put the Artemis-managed Node and the venv on PATH so `artemis desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\artemis\\node, not node\\bin.
  // PYTHONUNBUFFERED: `artemis update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    ARTEMIS_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithArtemisManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `artemis update` reaps stale `artemis serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ARTEMIS_DESKTOP_CHILD_PID and excludes
  ...
  if (artemisProcess && Number.isInteger(artemisProcess.pid)) {
    desktopChildPids.push(artemisProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.ARTEMIS_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Artemis (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(artemis, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'artemis update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'artemis update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(artemis, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Artemis to retry.',
  ...
  // Linux in-app update terminal state (#45205). `artemis desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Artemis…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `artemis-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Artemis to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Artemis desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Artemis to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Artemis.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Artemis.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Artemis to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.artemis-update-new"; then
    rm -rf "$DST.artemis-update-old" 2>/dev/null || true
    mv "$DST" "$DST.artemis-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.artemis-update-new" "$DST"
    rm -rf "$DST.artemis-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Artemis to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ARTEMIS_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    ...
    canImportArtemisCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ARTEMIS_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `artemis
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ARTEMIS_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: artemis desktop --force-build`
  )
...
function resolveArtemisCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Artemis.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ARTEMIS_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveArtemisCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'artemis_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ARTEMIS_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Artemis at ${ACTIVE_ARTEMIS_ROOT}`,
    command,
    args: ['-m', 'artemis_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      artemisHome: ARTEMIS_HOME,
      pythonPathEntries: [ACTIVE_ARTEMIS_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ARTEMIS_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveArtemisBackend(backendArgs) {
  // 1. Explicit override -- ARTEMIS_DESKTOP_ARTEMIS_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ARTEMIS_DESKTOP_ARTEMIS_ROOT && path.resolve(process.env.ARTEMIS_DESKTOP_ARTEMIS_ROOT)

  if (overrideRoot && isArtemisSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Artemis source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `artemis` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isArtemisSourceRoot.)
  if (!IS_PACKAGED && isArtemisSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Artemis source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ARTEMIS_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\artemis\\artemis-agent (Windows) or ~/.artemis/artemis-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Artemis runtime at ${ACTIVE_ARTEMIS_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `artemis` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ARTEMIS_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ARTEMIS_DESKTOP_IGNORE_EXISTING !== '1') {
    let artemisCommand = null
    const artemisOverride = process.env.ARTEMIS_DESKTOP_ARTEMIS

    if (artemisOverride) {
      const resolvedOverride = findOnPath(artemisOverride)

      if (resolvedOverride) {
        artemisCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(artemisOverride, { isWsl: IS_WSL })) {
        artemisCommand = artemisOverride
      } else {
        rememberLog(`Ignoring Windows Artemis override under WSL: ${artemisOverride}`)
      }
    } else {
      artemisCommand = findOnPath('artemis')
    }

    if (artemisCommand) {
      if (looksLikeDesktopAppBinary(artemisCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Artemis CLI: ${artemisCommand}`)
        artemisCommand = null
      }
    }

    if (artemisCommand) {
      const unwrapped = unwrapWindowsVenvArtemisCommand(artemisCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `artemis` shim
      ...
      const shellForProbe = isCommandScript(artemisCommand)

      // ARTEMIS_DESKTOP_ARTEMIS is an explicit deployment override (used by
      ...
      if (shouldTrustArtemisOverride(artemisOverride) || verifyArtemisCli(artemisCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvArtemisCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Artemis CLI at ${artemisCommand}`,
          command: artemisCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Artemis CLI at ${artemisCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed artemis_cli module via system Python.
  ...
  if (python) {
    ...
    // having artemis_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\artemis\artemis-agent\venv.
    if (canImportArtemisCli(python)) {
      return {
        kind: 'python',
        label: `installed artemis_cli module via ${python}`,
        command: python,
        args: ['-m', 'artemis_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: artemis_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveArtemisBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Artemis Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_ARTEMIS_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveArtemisBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Artemis install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Artemis recovery was handed off to Artemis Setup. The desktop will restart when recovery completes.'
      )
  ...
  if (!isArtemisSourceRoot(ACTIVE_ARTEMIS_ROOT)) {
    throw new Error(
      `Artemis install at ${ACTIVE_ARTEMIS_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Artemis' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\artemis\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `artemis` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Artemis on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Artemis.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isArtemisSourceRoot()
    // plus an importable artemis_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Artemis venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Artemis at ${ACTIVE_ARTEMIS_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Artemis runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----artemis-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Artemis backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Artemis backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Artemis backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Artemis-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('artemis:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveArtemisCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('artemis:preview-file-changed', payload)
}
...
// waitForArtemisReady how to read a 401 — rejected session vs gated route.
...
async function waitForArtemis(baseUrl, token, signal?, authMode?) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForArtemisReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('artemis:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('artemis:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('artemis:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Artemis...".
function sendPowerResume() {
  ...
  webContents.send('artemis:power-resume')
}
...
// 'artemis:power-battery' push below.
ipcMain.handle('artemis:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('artemis:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('artemis:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('artemis:window-state-changed', state)
}
...
//
// Hosted Artemis gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``artemis_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / artemis #37247).
//     Both are set as HttpOnly cookies (``artemis_session_at`` ~15 min,
//     ``artemis_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:artemis-remote-oauth'
...
// "not signed in", which on the initial boot path (startArtemis → the renderer's
// single-shot boot() with no retry) surfaced as the "Artemis couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Artemis
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Artemis backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in artemis_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// a valid stored value (pins the root ARTEMIS_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ARTEMIS_DESKTOP_REMOTE_URL)
  ...
  return {
    ...
    sshRemoteArtemisPath: (ssh || savedSsh)?.remoteArtemisPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ARTEMIS_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteArtemisPath: input.sshRemoteArtemisPath ?? existingBlock.remoteArtemisPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (artemis #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Artemis gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Artemis gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Artemis gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.ARTEMIS_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteArtemisPath || '')
    ...
    result = await lifecycle({
      ...
      remoteArtemisPath: sshConfig.remoteArtemisPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForArtemis: (baseUrl, token) => waitForArtemis(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    artemisVersion: result.artemisVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.artemisVersion || 'artemis (version unknown)'} at ${result.artemisPath || '?'}`
  )
  ...
  return { ...connection, remoteArtemisVersion: result.artemisVersion || '' }
}
...
//   2. env override (ARTEMIS_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ARTEMIS_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ARTEMIS_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ARTEMIS_DESKTOP_REMOTE_URL is set but ARTEMIS_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Artemis backend.'
      )
    }
...
// env → global) so a startArtemis() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Artemis
  ...
  if (authRequired) {
    ...
    // button can read "Sign in" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteArtemisPath: input.sshRemoteArtemisPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteArtemisPath || '')
          let artemisPath
          let artemisVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            artemisPath = runtime.artemisPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.artemisPath])
            artemisVersion = inspection.version
            supported = inspection.supported
          } else {
            artemisPath = await remoteLifecycle.locateArtemis(ssh, sshConfig.remoteArtemisPath || '')
            artemisVersion = await remoteLifecycle.probeArtemisVersion(ssh, artemisPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, artemisPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Artemis on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteArtemisPath: artemisPath,
            remoteArtemisVersion: artemisVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startArtemis())
  ...
  // connect to Artemis gateway". Mirror the renderer's connect here so the test
...
function resetArtemisConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const artemisProcess = backendConnectionState.invalidate()
  stopBackendChild(artemisProcess)
...
// startArtemis() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetArtemisConnection() invalidates it.
  const artemisProcess = backendConnectionState.getProcess()
  const dying = artemisProcess && !artemisProcess.killed ? artemisProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('artemis:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startArtemis()
...
// local-spawn portion of startArtemis() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForArtemis(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited ARTEMIS_HOME env (see _apply_profile_override
  // step 3 in artemis_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveArtemisBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const artemisCwd = resolveArtemisCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Artemis backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Artemis backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Artemis backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Artemis backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForArtemis(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Artemis backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_artemis_home() recreates the deleted profile directory.
//
...
async function startArtemis() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startArtemis() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Artemis backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote Artemis backend at ${remote.baseUrl}`, 24)
      await waitForArtemis(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Artemis backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteArtemisVersion: remote.remoteArtemisVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: artemisLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.artemis/active_profile file) and
    // resolves ARTEMIS_HOME the same way `artemis -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Artemis runtime', 28)

        return resolveArtemisBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Artemis backend via ${backend.label}`, 84)
    rememberLog(`Starting Artemis backend via ${backend.label}`)

    const artemisProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: artemisCwd,
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, artemisProcess)

    if (!processOwner) {
      stopBackendChild(artemisProcess)
      throw new Error('Artemis backend start was superseded by a newer connection attempt.')
    }

    artemisProcess.stdout.on('data', rememberLog)
    artemisProcess.stderr.on('data', rememberLog)
    ...
    artemisProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Artemis backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Artemis backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Artemis backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Artemis backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    artemisProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Artemis backend exit (${signal || code})`)
      ...
      rememberLog(`Artemis backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Artemis backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Artemis backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentArtemisLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Artemis backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(artemisProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Artemis backend to become ready', 90)
    await Promise.race([waitForArtemis(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => artemisProcess.exitCode === null && !artemisProcess.killed,
      rememberLog
    })
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Artemis backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Artemis is
...
// pushes pet state over IPC (artemis:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via artemis:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Artemis app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Artemis icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see artemis:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Artemis in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('artemis:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Artemis can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('artemis:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('artemis:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `artemis:hud:set-bounds`, which flips resizable on for the call — the
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('artemis:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('artemis:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('artemis:quick-entry:shown')
}
...
function createWindow() {
  ...
  startArtemis().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('artemis:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Artemis…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('artemis:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetArtemisConnection,
...
ipcMain.handle('artemis:backend:touch', async (_event, profile) => {
...
ipcMain.handle('artemis:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('artemis:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('artemis:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('artemis:zoom:get', event => {
...
ipcMain.on('artemis:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('artemis:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('artemis:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('artemis:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('artemis:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('artemis:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('artemis:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('artemis:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('artemis:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('artemis:pet-overlay:control', payload)
})

// --- HUD mode (chrome-free floating chat) -----------------------------------
ipcMain.handle('artemis:hud:open', async (_event, request) => {
  openHudWindow(
...
ipcMain.handle('artemis:hud:vibrancy', (_event, on) => {
...
// reaches the bar.
ipcMain.on('artemis:hud:ignore-mouse', (_event, ignore) => {
  if (hudWindow && !hudWindow.isDestroyed()) {
    hudWindow.setIgnoreMouseEvents(Boolean(ignore), { forward: true })
  }
})

ipcMain.on('artemis:hud:move-by', (event, delta) => {
...
ipcMain.on('artemis:hud:set-bounds', (event, bounds) => {
...
ipcMain.on('artemis:hud:session', (event, sessionId) => {
  if (hudWindow && !hudWindow.isDestroyed() && event.sender === hudWindow.webContents) {
    hudSessionId = typeof sessionId === 'string' && sessionId ? sessionId : null
  }
})

ipcMain.handle('artemis:hud:close', async () => {
  closeHudWindow()

  return { ok: true }
})
ipcMain.handle('artemis:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startArtemis() call restarts the
...
ipcMain.handle('artemis:bootstrap:repair', async () => {
  // Forceful repair: force the next startArtemis() through the full installer
  ...
  // "bypass active runtime" path inside startArtemis() and fall through
  ...
  remoteReauthFailure = null
  getFirstRunSetupGate().resetForRepair()
  resetArtemisConnection()

  return { ok: true }
})
ipcMain.handle('artemis:bootstrap:continue-local', async () => {
...
ipcMain.handle('artemis:boot-progress:get', async () => bootProgressState)
ipcMain.handle('artemis:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('artemis:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('artemis:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('artemis:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('artemis:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('artemis:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('artemis:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startArtemis() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('artemis:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('artemis:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('artemis:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('artemis:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ARTEMIS_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('artemis:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('artemis:requestMicrophoneAccess', async () => {
...
ipcMain.handle('artemis:window:readBelow', async event => {
...
ipcMain.handle('artemis:api', async (_event, request) => {
  ...
  // backend calls ensure_artemis_home() which recreates the profile directory,
...
ipcMain.handle('artemis:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('artemis:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('artemis:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('artemis:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('artemis:data-url-read-max:get', () => ({
...
ipcMain.handle('artemis:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('artemis:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('artemis:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('artemis:readFileText', async (_event, filePath) => {
...
ipcMain.handle('artemis:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('artemis:writeClipboard', (_event, text) => {
...
ipcMain.handle('artemis:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('artemis:readClipboard', () => clipboard.readText())

ipcMain.handle('artemis:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('artemis:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('artemis:saveClipboardImage', async () => {
...
ipcMain.handle('artemis:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('artemis:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('artemis:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('artemis:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('artemis:active-work', (event, payload) => {
...
ipcMain.on('artemis:titlebar-theme', (_event, payload) => {
...
ipcMain.on('artemis:native-theme', (_event, mode) => {
...
ipcMain.on('artemis:translucency', (_event, payload) => {
...
ipcMain.on('artemis:keep-awake', (_event, on) => {
...
ipcMain.handle('artemis:quick-entry:settings:get', async () => {
...
ipcMain.handle('artemis:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('artemis:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('artemis:quick-entry:submit', {
...
ipcMain.on('artemis:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('artemis:quick-entry:state', payload)
  }
})

ipcMain.on('artemis:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('artemis:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `artemis:window:openSession`).
...
ipcMain.handle('artemis:find-in-page', (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('artemis:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('artemis:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveArtemisCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('artemis:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveArtemisCwd()
}))

ipcMain.handle('artemis:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('artemis:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('artemis:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('artemis:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('artemis:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('artemis:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: artemisLog.slice(-200) }))
...
ipcMain.on('artemis:logs:renderer-error', (_event, report) => {
...
function terminalShellCommand() {
  // ARTEMIS_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ARTEMIS_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Artemis' TUI into
  ...
  env.TERM_PROGRAM = 'Artemis'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a artemis/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ARTEMIS_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ARTEMIS_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `artemis:terminal:${id}:${suffix}`
}
...
ipcMain.handle('artemis:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('artemis:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('artemis:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('artemis:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<ARTEMIS_HOME>/desktop-plugins`,
// resolved from the main-process ARTEMIS_HOME (see resolveArtemisHome) — NOT from
// the connected backend. A remote backend reports its own `artemis_home` over
...
ipcMain.handle('artemis:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped artemis_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(ARTEMIS_HOME, 'profiles', profile) : ARTEMIS_HOME
...
ipcMain.handle('artemis:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('artemis:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('artemis:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('artemis:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('artemis:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('artemis:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('artemis:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('artemis:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('artemis:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('artemis:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('artemis:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('artemis:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('artemis:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('artemis:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('artemis:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('artemis:git:review:prList', async (_event, repoPath, branches, numbers) =>
  reviewPrList(repoPath, resolveGhBinary(), branches, numbers)
)
ipcMain.handle('artemis:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('artemis:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('artemis:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('artemis:terminal:write', (_event, id, data) => {
...
ipcMain.handle('artemis:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('artemis:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('artemis:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('artemis:updates:check', async () =>
...
ipcMain.handle('artemis:updates:apply', async (_event, payload) =>
...
ipcMain.handle('artemis:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('artemis:updates:branch:set', async (_event, name) => {
...
function resolveArtemisVersion() {
  try {
    ...
    const initPath = path.join(root, 'artemis_cli', '__init__.py')
...
// the actual removal via `artemis uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (artemis_cli/uninstall.py + artemis_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ARTEMIS_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isArtemisSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'artemis_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ARTEMIS_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Artemis agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import artemis_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_ARTEMIS_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_ARTEMIS_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `artemis-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `artemis-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('artemis:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('artemis:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('artemis:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('artemis:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// artemis:// deep links (e.g. artemis://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ARTEMIS_PROTOCOL = 'artemis'
const DEEP_LINK_PROTOCOLS = [ARTEMIS_PROTOCOL] as const
...
function _extractDeepLink(argv) {
  ...
  return (
    argv.find(
      a =>
        typeof a === 'string' &&
        DEEP_LINK_PROTOCOLS.some(p => a.startsWith(`${p}://`))
    ) || null
  )
}
...
function handleDeepLink(url) {
  ...
  // artemis://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('artemis:deep-link', payload)
...
ipcMain.handle('artemis:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ARTEMIS_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
app.on('before-quit', event => {
  ...
  // quitting Artemis never keeps another app's chord hostage.
