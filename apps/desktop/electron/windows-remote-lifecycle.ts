...
const READY_RE = /^ARTEMIS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitArtemisPath = '') {
  const explicit = psLiteral(explicitArtemisPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$artemisHome=$env:ARTEMIS_HOME',
    'if(-not $artemisHome){$artemisHome=Join-Path $env:LOCALAPPDATA "artemis"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command artemis.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $artemisHome "artemis-agent\\venv\\Scripts\\artemis.exe")',
    '$candidates+=(Join-Path $artemisHome "artemis-agent\\venv\\Scripts\\artemis.exe")',
    '$candidates+=(Join-Path $HOME "artemis-agent\\.venv\\Scripts\\artemis.exe")',
    '$candidates+=(Join-Path $HOME "artemis-agent\\.venv\\Scripts\\artemis.exe")',
    '$artemis=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $artemis){throw "Artemis is not installed on the remote Windows host."}',
    'if($explicit -and $artemis -ne $explicit){throw "The configured Artemis path is not an executable file."}',
    '$python=Join-Path (Split-Path $artemis) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Artemis Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;artemisHome=$artemisHome;artemisPath=$artemis;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitArtemisPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitArtemisPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'artemis_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.artemisPath === 'string' &&
    typeof lock.artemisHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.artemisPath === runtime.artemisPath &&
    lock.artemisHome === runtime.artemisHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteArtemisPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.artemisPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Artemis on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.artemisPath = inspection.path
  const artemisVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located artemis at ${runtime.artemisPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, artemisPath: runtime.artemisPath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    artemisPath: runtime.artemisPath,
    artemisHome: runtime.artemisHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForArtemis(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Artemis SSH"', 'powershell.exe -NoLogo')
