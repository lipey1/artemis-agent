...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/artemis', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/artemis/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/artemis'), 'https://gw.example.com/artemis/auth/native/refresh')
})
