...
// compile and import the whole `artemis_cli.main` → `web_server` → FastAPI/
 ...
 * ARTEMIS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.ARTEMIS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Artemis backend: exited before port announcement (${signal || code})`))
    }
