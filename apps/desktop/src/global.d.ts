import type { GatewayWsUrlResult } from '@artemis/shared'
...
declare global {
  interface Window {
    artemisDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ArtemisConnection>
      ...
      // Artemis Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ARTEMIS_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ArtemisApiRequest) => Promise<T>
      notify: (payload: ArtemisNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ArtemisReadFileTextResult>
      selectPaths: (options?: ArtemisSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ArtemisPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ArtemisPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ArtemisPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ArtemisActiveWork) => void
      setTitleBarTheme?: (payload: ArtemisTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ArtemisReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ARTEMIS_HOME>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ArtemisGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ArtemisGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ArtemisGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ArtemisRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ArtemisReviewScope, baseRef?: null | string) => Promise<ArtemisReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ArtemisReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ArtemisReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<ArtemisRepoPullRequests>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ArtemisTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ArtemisTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ArtemisWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ArtemisPreviewFileChanged) => void) => () => void
...
export interface ArtemisTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ArtemisTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `artemis update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  artemisRoot?: string
...
export interface ArtemisConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface ArtemisTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ArtemisActiveWork {
  count: number
  titles: string[]
}

export interface ArtemisWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Artemis Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Artemis Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Artemis Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the artemis gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Artemis Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ArtemisApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ArtemisPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ArtemisReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ArtemisPreviewWatch {
  id: string
  path: string
}
...
export interface ArtemisGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ArtemisGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface ArtemisGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ArtemisRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ArtemisRepoStatus {
  branch: null | string
...
// turn began.
export type ArtemisReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ArtemisReviewFile {
  path: string
  added: number
  removed: number
...
export interface ArtemisReviewList {
  files: ArtemisReviewFile[]
...
export interface ArtemisReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ArtemisBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface ArtemisRepoPullRequests {
  ghReady: boolean
  prs: ArtemisBranchPullRequest[]
}
...
export interface ArtemisReviewShipInfo {
  ghReady: boolean
  pr: ArtemisReviewPr | null
}

export interface ArtemisReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ArtemisReadDirResult {
  entries: ArtemisReadDirEntry[]
  error?: string
}

export interface ArtemisPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ArtemisSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
