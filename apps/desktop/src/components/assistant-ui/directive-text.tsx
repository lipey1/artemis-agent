...
const ARTEMIS_REF_TYPES = WIRE_REFERENCE_KINDS
type ArtemisRefType = (typeof ARTEMIS_REF_TYPES)[number]
...
const ARTEMIS_DIRECTIVE_RE = referenceRe()
...
export const artemisDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ARTEMIS_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...artemisDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Artemis directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.artemisDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.artemisDesktop?.readFileDataUrl(id)
