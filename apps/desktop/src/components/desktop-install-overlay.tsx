 ...
 *
 * Renders the first-launch install progress for Artemis. Mounted always;
...
export function DesktopInstallOverlay({ enabled = true }: DesktopInstallOverlayProps) {
  ...
  // there's no Artemis install yet and we can't drive install.sh
        ...
        <div className="w-full max-w-xl rounded-xl border border-(--stroke-nous) bg-card p-8 shadow-nous">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <Button
                onClick={() => {
                  window.artemisDesktop?.openExternal?.(ups.docsUrl)
                }}
                size="sm"
                variant="ghost"
              >
  ...
  // Active success-path splash: logo + normal spinner. Raw stage names like
  // "git" / "Node" stay in the expandable installer output, not the hero.
  if (!failed) {
    return (
      <div className="fixed inset-0 z-(--z-setup) flex items-center justify-center bg-background/95 p-4 backdrop-blur-md">
        <div className="flex w-full max-w-md flex-col items-center rounded-2xl border border-(--stroke-nous) bg-card px-8 py-10 shadow-nous">
          ...
          <p className="mt-2 max-w-sm text-center text-sm text-muted-foreground">{copy.activeDesc}</p>
          ...
          <div className="mt-8 flex items-center gap-3 text-sm text-muted-foreground">
            ...
            <span>
              {totalCount === 0
                ...
                : `${copy.progress(completedCount, totalCount)}${currentElapsed ? ` · ${currentElapsed}` : ''}`}
            </span>
          </div>
            ...
            <div className="mt-4">
              <Button
                ...
                onClick={async () => {
                  setCancelling(true)
                  try {
                    await window.artemisDesktop?.cancelBootstrap?.()
                  } catch {
    ...
    <div className="fixed inset-0 z-(--z-setup) flex items-center justify-center bg-background/90 backdrop-blur-md p-4">
      <div className="flex w-full max-w-2xl max-h-[90vh] flex-col rounded-xl border border-(--stroke-nous) bg-card shadow-nous">
        ...
        <div className="flex flex-shrink-0 items-start gap-4 p-8 pb-4">
          <div className="min-w-0">
            <h2 className="text-xl font-semibold tracking-tight">{copy.failedTitle}</h2>
            <p className="mt-1.5 text-sm text-muted-foreground">{copy.failedDesc}</p>
          </div>
        </div>
          ...
          <div className="flex-shrink-0 bg-card p-4">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs text-muted-foreground">
                ...
                <code className="font-mono text-(--ui-text-secondary)">%LOCALAPPDATA%\artemis\logs\</code>
              </span>
              <div className="flex gap-2">
                ...
                <Button
                  onClick={async () => {
                    ...
                    try {
                      await window.artemisDesktop?.resetBootstrap?.()
                    } catch {
