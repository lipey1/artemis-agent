...
describe('connecting overlay vs recovery surface', () => {
  it('hard initial-boot failure surfaces the recovery overlay (the working path)', async () => {
    ...
    $desktopBoot.set({
      ...$desktopBoot.get(),
      error: 'Artemis backend did not become ready',
      running: false,
      visible: true
    })
  ...
  it('FIX: once the prolonged reconnect raises a recoverable boot error, the recovery overlay takes over', async () => {
    ...
    $desktopBoot.set({
      ...$desktopBoot.get(),
      error: 'Lost connection to the Artemis gateway and could not reconnect.',
      running: false,
      visible: true
    })
  ...
  it('hides the black sheet when boot completes even if the gateway never opens', async () => {
    ...
    const overlay = document.querySelector('[data-connecting-overlay]')
    expect(overlay === null || overlay.hasAttribute('hidden')).toBe(true)
