...
const STORAGE_KEY = 'artemis.desktop.layoutTree.v2'

writeKey('artemis.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('artemis.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('artemis.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'artemis.desktop.dismissedPanes.v1'
...
const PANE_SHARE_KEY = 'artemis.desktop.paneShare.v1'
...
const USER_PLACED_KEY = 'artemis.desktop.userPlacedPanes.v1'
...
/** Default tree declared by the app root, or null before that runs. */
export function getDefaultLayoutTree(): LayoutNode | null {
  return defaultTree
}

/** If the live tree is missing, adopt the declared default so the shell
 *  never paints an empty black content area under the titlebar. */
export function ensureDefaultLayoutTree(): LayoutNode | null {
  const current = $layoutTree.get()
...
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ARTEMIS_LAYOUT_TREE__ = {
    close: closeTreePane,
