...
import { $layoutTree, ensureDefaultLayoutTree, getDefaultLayoutTree, trackActiveTreeGroup } from '../store'
...
export function LayoutTreeRoot({ children }: { children?: ReactNode }) {
  const live = useStore($layoutTree)
  const tree = live ?? getDefaultLayoutTree()
  ...
  useEffect(() => {
    if (!live) {
      ensureDefaultLayoutTree()
    }
  }, [live])

  if (!tree) {
    return <div className="relative flex min-h-0 min-w-0 flex-1" />
  }

  return (
    <div className="relative flex min-h-0 min-w-0 flex-1">
      {/* ZonesOverlay::GetAnimationAlpha ramp: clamp(t / 200ms, 0.001, 1). */}
      <style>{`@keyframes artemis-zone-fade { from { opacity: 0.001 } to { opacity: 1 } }`}</style>
