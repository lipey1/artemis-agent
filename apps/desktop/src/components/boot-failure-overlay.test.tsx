...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Artemis gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.artemisDesktop
  Object.defineProperty(window, 'artemisDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'artemisDesktop', { configurable: true, value: original })
}
