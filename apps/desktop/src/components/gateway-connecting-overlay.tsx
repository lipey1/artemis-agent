...
// Never cover the shell forever: if boot/gateway stall, drop the overlay.
...
export function GatewayConnectingOverlay() {
  ...
  // Kick off the exit when the shell is usable: gateway open, boot finished,
  // or `connecting` dropped after we already painted. Opacity-only hide used
  // to leave a `fixed inset-0` black sheet in the tree forever when the
  // gateway never reached `open` after boot.progress hit 100.
  const bootReady = !boot.running && boot.progress >= 100 && !boot.error
  const shouldExit = shownRef.current && (gatewayState === 'open' || bootReady || !connecting)
  ...
  }, [phase, previewing, shouldExit, reduce])

  // Last-resort: never sit on a black sheet if boot/gateway stall.
  useEffect(() => {
    if (phase !== 'live' || previewing) {
      return
    }

    const id = window.setTimeout(() => setPhase(reduce ? 'gone' : 'text-out'), FAILSAFE_MS)

    return () => window.clearTimeout(id)
  }, [phase, previewing, reduce])
