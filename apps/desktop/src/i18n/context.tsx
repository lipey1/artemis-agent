...
import { getArtemisConfigRecord, type ArtemisConfigRecord, saveArtemisConfig } from '@/artemis'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ArtemisConfigRecord>
  saveConfig: (config: ArtemisConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.artemisDesktop?.api) {
      return Promise.resolve({})
    }

    return getArtemisConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.artemisDesktop?.api) {
    ...
    return saveArtemisConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ArtemisConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ArtemisConfigRecord, locale: Locale): ArtemisConfigRecord {
