import { JsonRpcGatewayClient } from '@artemis/shared'
...
// "Timed out connecting to Artemis backend" that hangs the UI (#48504).
//
...
export class ArtemisGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Artemis gateway connection closed',
      connectErrorMessage: 'Could not connect to Artemis gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Artemis gateway is not connected',
...
// the call; each pooled backend already has its own ARTEMIS_HOME, so no backend
...
/** Options for a plugin REST call — mirrors the app's own `artemisDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see ArtemisApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.artemisDesktop?.api) {
    throw new Error('Artemis desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.artemisDesktop.api<T>({
...
export function pluginSocket(pluginId: string, path: string, onMessage: (data: unknown) => void): () => void {
  ...
  const connect = async () => {
    const connection = await window.artemisDesktop.getConnection().catch(() => null)
...
export async function listSessions(
  ...
  const result = await window.artemisDesktop.api<PaginatedSessions>({
    path:
...
// "Error invoking remote method 'artemis:api': Error: 404: ..." through the
...
export function scanSessionPullRequests(
  ...
  return window.artemisDesktop.api<{
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.artemisDesktop.api<SidebarSessionsResponse>({
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.artemisDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.artemisDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.artemisDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(
  ...
  return window.artemisDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.artemisDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.artemisDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.artemisDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.artemisDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getArtemisConfig(profile?: string): Promise<ArtemisConfig> {
  return window.artemisDesktop.api<ArtemisConfig>({
...
export function getArtemisConfigRecord(): Promise<ArtemisConfigRecord> {
  return window.artemisDesktop.api<ArtemisConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getArtemisConfigDefaults(): Promise<ArtemisConfigRecord> {
  return window.artemisDesktop.api<ArtemisConfigRecord>({
    ...profileScoped(),
...
export function getArtemisConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.artemisDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveArtemisConfig(config: ArtemisConfigRecord): Promise<{ ok: boolean }> {
  return window.artemisDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.artemisDesktop.api<MemoryProviderConfig>({
    ...profileScoped(),
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.artemisDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.artemisDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.artemisDesktop.api<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.artemisDesktop.api<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return window.artemisDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function revealEnvVar(key: string): Promise<{ key: string; value: string }> {
  return window.artemisDesktop.api<{ key: string; value: string }>({
    ...profileScoped(),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.artemisDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.artemisDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string): Promise<OAuthStartResponse> {
  return window.artemisDesktop.api<OAuthStartResponse>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.artemisDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(providerId: string, sessionId: string): Promise<OAuthPollResponse> {
  return window.artemisDesktop.api<OAuthPollResponse>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.artemisDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getSkills(): Promise<SkillInfo[]> {
  return window.artemisDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
}
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.artemisDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function getLearningNode(id: string): Promise<LearningNodeDetail> {
  return window.artemisDesktop.api<LearningNodeDetail>({
    ...profileScoped(),
...
export function deleteLearningNode(id: string): Promise<{ message: string; ok: boolean }> {
  return window.artemisDesktop.api<{ message: string; ok: boolean }>({
    ...profileScoped(),
...
export function setSkillEnabled(
  ...
  return window.artemisDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(),
...
export function testMcpServer(name: string): Promise<McpTestResult> {
  return window.artemisDesktop.api<McpTestResult>({
    ...profileScoped(),
 ...
 *  `saveArtemisConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string): Promise<McpOAuthFlow> {
  return window.artemisDesktop.api<McpOAuthFlow>({
    ...profileScoped(),
...
export function getToolsets(): Promise<ToolsetInfo[]> {
  return window.artemisDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export function getToolsetConfig(name: string): Promise<ToolsetConfig> {
  return window.artemisDesktop.api<ToolsetConfig>({
    ...profileScoped(),
...
export function getToolsetModels(name: string, provider?: string): Promise<ToolsetModelsResponse> {
  ...
  return window.artemisDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(),
...
export function selectToolsetModel(
  ...
  return window.artemisDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(),
...
export function selectToolsetProvider(
  ...
  return window.artemisDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(),
...
export function runToolsetPostSetup(name: string, key: string): Promise<ActionResponse & { key: string }> {
  return window.artemisDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.artemisDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.artemisDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.artemisDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function grantComputerUsePermissions(): Promise<ActionResponse> {
  return window.artemisDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.artemisDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.artemisDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.artemisDesktop.api<MessagingPlatformTestResponse>({
...
export function getPairing(): Promise<PairingResponse> {
  return window.artemisDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.artemisDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.artemisDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.artemisDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.artemisDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.artemisDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
// Cron jobs are stored per-profile (<ARTEMIS_HOME>/cron/jobs.json), and the
...
export function getCronJobs(profile?: string): Promise<CronJob[]> {
  ...
  return window.artemisDesktop.api<CronJob[]>({
    ...profileScoped(),
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.artemisDesktop.api<{ runs: SessionInfo[] }>({
    ...profileScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.artemisDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.artemisDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.artemisDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.artemisDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.artemisDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.artemisDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.artemisDesktop.api<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return window.artemisDesktop.api<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return window.artemisDesktop.api<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
...
export function getUsageAnalytics(days = 30): Promise<AnalyticsResponse> {
  return window.artemisDesktop.api<AnalyticsResponse>({
    ...profileScoped(),
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.artemisDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
// curation `artemis model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(provider: string): Promise<RecommendedDefaultModel> {
  return window.artemisDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(),
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.artemisDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.artemisDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.artemisDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  return window.artemisDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function updateArtemis(): Promise<ActionResponse> {
  return window.artemisDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/artemis/update',
    method: 'POST'
  })
}
...
export function checkArtemisUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.artemisDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/artemis/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200): Promise<ActionStatusResponse> {
  return window.artemisDesktop.api<ActionStatusResponse>({
    ...profileScoped(),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.artemisDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.artemisDesktop.api<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.artemisDesktop.api<ElevenLabsVoicesResponse>({
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `artemis skills`
...
export function getSkillHubSources(): Promise<SkillHubSourcesResponse> {
  return window.artemisDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(),
...
export function searchSkillsHub(query: string, source = 'all', limit = 20): Promise<SkillHubSearchResponse> {
  ...
  return window.artemisDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(),
...
export function previewSkillHub(identifier: string): Promise<SkillHubPreview> {
  return window.artemisDesktop.api<SkillHubPreview>({
    ...profileScoped(),
...
export function scanSkillHub(identifier: string): Promise<SkillHubScanResult> {
  return window.artemisDesktop.api<SkillHubScanResult>({
    ...profileScoped(),
...
// `artemis mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveArtemisConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.artemisDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getMcpCatalog(): Promise<McpCatalogResponse> {
  return window.artemisDesktop.api<McpCatalogResponse>({
    ...profileScoped(),
...
export function installMcpCatalogEntry(
  ...
  return window.artemisDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `artemis memory` / `artemis curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.artemisDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.artemisDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.artemisDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.artemisDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `artemis doctor` / `artemis security
// audit` / `artemis backup` / `artemis debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.artemisDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.artemisDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.artemisDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.artemisDesktop.api<DebugShareResponse>({
