 ...
 * composer with the reply scrolling above it, so Artemis can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.artemisDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.artemisDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.artemisDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.artemisDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
