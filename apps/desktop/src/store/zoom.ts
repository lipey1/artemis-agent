...
export function setZoomPercent(percent: number): void {
  window.artemisDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.artemisDesktop?.zoom) {
  void window.artemisDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.artemisDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
