...
import type { ArtemisGitWorktree, ArtemisRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ArtemisGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ArtemisRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ArtemisGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ArtemisRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ArtemisGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ArtemisRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ArtemisGitWorktree[]>>()
const $noRepoStatus = atom<ArtemisRepoStatus | null>(null)
const $noWorktrees = atom<ArtemisGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ArtemisRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ArtemisGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ArtemisRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ArtemisGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ArtemisRepoStatus | null>
  seq: number
  target: string
}
