...
import type { ArtemisGitBaseBranch, ArtemisGitBranch } from '@/global'
import { getArtemisConfig, type ArtemisGateway } from '@/artemis'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/artemis'
...
const PROJECT_SCOPE_KEY = 'artemis.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Artemis gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Artemis profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: ArtemisGateway): Promise<void> {
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await window.artemisDesktop.api<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<ArtemisGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ArtemisGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getArtemisConfig(context.profile))
...
// repo, returning where Artemis should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ArtemisGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ArtemisGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.artemisDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.artemisDesktop?.writeClipboard?.(path)
  }
}
