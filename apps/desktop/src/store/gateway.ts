import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@artemis/shared'
...
const isOpen = (gateway: ArtemisGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: ArtemisGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<ArtemisGateway | null>>
}

const STATE_KEY = Symbol.for('artemis.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<ArtemisGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: ArtemisGateway | null, profile = 'default'): void {
...
export function activeGateway(): ArtemisGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ArtemisGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
