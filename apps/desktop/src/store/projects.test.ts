...
const artemis = await import('@/artemis')
const getArtemisConfig = vi.mocked(artemis.getArtemisConfig)
...
describe('project scope', () => {
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('artemis.desktop.projectScope')).toBe('p_abc')
  })
})
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getArtemisConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getArtemisConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getArtemisConfig).not.toHaveBeenCalled()
  })
})
