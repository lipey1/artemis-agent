...
const STORAGE_KEY = 'artemis:native-notifications'
...
// "Backgrounded" = the user isn't on Artemis. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.artemisDesktop?.notify({
 ...
 *  user is away from Artemis — the in-app toast (`host.notify`) covers the
 *  foreground case. */
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.artemisDesktop
