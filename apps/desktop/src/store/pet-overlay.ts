 ...
 * app's bounds and stays visible while Artemis is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'artemis.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'artemis.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.artemisDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.artemisDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.artemisDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.artemisDesktop?.petOverlay?.close()
}
