...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/artemis'
...
import type { ProfileInfo } from '@/types/artemis'
...
// /api/profiles/active `current`). "default" is the root ~/.artemis. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'artemis.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'artemis.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.artemisDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ARTEMIS_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.artemisDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.artemisDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'artemis.desktop.showAllProfiles'
...
// Switch to the default (root ~/.artemis) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.artemisDesktop?.touchBackend?.(target).catch(() => undefined)
}
