 ...
 *
 * The archive is the CLI's own `artemis profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/artemis'
...
import type { ProfileDesktopOverlay } from '@/types/artemis'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Artemis profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.artemisDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.artemisDesktop?.selectPaths?.({
