...
import type { ArtemisReviewFile, ArtemisReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Artemis' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'artemis.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'artemis.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'artemis.desktop.reviewTreeMode'
const SELECTED_KEY = 'artemis.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ArtemisReviewFile[]>([])
...
export const $reviewShipInfo = atom<ArtemisReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['artemisDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ArtemisReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ArtemisReviewFile[], path: string): ArtemisReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.artemisDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.artemisDesktop?.openExternal?.(url)
    }
