import type { ConnectionState } from '@artemis/shared'
import { atom, computed } from 'nanostores'

import { lastVisibleMessageIsUser } from '@/app/chat/thread-loading'
import type { ContextSuggestion } from '@/app/types'
import type { ArtemisConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/artemis'
...
const COMPOSER_MODEL_KEY = 'artemis.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'artemis.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'artemis.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'artemis.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'artemis.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'artemis.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'artemis.desktop.lastRoute'
...
function workspaceCwdKey(connection: ArtemisConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.artemisDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.artemisDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ArtemisConnection | null>(null)
...
export const setConnection = (next: Updater<ArtemisConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useArtemisConfig.
