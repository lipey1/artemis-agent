...
export function closeFindBar(): void {
  ...
  void window.artemisDesktop?.stopFindInPage()
}
...
export function setFindQuery(query: string): void {
  ...
  void window.artemisDesktop?.findInPage(query, { forward: true, findNext: false })
}
...
export function findNext(): void {
  ...
  if (query) {
    void window.artemisDesktop?.findInPage(query, { forward: true, findNext: true })
  }
}
...
export function findPrevious(): void {
  ...
  if (query) {
    void window.artemisDesktop?.findInPage(query, { forward: false, findNext: true })
  }
}
...
export function initFindInPageListener(): () => void {
  ...
  if (listenerRefs === 1) {
    detachListener = window.artemisDesktop?.onFoundInPage?.(result => {
