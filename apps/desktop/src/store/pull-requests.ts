...
import type { ArtemisBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/artemis'
...
export const $pullRequestsByBranch = atom<Record<string, ArtemisBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'artemis.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('artemis.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: ArtemisBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
