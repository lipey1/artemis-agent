...
import { getArtemisConfigRecord, saveArtemisConfig } from '@/artemis'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getArtemisConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveArtemisConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
