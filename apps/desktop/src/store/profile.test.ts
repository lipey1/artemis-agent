...
const { getProfiles } = await import('@/artemis')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/artemis/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<ArtemisConnection> = {}): ArtemisConnection =>
  ({ baseUrl: 'https://artemis-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ArtemisConnection

const localConn = (over: Partial<ArtemisConnection> = {}): ArtemisConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ArtemisConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ArtemisConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { artemisDesktop: { getConnection } })
