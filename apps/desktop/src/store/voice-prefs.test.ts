...
vi.mock('@/artemis', () => ({
  getArtemisConfigRecord: vi.fn(async () => ({})),
  saveArtemisConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye artemis', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye artemis')
  })
