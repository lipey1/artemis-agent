 ...
 * (`artemis:data-url-read-max:*`) and the default/clamp in electron/hardening.ts.
 */
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.artemisDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.artemisDesktop?.dataUrlReadMax) {
