...
//
// When someone is in a hands-free "Hey Artemis" voice chat, the natural way to
...
// phrase (optionally addressed to Artemis), so a real turn that merely contains
...
// Optional address prefixes so "artemis stop" / "ok stop" / "hey artemis, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey artemis', 'hey artemis,', 'artemis', 'artemis,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("artemis") is not a stop command on its own.
      continue
    }
 ...
 * to Artemis). Returns false for anything that merely contains "stop" as part of
