...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.artemisDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.artemisDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.artemisDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.artemisDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `artemis-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.artemis/cache, etc.); /api/media is intentionally
