import type {
  ArtemisConnection,
  ArtemisReadDirResult,
  ArtemisReadFileTextResult,
  ArtemisSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ArtemisConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: ArtemisConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<ArtemisReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ArtemisReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ArtemisReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ArtemisReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: ArtemisSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
