...
/** Why a known Artemis command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Artemis profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Artemis version', surface: exec() },
...
function isKnownArtemisSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Artemis' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownArtemisSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Artemis and were never asked for. It is
