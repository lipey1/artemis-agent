 ...
 * `ArtemisPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/artemis'
...
export type { PluginRestOptions } from '@/artemis'
 ...
 *  under `artemis.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.artemisDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Artemis — use `host.notify` for the in-app toast.
...
export interface ArtemisPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `artemis.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.artemisDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.artemisDesktop
