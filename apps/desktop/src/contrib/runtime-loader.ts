 ...
 *   (`@artemis/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default ArtemisPlugin -> register(ctx)
 *
 ...
 * and `<artemis home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { createPluginContext, type ArtemisPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@artemis/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @artemis/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: ArtemisPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default ArtemisPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<artemis home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's artemis_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // artemis_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
