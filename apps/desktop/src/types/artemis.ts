...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Artemis doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in artemis_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `artemis model` uses — instead of
...
export type ArtemisConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `artemis model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Artemis simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (artemis.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors artemis_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Artemis, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `artemis tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Artemis); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/artemis/update/check` — the backend's own update state.
