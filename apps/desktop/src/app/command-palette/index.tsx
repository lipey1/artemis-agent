...
import { getArtemisConfigRecord, listAllProfileSessions } from '@/artemis'
...
// Artemis session ids: <YYYYMMDD>_<HHMMSS>_<6 hex>. Used to offer a direct
...
const NON_CONFIG_SETTINGS: ReadonlyArray<{
  ...
  {
    icon: Globe,
    keywords: ['custom', 'endpoint', 'openai compatible', 'ollama', 'vllm', 'local'],
...
function CommandPaletteBody({ onExited }: { onExited: () => void }) {
  ...
  const configQuery = useQuery({
    ...
    queryFn: getArtemisConfigRecord
  })
        ...
        : []),
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Download,
            id: 'cc-update-artemis',
            keywords: ['update', 'upgrade', 'artemis', 'version', 'system', 'restart'],
            label: cc.updateArtemis,
