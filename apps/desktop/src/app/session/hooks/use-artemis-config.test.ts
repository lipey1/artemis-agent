...
import { getArtemisConfig } from '@/artemis'
...
import { useArtemisConfig } from './use-artemis-config'

vi.mock('@/artemis', () => ({
  getArtemisConfig: vi.fn(),
  getArtemisConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'artemis.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getArtemisConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getArtemisConfig>>)

describe('useArtemisConfig refreshArtemisConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Artemis' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useArtemisConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshArtemisConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useArtemisConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getArtemisConfig>>>()
    vi.mocked(getArtemisConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useArtemisConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshArtemisConfig(true)
    })
    expect(getArtemisConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getArtemisConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getArtemisConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getArtemisConfig>>>()
    vi.mocked(getArtemisConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshArtemisConfig(true)
      refreshC = result.current.refreshArtemisConfig(true)
    })
