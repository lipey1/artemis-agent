...
import { sameGatewayFailure, unwrapAgentInitErrorMessage } from '@/lib/provider-setup-errors'
        ...
        if (streamId && prev.some(m => m.id === streamId)) {
          ...
          if (fallbackIndex >= 0) {
            ...
            if (
              completionError &&
              existing.error &&
              sameGatewayFailure(existing.error, completionError)
            ) {
              // The gateway `error` event already created this bubble
              // (failAssistantMessage). The terminal frame is the same
              // failure, possibly wrapped as "agent init failed: …". Reuse
              // the row with the unwrapped copy instead of appending a twin.
              const error = unwrapAgentInitErrorMessage(completionError)

              nextMessages = prev.map((message, messageIndex) =>
                messageIndex === index ? { ...completeMessage(message), error } : message
              )
            } else if (existing.pending || (!interimBoundaryPending && finalText && existingText === finalText)) {
              nextMessages = prev.map((message, messageIndex) =>
  ...
  const failAssistantMessage = useCallback(
    (sessionId: string, errorMessage: string) => {
      updateSessionState(sessionId, state => {
        ...
        const groupId = state.pendingBranchGroup ?? undefined
        const prev = state.messages
        const error = errorMessage.trim() || 'Artemis reported an error'
