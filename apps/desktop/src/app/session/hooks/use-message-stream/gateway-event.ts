...
import type { ArtemisSkin } from '@artemis/shared/skin'
...
import { isProviderSetupErrorMessage, unwrapAgentInitErrorMessage } from '@/lib/provider-setup-errors'
...
/** Toast or onboarding for a turn-ending gateway failure. Stable id collapses
 *  a wrapped `agent init failed: <msg>` event and the raw `<msg>` terminal
 *  frame into one toast. */
function surfaceTurnEndingError(errorMessage: string): string {
  const message = unwrapAgentInitErrorMessage(errorMessage) || errorMessage

  if (isProviderSetupErrorMessage(message)) {
    requestDesktopOnboarding(message)
  } else if (isDiskFullErrorMessage(message)) {
    notifyError(new Error(message), translateNow('notifications.errors.diskFull'))
  } else {
    notify({
 ...
 * billing-specific toast — never the generic "Artemis error" — with a smart CTA
...
interface GatewayEventDeps {
  ...
  refreshArtemisConfig: () => Promise<void>
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshArtemisConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: ArtemisSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (Artemis activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as ArtemisSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshArtemisConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: unwrapAgentInitErrorMessage(
                  coerceGatewayText(payload.error).trim() || finalText || 'Artemis reported an error'
                ),
        ...
        } else if (failure && !isDiskFullErrorMessage(failure.error)) {
          // Terminal frames are the single user-facing signal for a failed
          // turn (agent-init no longer also emits a wrapped `error` event).
          // Toast here so a turn-ending failure is not easy to miss inline.
          // Disk-full is toasted in completeAssistantMessage.
          surfaceTurnEndingError(failure.error)
        }
        ...
        if (requestId) {
          const read = window.artemisDesktop?.readWindowBelow
        ...
        // native OS notification when Artemis is backgrounded; dispatch is gated
        ...
        const errorMessage = unwrapAgentInitErrorMessage(payload?.message || 'Artemis reported an error')
        ...
        surfaceTurnEndingError(errorMessage)
