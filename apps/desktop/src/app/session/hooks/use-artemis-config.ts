...
import { getArtemisConfig, getArtemisConfigDefaults } from '@/artemis'
...
interface ArtemisConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useArtemisConfig({ activeSessionIdRef }: ArtemisConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshArtemisConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getArtemisConfig(), getArtemisConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Artemis' built-in medium over the user's config.
  ...
  return { refreshArtemisConfig, sttEnabled, voiceMaxRecordingSeconds }
}
