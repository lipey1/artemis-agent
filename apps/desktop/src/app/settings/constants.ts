...
// lib/personalities (mirrors artemis_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__artemis_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Artemis & Nous-trained models',
  ...
  { prefix: 'ARTEMIS_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // artemis_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Artemis stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Artemis can read from one file request.',
  approvals: {
    mode: 'How Artemis handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Artemis updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
