...
beforeEach(() => {
  profiles.set([
    ...
    },
    {
      ...
      model: null,
      name: 'work',
      path: '/tmp/artemis/profiles/work',
      provider: null,
      skill_count: 0
    }
  ])
...
describe('GatewaySettings', () => {
  it('labels local mode as default inheritance for a named profile', async () => {
    ...
    expect(
      screen.getByText('Start a private Artemis backend on localhost. This is the default and works offline.')
    ).toBeTruthy()
    ...
    expect(
      screen.queryByText('Start a private Artemis backend on localhost. This is the default and works offline.')
    ).toBeNull()
  })
    ...
    getConnectionConfig.mockImplementation(async profile =>
      profile === 'work'
        ? {
            ...
            sshPort: 22,
            sshKeyPath: '',
            sshRemoteArtemisPath: '/opt/artemis/bin/artemis',
