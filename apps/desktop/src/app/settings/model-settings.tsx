...
import { invalidateArtemisConfig, setArtemisConfigCache, useArtemisConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `artemis model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in artemis_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useArtemisConfigRecord()
  const setConfig = setArtemisConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `artemis model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Artemis runs the flow for you.`}
          </p>
        )}
