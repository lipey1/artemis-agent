...
import { getElevenLabsVoices, getArtemisConfigSchema, saveArtemisConfig } from '@/artemis'
...
import { setArtemisConfigCache, useArtemisConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ArtemisConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ArtemisConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useArtemisConfigRecord()
  ...
  const {
    ...
    queryKey: ['artemis-config-schema'],
    queryFn: getArtemisConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveArtemisConfig(config)
  ...
  const updateConfig = (next: ArtemisConfigRecord) => {
