...
// User-installed plugins first, bundled last — mirrors `artemis plugins list`.
...
// to `artemis dashboard`; model-providers/* are configured in Settings →
...
function reveal(file: string) {
  void window.artemisDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // artemis_home breaks against a remote backend (#66899).
    const dir = await window.artemisDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.artemisDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's artemis home (profile-aware), so the
// path comes from the gateway — not from the renderer's local ARTEMIS_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.artemisDesktop?.openDir?.(`${home}/plugins`)
