...
import type { ToolsetConfig } from '@/types/artemis'
...
const getArtemisConfigSchema = vi.fn()
...
vi.mock('@/artemis', () => ({
  ...
  getArtemisConfigSchema: () => getArtemisConfigSchema(),
...
beforeEach(() => {
  ...
  getArtemisConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getArtemisConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveArtemisConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveArtemisConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
