...
import type { ArtemisConfigRecord, SessionInfo } from '@/types/artemis'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveArtemisConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Artemis wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.artemisDesktop` is
    ...
    const settings = window.artemisDesktop?.settings
