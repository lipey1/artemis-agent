...
import { disconnectOAuthProvider, listOAuthProviders } from '@/artemis'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/artemis'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.artemisDesktop?.terminal)
...
function ConnectedProviderRow({
  ...
  // Artemis can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Artemis can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Artemis never deletes creds another tool owns behind a silent API call.
