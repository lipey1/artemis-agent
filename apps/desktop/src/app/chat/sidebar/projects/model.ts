...
// zero Artemis sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, ArtemisGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, ArtemisGitWorktree[]>>({})
