...
import type { SessionInfo } from '@/artemis'
...
function SidebarSectionHeader({
  ...
  const labelBody = (
    <>
      <SidebarPanelLabel icon={icon}>{label}</SidebarPanelLabel>
      {meta && <SidebarSectionMeta>{meta}</SidebarSectionMeta>}
    </>
  )

  return (
    <div className="group/section flex shrink-0 items-center justify-between gap-1 px-2 pb-1 pt-1.5">
      {collapsible ? (
        <button
...
interface SidebarSessionsSectionProps {
  ...
  projectRepoWorktrees?: Record<string, ArtemisGitWorktree[]>
