...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/artemis'
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('artemis:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('artemis:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Artemis to
          ...
          <div className={cn('flex min-h-0 flex-1 flex-col pb-1.75', SCROLL_Y, SCROLL_GUTTER)}>
            ...
            {!trimmedQuery && (
              <SidebarSessionsSection
                ...
                label={sessionsLabel}
                labelIcon={<Codicon className="shrink-0" name="comment-discussion" size="0.75rem" />}
                labelMeta={
                  worktreeGroupingActive ? (
