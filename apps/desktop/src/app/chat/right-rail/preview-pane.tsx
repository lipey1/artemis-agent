        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.artemisDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Artemis to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.artemisDesktop?.watchPreviewFile ||
      !window.artemisDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.artemisDesktop.onPreviewFileChanged(payload => {
    ...
    void window.artemisDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.artemisDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.artemisDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:artemis-preview')
