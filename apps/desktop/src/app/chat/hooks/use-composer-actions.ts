...
export async function attachmentPreviewDataUrl(filePath: string): Promise<string> {
  try {
    const local = await window.artemisDesktop?.readFileDataUrl?.(filePath)
...
export const ARTEMIS_PATHS_MIME = 'application/x-artemis-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Artemis sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.artemisDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(ARTEMIS_PATHS_MIME)
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.artemisDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.artemisDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.artemisDesktop?.getPathForFile ? window.artemisDesktop.getPathForFile(file) : ''
