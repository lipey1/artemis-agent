import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@artemis/shared'
import { useEffect, useRef } from 'react'

import type { ArtemisConnection } from '@/global'
import { ArtemisGateway } from '@/artemis'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.artemisDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ArtemisGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ArtemisConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Artemis...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Artemis…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Artemis gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshArtemisConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshArtemisConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startArtemis re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new ArtemisGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshArtemisConfig(),
          ...
          // retry it), not brick boot behind the "Artemis couldn't start"
