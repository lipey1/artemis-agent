...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ArtemisGateway | null
  const gatewayRef = useRef<ArtemisGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.artemisDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ArtemisGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ArtemisGateway | null
      }),
    []
  )
