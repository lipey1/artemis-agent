...
// bug that drives the REAL useGatewayBoot hook + REAL ArtemisGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshArtemisConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { artemisDesktop?: unknown }).artemisDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { artemisDesktop?: unknown }).artemisDesktop
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForArtemis) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startArtemis()'s remote branch awaits waitForArtemis() for 45s before it
    ...
    ;(window as { artemisDesktop?: unknown }).artemisDesktop = desktop
    ...
    // After ~45s waitForArtemis gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Artemis backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Artemis couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Artemis')
    expect($currentCwd.get()).toBe('C:\\Artemis')
  })
})
