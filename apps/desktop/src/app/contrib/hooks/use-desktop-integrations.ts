...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.artemisDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.artemisDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.artemisDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.artemisDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // artemis:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.artemisDesktop?.onDeepLink?.(payload => {
    ...
    void window.artemisDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.artemisDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.artemisDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
