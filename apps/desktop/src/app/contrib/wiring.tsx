...
import { getLatestSessionMessages, triggerCronJob } from '@/artemis'
...
import { useArtemisConfig } from '../session/hooks/use-artemis-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshArtemisConfig, sttEnabled, voiceMaxRecordingSeconds } = useArtemisConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('artemis:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('artemis:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshArtemisConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshArtemisConfig])
