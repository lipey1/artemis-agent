...
const ARTEMIS_PATHS_MIME = 'application/x-artemis-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ARTEMIS_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ARTEMIS_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.artemisDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (artemis --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.artemisDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[artemis-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.artemisDesktop?.terminal?.write(sessionId, `${command}\r`)
