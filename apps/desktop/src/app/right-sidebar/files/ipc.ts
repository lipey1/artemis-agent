...
import type { ArtemisReadDirEntry, ArtemisReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ArtemisReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ArtemisReadDirEntry) {
...
async function filterIgnored(entries: ArtemisReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ArtemisReadDirResult> {
  if (!window.artemisDesktop) {
