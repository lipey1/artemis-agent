...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { artemisDesktop: { readDir: typeof readDir } }).artemisDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ArtemisReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { artemisDesktop: unknown }).artemisDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ArtemisReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ArtemisReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { artemisDesktop: unknown }).artemisDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.artemisDesktop is missing', async () => {
    delete (window as unknown as { artemisDesktop?: unknown }).artemisDesktop
