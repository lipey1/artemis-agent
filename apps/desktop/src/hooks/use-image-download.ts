...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'artemis:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.artemisDesktop?.saveImageFromUrl) {
        if (await window.artemisDesktop.saveImageFromUrl(src)) {
