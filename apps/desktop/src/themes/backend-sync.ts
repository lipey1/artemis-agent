/**
 * Live skin sync from the Artemis backend.
 *
 * The backend resolves the active skin (built-in or `$ARTEMIS_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Artemis
 ...
 */
export function ingestBackendSkin(skin: ArtemisSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ArtemisSkin)
