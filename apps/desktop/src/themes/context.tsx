...
const SKIN_KEY = 'artemis-desktop-theme-v2'
const MODE_KEY = 'artemis-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'artemis-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'artemis-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'artemis-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.artemisTheme = skinName
  root.dataset.artemisMode = rendered
  ...
  window.artemisDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('artemis-boot-background', chromeBg)
    window.localStorage.setItem('artemis-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.artemisThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.artemisDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Artemis authoring/activating a skin from a
