...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<artemis_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active ARTEMIS_HOME."""
    from artemis_constants import get_artemis_home

    flush_dir = get_artemis_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<artemis_home>/pending_messages/``), so a runtime cap rotation no
