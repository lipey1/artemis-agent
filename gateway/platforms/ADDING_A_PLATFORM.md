...
There are two ways to add a platform to the Artemis gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.artemis/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Artemis code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `artemis gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `artemis_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Artemis core.
...
And add it to the `artemis-gateway` composite:

```python
"artemis-gateway": {
    "includes": [..., "artemis-your-platform"]
}
```
...
## 12. Status Display (`artemis_cli/status.py`)
...
## 13. Gateway Setup Wizard (`artemis_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ artemis_cli/ toolsets.py \
  --include="*.py" -l | sort -u
