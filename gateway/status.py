...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_artemis_home() / "gateway-starts.log"
...
def _get_process_hermes_home() -> Path:
    ...
    ``get_artemis_home()`` honors ``_ARTEMIS_HOME_OVERRIDE`` contextvar used for
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ARTEMIS_GATEWAY_LOCK_DIR")
    if override:
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    test also matched ``artemis_cli.main gateway status`` and even unrelated
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `artemis gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``artemis_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ARTEMIS_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a ARTEMIS_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"artemis_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit ARTEMIS_HOME= on the argv. ARTEMIS_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "artemis_home=" in command_lc and f"artemis_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `artemis status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ARTEMIS_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``artemis_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ARTEMIS_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ARTEMIS_HOME dirs).
    """
...
def _get_takeover_marker_path(artemis_home: Optional[Path] = None) -> Path:
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-ARTEMIS_HOME --replace while
    ...
    # ``artemis gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's ARTEMIS_HOME rather than the replacer's.  Same-home callers omit
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target ARTEMIS_HOME's gateway PID record, and the live
    ...
    raw_home = record.get("artemis_home")
