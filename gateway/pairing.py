...
PAIRING_DIR = get_artemis_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``artemis_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from artemis_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ARTEMIS_HOME}/pairing`` while newer code/docs may
    write ``{ARTEMIS_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own ARTEMIS_HOME using the same legacy/consolidated layout rules
    as ``artemis -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current ARTEMIS_HOME.
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            # ``artemis -p <profile> pairing ...`` process does. Merge the
        ...
        if path.exists():
            try:
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> artemis pairing approve ...` as root, "
                    "re-run with `docker exec -u artemis <container> ...` and "
                    "chown the existing file to the artemis user, or restart the "
