...
class PlatformEntry:
    ...
    # status displays (``artemis setup``, ``artemis status``, the dashboard
    ...
    # Env vars this platform needs (for ``artemis setup`` display).
    ...
    # built-ins).  Used by ``artemis gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `artemis`
        # invocation -- including plain `artemis chat`, which never touches any
        ...
        # `artemis setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``artemis setup``/``gateway status``, channel
