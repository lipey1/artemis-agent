...
Defenses 1 and 2 (the ``_ARTEMIS_GATEWAY`` guard on ``artemis gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.artemis.gateway")``,
...
State lives in ``<ARTEMIS_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_artemis_home() / "gateway" / "restart_loop.json"
