...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from artemis_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (artemis_cli/web_server.py) so
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_artemis_home() / "sessions")
    ...
    # gateway behaves exactly as before — single ARTEMIS_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``artemis config set gateway.multiplex_profiles true``.
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through artemis_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `artemis config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `artemis config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``artemis config set gateway.streaming.*``
