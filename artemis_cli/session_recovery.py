"""Offline, non-destructive recovery for a damaged Artemis session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``artemis_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``artemis_state._backup_db_file``.
    """
    from artemis_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="artemis-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Artemis process using this profile and retry."
            )
