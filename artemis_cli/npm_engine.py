...
Scope of the repair is deliberately narrow. Artemis only upgrades an npm that
lives inside its **own** managed Node tree (``$ARTEMIS_HOME/node``), installing
...
and their other projects; Artemis never modifies those. When the failing npm is
one of those foreign installs, Artemis instead provisions its own managed Node
...
from artemis_constants import (
    bootstrap_artemis_managed_node,
    get_artemis_home,
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Artemis-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$ARTEMIS_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$ARTEMIS_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Artemis-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
        # A temp cwd keeps the checkout's .npmrc (engine-strict, min-release-age)
        # from applying to the upgrade itself.
        with tempfile.TemporaryDirectory(prefix="artemis-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Artemis could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Artemis-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$ARTEMIS_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Artemis-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_artemis_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Artemis-managed install, or
    ...
    if prefix is not None:
        # Artemis owns this npm — upgrade it in place. Only an npm-range
