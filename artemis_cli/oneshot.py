...
configured for "cli" in `artemis tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (ARTEMIS_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `artemis chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"artemis -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "artemis -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"artemis -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "artemis -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "artemis -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to ARTEMIS_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("ARTEMIS_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "artemis -z: --provider requires --model (or ARTEMIS_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"artemis -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("artemis -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``artemis -z`` / oneshot mode.

    Oneshot bypasses ``ArtemisCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when artemis is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from artemis_cli.config import load_config
    from artemis_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("ARTEMIS_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from artemis_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("ARTEMIS_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # ArtemisCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from artemis_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                ARTEMIS_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via ARTEMIS_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via ARTEMIS_YOLO_MODE=1
