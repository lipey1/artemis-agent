...
Profile-aware location: ``$ARTEMIS_HOME/logs/dashboard-auth.log``.
...
from ``artemis_constants`` or other artemis_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ARTEMIS_HOME/logs/dashboard-auth.log``.

    Uses ``artemis_constants.get_artemis_home()`` (a leaf module — no import
    ...
    return get_artemis_home() / "logs" / "dashboard-auth.log"
