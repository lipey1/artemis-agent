...
from artemis_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /artemis``, the
    ``login_url`` is prefixed (``/artemis/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from artemis_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["artemis.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``artemis_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``artemis_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from artemis_cli.dashboard_auth.cookies import clear_session_cookies
        from artemis_cli.dashboard_auth.prefix import prefix_from_request
