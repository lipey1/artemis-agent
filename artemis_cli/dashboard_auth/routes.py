...
from artemis_cli.dashboard_auth.base import (
    InvalidCodeError,
    InvalidCredentialsError,
    ProviderError,
)
from artemis_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from artemis_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``ARTEMIS_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /artemis`` (Mission Control deploys) — we
    ...
    from artemis_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``artemis_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from artemis_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("artemis_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from artemis_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from artemis_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from artemis_cli.dashboard_auth import list_session_providers
    from artemis_cli.dashboard_auth.base import RefreshExpiredError
