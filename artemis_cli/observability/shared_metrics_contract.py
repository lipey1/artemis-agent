"""Bounded product contract for the first Artemis shared-metrics slice."""
...
SCHEMA_KEY = "artemis.metrics.schema_version"
SCHEMA_VERSION = "artemis.metrics.event.v2"
MODEL_CALL_SCOPE = "artemis.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "artemis.task_run"
TOOL_CALL_SCOPE = "artemis.tool_call"
CLIENT_ACTIVE_MARK = "artemis.client.active"
TOOL_APPROVAL_MARK = "artemis.tool_approval"
SKILL_LIFECYCLE_MARK = "artemis.skill.lifecycle"
SKILL_LOAD_MARK = "artemis.skill.load"
SUBSCRIBER_NAME = "artemis.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "artemis.client.active"
LEGACY_MODEL_CALL_METRIC = "artemis.model_call.count"
MODEL_ROUTE_METRIC = "artemis.model_route.count"
TASK_STARTED_METRIC = "artemis.task_run.started"
TASK_FINISHED_METRIC = "artemis.task_run.finished"
TOOL_CALL_METRIC = "artemis.tool_call.count"
TOOL_APPROVAL_METRIC = "artemis.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "artemis.skill.lifecycle.count"
SKILL_LOAD_METRIC = "artemis.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Artemis installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(artemis_version or "").strip()
    ...
    return {
        ...
        "artemis_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("artemis_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Artemis logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("artemis.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from artemis_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Artemis terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Artemis registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "artemis-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Artemis tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Artemis."""
