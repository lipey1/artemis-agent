"""Dashboard process-hygiene helpers — extracted from ``artemis_cli/main.py``.
...
``_detect_concurrent_artemis_instances``) are lifted verbatim. References to
helpers that STAY in ``artemis_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``artemis_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``artemis_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``artemis_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``artemis dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``artemis update`` replaces files on disk, the running
    ...
    ``artemis-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Artemis Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``artemis serve`` as
    ...
    environment variable ``ARTEMIS_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`artemis serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "artemis.*dashboard"`) keeps us consistent
            # with `artemis_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``artemis dashboard`` / ``artemis serve`` processes.

    Called at the end of ``artemis update`` (default ``reason``) and also
    from ``artemis dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``artemis update`` path), a detected
    ``artemis-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``artemis-serve.service``) has its owning unit
    ...
    # When the Artemis Desktop Electron app spawns this dashboard as a
    # backend child, it sets ARTEMIS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ARTEMIS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `artemis update` path) — `--stop` must
    ...
    #    remote backend (artemis serve) under Restart=on-failure never comes
...
def _detect_concurrent_artemis_instances(
    ...
    The Artemis Desktop Electron app spawns ``artemis.EXE`` as a backend child,
    so during ``artemis update`` the user-invoked process and the desktop's
    ...
    shims (``artemis.exe`` / ``artemis-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``artemis update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``artemis.exe``) is a separate process from the Python
    ...
    for shim in _m()._artemis_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated artemis.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``artemis update``
    ...
    #      artemis.exe sitting *under* a non-Artemis parent (e.g. a Artemis
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        artemis serve --host 127.0.0.1 --port 0
        artemis serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "artemis" not in cmd and "artemis_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (ARTEMIS_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("ARTEMIS_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``artemis serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``artemis serve`` on a Mac
...
def _artemis_home_dir() -> Path:
    """Resolved Artemis home (ARTEMIS_HOME override or ~/.artemis)."""
    override = os.environ.get("ARTEMIS_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "artemisPath", "artemisHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated ARTEMIS_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{artemis_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _artemis_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local artemis serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``artemis serve`` backends with no parent.
    ...
    running under ARTEMIS_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never ARTEMIS_DESKTOP_CHILD_PID entries
