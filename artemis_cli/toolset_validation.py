...
the tool registry or launching Artemis (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``artemis-cli`` to the non-existent ``artemis``. ``resolve_toolset('artemis')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``artemis-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``artemis`` instead of ``artemis-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `artemis tools` to reconfigure."
        )
    return warnings
