"""
Status command for artemis CLI.

Shows the status of all Artemis Agent components.
"""
...
from artemis_cli.auth import AuthError, resolve_provider
from artemis_cli.colors import Colors, color
from artemis_cli.config import get_env_path, get_env_value, get_artemis_home, load_config
from artemis_cli.models import provider_label
from artemis_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from artemis_cli.nous_subscription import get_nous_subscription_features
from artemis_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``artemis config``'s
...
def _estop_status_line():
    """One-line pause banner for `artemis status`, or None when not paused.

    Cheap: a single stat on $ARTEMIS_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `artemis resume` to lift)"


def show_status(args):
    """Show status of all Artemis Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Artemis Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `artemis status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: artemis portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: artemis model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: artemis auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: artemis auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: artemis model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from artemis_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'artemis-agent[vercel]')"
    ...
    try:
        from artemis_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   artemis gateway")
    ...
    jobs_file = get_artemis_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_artemis_home() / "sessions" / "sessions.json"
    ...
    try:
        from artemis_cli.active_sessions import (
    ...
    print(color("  Run 'artemis doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'artemis setup' to configure", Colors.DIM))
    print()
