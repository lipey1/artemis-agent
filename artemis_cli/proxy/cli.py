"""CLI handlers for the ``artemis proxy`` subcommand."""
...
from artemis_cli.proxy.adapters import ADAPTERS, get_adapter
from artemis_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "artemis proxy requires aiohttp. Run `artemis setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"artemis auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Artemis proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Artemis proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: artemis proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``artemis proxy <subcommand>``."""
    ...
    print(
        "artemis proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  artemis proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  artemis proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  artemis proxy providers\n"
