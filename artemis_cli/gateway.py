"""
Gateway subcommand for artemis CLI.

Handles: artemis gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_artemis_home is imported lazily at call sites to avoid ImportError
# when artemis_constants is cached from a pre-update version during `artemis update`.
...
from artemis_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "artemis CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``artemis gateway status`` from falsely counting the ``artemis`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``artemis gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_artemis_home().resolve())
    # Forward slashes on both sides of the ARTEMIS_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"artemis_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ARTEMIS_HOME
        ...
        # ARTEMIS_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "artemis_home=" in command_lc
            and f"artemis_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``artemis update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Artemis profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Artemis profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from artemis_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "artemis_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m artemis_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``artemis update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Artemis's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ARTEMIS_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``artemis update`` users
    # with no running gateway until they re-invoke ``artemis gateway``
    ...
    from artemis_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (ARTEMIS_HOME,
    ...
    if sys.platform == "win32":
        try:
            from artemis_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ARTEMIS_HOME) the windowless
        # base interpreter needs to import artemis_cli.  Empty on POSIX, where
...
def _artemis_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ARTEMIS_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("ARTEMIS_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_artemis_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ARTEMIS_HOME``.

    Under ``sudo``, ``ARTEMIS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_artemis_home` falls back to ``/root/.artemis`` — the wrong
    profile. The unit file pins ``ARTEMIS_HOME`` for the actual gateway
    ...
    unit_home = (_artemis_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ARTEMIS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ARTEMIS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ARTEMIS_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}artemis gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}artemis gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from artemis_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: artemis gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `artemis gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``artemis gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from artemis_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (artemis_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ARTEMIS_HOME-scoped).
...
from artemis_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Artemis Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``artemis gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ARTEMIS_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ARTEMIS_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "artemis-gateway"
SERVICE_DESCRIPTION = "Artemis Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ARTEMIS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ARTEMIS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from artemis_constants import get_default_artemis_root

    home = get_artemis_home().resolve()
    default = get_default_artemis_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ARTEMIS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(artemis_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ARTEMIS_HOME is a named profile.

    For ``~/.artemis/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        artemis_home: Optional explicit ARTEMIS_HOME path. Defaults to the current
            ``get_artemis_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Artemis root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_artemis_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from artemis_constants import get_default_artemis_root

    home = Path(artemis_home or str(get_artemis_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_artemis_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(artemis_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".artemis"
    try:
        Path(artemis_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(artemis_home, default_root=target_root)
    except ValueError:
        return _profile_arg(artemis_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this ARTEMIS_HOME.

    Default ``~/.artemis`` returns ``artemis-gateway`` (backward compatible).
    Profile ``~/.artemis/profiles/coder`` returns ``artemis-gateway-coder``.
    Any other ARTEMIS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Artemis installs that predate the
# artemis-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (artemis-gateway-*.service) and unrelated third-party
# "artemis" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("artemis.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "artemis_cli.main gateway",
    "artemis_cli/main.py gateway",
    "gateway/run.py",
    " artemis gateway ",
    "/artemis gateway ",
)
...
def _find_legacy_artemis_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Artemis gateway units.

    Detects unit files installed by older Artemis versions that used a
    different service name (e.g. ``artemis.service`` before the rename to
    ``artemis-gateway.service``). When both a legacy unit and the current
    ``artemis-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``artemis-gateway-coder.service`` and unrelated third-party
      ``artemis-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``artemis.service`` running an unrelated
...
def has_legacy_artemis_units() -> bool:
    """Return True when any legacy Artemis gateway unit files exist."""
    return bool(_find_legacy_artemis_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Artemis gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_artemis_units()
    if not legacy:
        return
    print_warning("Legacy Artemis gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current artemis-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    artemis gateway migrate-legacy")


def remove_legacy_artemis_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Artemis gateway unit files.

    Iterates over whatever ``_find_legacy_artemis_units()`` returns — which is
    ...
    legacy = _find_legacy_artemis_units()
    if not legacy:
        print("No legacy Artemis gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Artemis gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: artemis gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo artemis gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    artemis gateway uninstall")
    print_info("    sudo artemis gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: artemis gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Artemis often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.artemis`` → ``ai.artemis.gateway.plist`` (backward compatible).
    Profile ``~/.artemis/profiles/coder`` → ``ai.artemis.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.artemis.gateway-{suffix}" if suffix else "ai.artemis.gateway"
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # artemis_constants cached from before this symbol existed. See
            # _reload_artemis_constants() in artemis_cli/managed_uv.py.
            from artemis_cli.managed_uv import _reload_artemis_constants

            venv_python_path = _reload_artemis_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.artemis/artemis-agent  -> /home/alice/.artemis/artemis-agent
      /opt/artemis                 -> /opt/artemis  (kept as-is)
...
def _artemis_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ARTEMIS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_artemis_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.artemis                    → /home/alice/.artemis
      /root/.artemis/profiles/coder     → /home/alice/.artemis/profiles/coder
      /opt/custom-artemis               → /opt/custom-artemis  (kept as-is)
    """
    current_artemis_raw = os.environ.get("ARTEMIS_HOME", "").strip()
    current_artemis = (
        Path(current_artemis_raw).expanduser()
        if current_artemis_raw
        else get_artemis_home()
    )
    ...
    # different ARTEMIS_HOME into the generated service unit.
    current_default = Path.home() / ".artemis"
    target_default = Path(target_home_dir) / ".artemis"

    # Default ~/.artemis → remap to target user's default
    if current_artemis == current_default:
        return str(target_default)

    # Profile or subdir of ~/.artemis → preserve the relative structure
    try:
        relative = current_artemis.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.artemis) — keep as-is
        return str(current_artemis)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    artemis_node = artemis_home / "node" / "bin"
    if _is_dir(artemis_node):
        candidates.append(str(artemis_node))
    artemis_nm = artemis_home / "node_modules" / ".bin"
    if _is_dir(artemis_nm):
        candidates.append(str(artemis_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m artemis_cli.main``, so module
    ...
    dir, or a clone that ``artemis update`` later relocates/removes — the path
    ...
    ``ARTEMIS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ARTEMIS_HOME
...
def _systemd_watchdog_seconds(artemis_home: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = None
    reset_home_override = None
    if artemis_home is not None:
        from artemis_constants import (
            reset_artemis_home_override,
            set_artemis_home_override,
        )

        override_token = set_artemis_home_override(artemis_home)
        reset_home_override = reset_artemis_home_override
    try:
...
def _systemd_watchdog_service_fields(
    artemis_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(artemis_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], artemis_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Artemis-managed Node under ``$ARTEMIS_HOME/node`` goes first when it
    ...
    *artemis_root* is the Artemis home the unit will run against. System units
    ...
    """
    from artemis_constants import iter_artemis_node_dirs

    for directory in iter_artemis_node_dirs(artemis_root):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Artemis home is known — probing here would stat the calling
    ...
    if system:
        ...
        artemis_home = _artemis_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            artemis_home
        )
        profile_arg = _profile_arg_for_target_user(artemis_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ARTEMIS_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(artemis_home) if artemis_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped artemis_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(artemis_home) if artemis_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m artemis_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="ARTEMIS_HOME={artemis_home}"
Restart=always
RestartSec=5
...
"""
    ...
    artemis_home = str(get_artemis_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        artemis_home
    )
    profile_arg = _profile_arg(artemis_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m artemis_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ARTEMIS_HOME={artemis_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned ARTEMIS_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo artemis gateway … --system``, ARTEMIS_HOME is often stripped
    # and falls back to ``/root/.artemis``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ARTEMIS_HOME comparisons
    ...
    _sync_artemis_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ARTEMIS_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ARTEMIS_HOME
    ...
    exported a throwaway ``ARTEMIS_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``artemis gateway restart``
    with ``ARTEMIS_HOME=/tmp/artemis-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ARTEMIS_HOME=..."`` lines and launchd
    ``<key>ARTEMIS_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ARTEMIS_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ARTEMIS_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ARTEMIS_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ARTEMIS_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ARTEMIS_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ARTEMIS_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only ARTEMIS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ARTEMIS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../artemis_test`` path into
    # ``Environment="ARTEMIS_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/artemis_test"' in new_unit
        or "/artemis_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # ARTEMIS_HOME into the unit (manual E2E homes like /tmp/artemis-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Artemis install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo artemis gateway uninstall --system")
    print_info("         artemis gateway install")
    print_info("         artemis gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("ARTEMIS_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("ARTEMIS_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (artemis.service from pre-rename installs)
    # before installing the new artemis-gateway.service. If both remain, they
    ...
    if has_legacy_artemis_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_artemis_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin ARTEMIS_HOME; adopt it before any
    ...
    # ``sudo artemis gateway install --system --force`` would bake /root/.artemis
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}artemis gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}artemis gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}artemis gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # ARTEMIS_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `artemis gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `artemis gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}artemis gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}artemis gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}artemis gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}artemis gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.artemis.gateway-{suffix}" if suffix else "ai.artemis.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``artemis gateway start/stop/status`` call).
...
#      background process (the `nohup artemis gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_artemis_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_artemis_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m artemis_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ARTEMIS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "artemis_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup artemis gateway run --replace`
    ...
    """
    from artemis_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from artemis_constants import display_artemis_home as _dhh
    ...
    print(
        f"  Try manually: nohup artemis gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>artemis_cli.main</string>",
    ]
    if profile_arg:
    ...
    # without this block every plist rewrite (e.g. `artemis gateway start`)
    ...
    nofile_block = ""
    try:
        from artemis_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>ARTEMIS_HOME</key>
        <string>{artemis_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.artemis/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See artemis-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_artemis_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Artemis install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  artemis gateway status             # Check status")
    from artemis_constants import display_artemis_home as _dhh
...
def launchd_stop():
    ...
    # `artemis gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Artemis PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Artemis PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Artemis install")
    else:
        print("⚠ Service definition is stale relative to the current Artemis install")
        print("  Run: artemis gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: artemis gateway stop")
            else:
    ...
    if deep:
        log_file = get_artemis_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ARTEMIS_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``artemis gateway run`` is always a misconfiguration, so we
    ...
    try:
        from artemis_constants import get_default_artemis_root
        default_root = get_default_artemis_root()
    ...
    try:
        ...
        from artemis_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``artemis gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``artemis gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ARTEMIS_HOME roots.
    """
    ...
    print("  Use 'artemis gateway restart' to replace it,")
    print("  or 'artemis gateway stop' first.")
    print("  Or use 'artemis gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ARTEMIS_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Artemis gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'artemis' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/artemis/docker/entrypoint.sh before the Artemis command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ARTEMIS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ARTEMIS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `artemis gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ARTEMIS_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `artemis gateway restart` which already
    ...
    # `artemis gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Artemis Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # ARTEMIS_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ARTEMIS_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``ARTEMIS_GATEWAY_MAX_STARTS`` /
    # ``ARTEMIS_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("ARTEMIS_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ARTEMIS_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``artemis gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. artemis) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Artemis delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Artemis connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: artemis pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Artemis will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``artemis setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run artemis under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.artemis/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_artemis_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from artemis_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from artemis_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'artemis pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: artemis pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'artemis gateway setup'"
                    )
            continue
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from artemis_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Artemis will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Artemis will store the returned account_id/token in ~/.artemis/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `artemis gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_artemis_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `artemis pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `artemis pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "ArtemisAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``artemis_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from artemis_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.artemis/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.artemis/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_artemis_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: artemis gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: artemis gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: artemis gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo artemis gateway install --system"
                        )
                    print_info("  Or run in foreground:  artemis gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: artemis gateway run")
                print_info(
                    "  For persistence:   tmux new -s artemis 'artemis gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: artemis gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup artemis gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'artemis gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`artemis_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from artemis_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ARTEMIS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``artemis gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``artemis gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``artemis gateway run`` are unaffected.
      2. ``ARTEMIS_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``artemis gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ARTEMIS_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ARTEMIS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ARTEMIS_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ARTEMIS_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ARTEMIS_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ARTEMIS_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-artemis service's
    # pattern in docker/s6-rc.d/main-artemis/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: artemis gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: artemis gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s artemis 'artemis gateway run'"
                )
                print()
            ...
            print()
            print(
                "  artemis gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s artemis 'artemis gateway run'         # persistent via tmux"
            )
            print(
                "  nohup artemis gateway run > ~/.artemis/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from artemis_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  artemis profile create <name>     # creates the s6 service slot")
                print("  artemis -p <name> gateway start   # bring it up via s6")
                print("  artemis status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: artemis gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: artemis gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: artemis gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  artemis profile delete <name>     # tears down the s6 service slot")
                print("  artemis -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `artemis -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: artemis gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ARTEMIS_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `artemis gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_ARTEMIS_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `artemis gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: artemis gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  artemis gateway install")
                else:
                    print("To install as a service:")
                    print("  artemis gateway install")
                    print("  sudo artemis gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  artemis gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup artemis gateway run > ~/.artemis/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s artemis 'artemis gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  artemis gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  artemis gateway install  # Install as user service")
                    print(
                        "  sudo artemis gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Artemis gateway unit files from
        # pre-rename installs (e.g. artemis.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_artemis_units(interactive=not yes, dry_run=dry_run)
