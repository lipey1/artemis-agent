"""Artemis update pipeline — extracted from ``artemis_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``artemis_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``artemis_cli.main`` reference — so existing call sites and test monkeypatches
that target ``artemis_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``artemis_cli.main``.
...
Imports are one-way: ``artemis_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``artemis_cli.main`` reference.

    Lets callers keep patching ``artemis_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``artemis_cli.main`` -> ``artemis_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``artemis update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``artemis update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``artemis_cli.config_defaults``,
    ``artemis_cli.config``, and ``artemis_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.
    """
    import importlib

    importlib.invalidate_caches()
    for mod_name in ("artemis_cli.config_defaults", "artemis_cli.config", "artemis_cli.config_migrations"):
...
def _run_config_check_fresh() -> tuple:
    ...
    from artemis_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from artemis_cli.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that Artemis must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "artemis_cli/main.py",
    "artemis_cli/config.py",
    "artemis_cli/__init__.py",
    "artemis_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "artemis_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``artemis`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="artemis-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``artemis update`` can be driven by a
    ...
    """
    from artemis_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from artemis_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('artemis_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``artemis update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `artemis update`.
    ...
    print("  Preview now:  artemis curator run --dry-run")
    print("  Pause it:     artemis curator pause")
    print(
        "  Docs:         https://artemis-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `artemis update`.
    ...
    try:
        ...
        return
    db_path = get_artemis_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `artemis sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    artemis sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  artemis sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``artemis update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``artemis update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: artemis curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    artemis dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``artemis_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.artemis-update-staging"
    backup = f"{dst}.artemis-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `artemis update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``artemis_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("ARTEMIS_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== artemis-update completed {action_id} ===")


def _update_via_zip(args):
    """Update Artemis Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `artemis update "
            f"--branch {branch}`, or update against main with `artemis update`."
        )
        _m().sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="artemis-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"artemis-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for artemis-agent itself
        ...
        # GitHub ZIPs extract to artemis-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"artemis-agent-{branch}")
        ...
        print(
            "  Re-run `artemis update` to retry; if the agent won't start, "
            "reinstall from https://artemis-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from artemis_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `artemis update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: artemis skills list-modified  "
    ...
    try:
        from artemis_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from artemis_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_artemis_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_artemis_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "artemis-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Artemis behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes artemis completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Artemis couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Artemis couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Artemis behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Artemis couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Artemis couldn't drop "
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_artemis_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_artemis_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Artemis repository.")
        print("  This means you may miss updates from NousResearch/artemis-agent.")
        print()
        try:
            response = (
...
def _invalidate_update_cache():
    ...
    ``artemis update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from artemis_constants import get_default_artemis_root

    default_home = get_default_artemis_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "artemis.exe"
    lines = ["✗ Another artemis.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Artemis Desktop, exit any open `artemis` REPLs, and")
    lines.append("  stop the gateway (`artemis gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `artemis update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``artemis update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `artemis update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `artemis update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Artemis' editable-install extras or
    ...
    try:
        from artemis_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from artemis_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ARTEMIS_HOME/bin/uv``, but on Termux the official
    ...
    """
    from artemis_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `artemis update` is exactly the step
...
def _npm_lockfile_changed(artemis_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `artemis
    ...
    try:
        ...
        cache_file = artemis_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(artemis_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from artemis_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `artemis update`.")
            failed = ["repo root"]
            if any(
    ...
    # Artemis profile using this checkout. Keep one per-checkout cache under the
    # shared Artemis root rather than rerunning npm once per named profile.
    shared_artemis_root = get_default_artemis_root()
    if not _m()._npm_lockfile_changed(shared_artemis_root):
    ...
    # need desktop during `artemis update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `artemis update`.")
        return list(labels)
    ...
    nixos_env = with_artemis_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `artemis update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_artemis_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.artemis/logs/update.log`` only, never the terminal.

    During ``artemis update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from artemis_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``artemis update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from artemis_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``artemis update`` without requiring a reinstall.  The
    ...
    ``artemis`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/artemis.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v artemis'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/artemis, code at /usr/local/lib/artemis-agent).
    fhs_link = Path("/usr/local/bin/artemis")
    ...
    path_comment = (
        "# Artemis Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``artemis-acp`` launcher next to the ``artemis`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``artemis update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``artemis-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Artemis as not installed even when it is.

    The shim simply delegates to the sibling ``artemis`` launcher with the
    ...
    ``artemis-acp.exe`` already resolves) and wherever a ``artemis-acp`` is
    already present next to the ``artemis`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        artemis_cmd = bin_dir / "artemis"
        acp_cmd = bin_dir / "artemis-acp"
        try:
            if not (artemis_cmd.is_file() or artemis_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ARTEMIS_HOME under
      ``backups/`` (restorable via ``artemis import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from artemis_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from artemis_constants import
        # get_artemis_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from artemis_cli.config import get_artemis_home as _get_home
    ...
    try:
        from artemis_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from artemis_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_artemis_home so the user sees ~/.artemis/...
    try:
        from artemis_constants import get_artemis_home, display_artemis_home

        home = get_artemis_home()
        try:
            display_path = f"{display_artemis_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  artemis import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``artemis update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``artemis update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run artemis from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.artemis-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".artemis-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The artemis.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m artemis_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``artemis
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m artemis_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "artemis_cli.main" in cmdline_low:
        ...
        # the `-m artemis_cli.main gateway run` argv, so autostarted gateways
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Artemis processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Artemis Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Artemis desktop app / other Artemis terminals, then re-run:"
    )
    lines.append("    artemis update")
    lines.append("  (or use `artemis update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``artemis update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from artemis_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning artemis-setup, and the update-in-progress marker
    ...
    the update with "Artemis is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Artemis backend (``artemis_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.artemis-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "artemis_cli.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.artemis-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    artemis.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from artemis_cli.gateway import (
    ...
    print("→ Stopping Windows gateway process(es) before updating Artemis...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m artemis_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: artemis gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``artemis gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from artemis_cli import gateway_windows
        from artemis_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``artemis-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("artemis-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``artemis update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``artemis-setup.exe``) executes
    ``$ARTEMIS_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_artemis_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``artemis update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``artemis update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Artemis Agent...")
    print()

    # On Windows, abort early if another artemis.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_artemis_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `artemis serve` backend) will keep .pyd
    ...
    # the artemis.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .artemis-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Artemis is still
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://artemis-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Artemis windows/gateways and re-run: artemis update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Artemis gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in artemis_cli/config.py bricked
        # every user who ran ``artemis update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``artemis
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``artemis update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_artemis_home added to artemis_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``artemis`` launch finishes the install
        ...
        # symbols in artemis_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `artemis` again "
        ...
        if not import_ok:
            ...
            print("    Run `artemis update` again — if it persists, reinstall:")
            print("    https://artemis-agent.nousresearch.com")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``artemis desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``artemis desktop`` shouldn't be forced into a full
        # Electron build by ``artemis update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `artemis desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "artemis_cli.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Artemis-managed Node on PATH:
                # when `artemis update` runs inside the desktop updater chain
                # (Desktop → artemis-setup → artemis update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from artemis_constants import with_artemis_node_path

                _build_env = with_artemis_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `artemis desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_artemis_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.artemis/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ARTEMIS_HOME so
        # it is not affected by sync_skills()'s module-level ARTEMIS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ARTEMIS_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from artemis_cli.profiles import backfill_profile_envs
        ...
        # ``artemis update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``artemis_cli.config`` and
        # ``artemis_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from artemis_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'artemis config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'artemis config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: artemis config migrate")
            else:
                print()
                print("Skipped. Run 'artemis config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from artemis_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `artemis curator status`. Self-stamps after printing so it
        ...
        # Self-heal the artemis-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Artemis on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("artemis-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``artemis update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `artemis update`
                # must stay fast; `artemis computer-use install --upgrade`
        ...
        # When running as ``artemis update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_artemis_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... artemis-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... artemis-gateway*)
            ...
            try:
                from artemis_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all artemis-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: artemis gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `artemis update` restart it automatically, allow\n"
                        ...
                        # path in `artemis gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: artemis gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: artemis -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Artemis gateway unit files are still installed.
        # When both artemis.service (from a pre-rename install) and the
        # current artemis-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `artemis update` surfaces the issue until the user migrates.
        try:
            from artemis_cli.gateway import (
                has_legacy_artemis_units,
                _find_legacy_artemis_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_artemis_units():
                print()
                print("⚠ Legacy Artemis gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_artemis_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (artemis.service) fight the current")
                print("  artemis-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    artemis gateway migrate-legacy")
                print()
        ...
        print("  artemis model              # Select provider and model")
