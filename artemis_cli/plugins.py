...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with artemis-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.artemis/plugins/<name>/``
3. **Project plugins** – ``./.artemis/plugins/<name>/`` (opt-in via
   ``ARTEMIS_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``artemis_agent.plugins``
   entry-point group.
...
from artemis_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``ARTEMIS_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ARTEMIS_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ARTEMIS_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.artemis/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ARTEMIS_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ARTEMIS_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Artemis loggers.
    """
    ...
    logger.debug(
        "ARTEMIS_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Artemis' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by artemis_cli.kanban_db when a task
    ...
    # separate `artemis -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `artemis kanban dispatch`),
...
ENTRY_POINTS_GROUP = "artemis_agent.plugins"

_NS_PARENT = "artemis_plugins"
...
class PluginManifest:
    ...
    #              in ~/.artemis/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``artemis plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Artemis profile name (e.g. ``"default"``).

        Derived from ``ARTEMIS_HOME`` via
        :func:`artemis_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.artemis/profiles/<name>``, or ``"custom"`` when
        ``ARTEMIS_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Artemis core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``artemis honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``artemis <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`artemis_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_artemis_dotenv()`` startup — after
        ``~/.artemis/.env`` loads, before Artemis reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Artemis' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``artemis model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: artemis_cli.main imports plugins indirectly
        from artemis_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.artemis/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("ARTEMIS_SAFE_MODE"):
            logger.info("ARTEMIS_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with artemis and must
            ...
            # all ~20 of them added several seconds to every `artemis`
            # invocation — including plain `artemis chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Artemis ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `artemis plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.artemis/plugins/)
        user_dir = get_artemis_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.artemis/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("ARTEMIS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".artemis" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ARTEMIS_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("ARTEMIS_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from artemis_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from artemis_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``artemis plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`artemis plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `artemis plugins list`.
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from artemis_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_artemis_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``artemis_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``artemis_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def _plugin_toolset_keys_cache_path():
    from artemis_constants import get_artemis_home
    return get_artemis_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from artemis_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="artemis-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``artemis tools`` TUI so plugin-provided toolsets appear
