"""``artemis verify`` subcommand parser.

Follows the pattern of ``artemis_cli/subcommands/doctor.py``: parser built
...
# avoid paying an extra module import on every `artemis` invocation.
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "(or load the saved manifest at .artemis/environment.json), then "
    ...
    verify_parser.add_argument(
        "--save",
        action="store_true",
        help="Save the recipe as .artemis/environment.json in the project",
    )
    verify_parser.add_argument(
        "--skip-start",
        action="store_true",
