"""``artemis debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Artemis Agent. Use 'artemis debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    artemis debug share              Upload debug report (asks for confirmation)
    artemis debug share --yes        Skip confirmation (for scripts/CI)
    artemis debug share --lines 500  Include more log lines
    artemis debug share --expire 30  Keep paste for 30 days
    artemis debug share --local      Print report locally (no upload)
    artemis debug share --no-redact  Disable upload-time secret redaction
    artemis debug share --nous       Upload to Nous-internal storage (private)
    artemis debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'artemis debug share'",
    )
