"""``artemis dashboard`` / ``artemis serve`` subcommand parsers.
...
``artemis_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `artemis dashboard` / `artemis serve` cmdlines and SIGTERM them directly —
    # the same path `artemis update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Artemis web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Artemis web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Artemis Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Artemis desktop app shells (<= 0.15.x) spawn the
    # backend as `artemis dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Artemis couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Artemis Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Artemis backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Artemis backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports ARTEMIS_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `artemis dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.artemis/.env.
    # Nested subparser so bare `artemis dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ARTEMIS_DASHBOARD_OAUTH_CLIENT_ID into ~/.artemis/.env, and prints "
            "how to engage the login gate. Requires being logged in (artemis setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://artemis.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via ARTEMIS_DASHBOARD_PORTAL_URL. Mainly for "
