"""``artemis sync`` subcommand parser — Skill Sync.

Cloned from ``artemis_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    artemis sync status                 show gate/opt-in/head state
    artemis sync pull                   pull and materialize opted-in skills
    artemis sync push                   push opted-in skills
    artemis sync now                    reconcile: pull then push
    artemis sync enable <skill>         opt a skill into sync
    artemis sync disable <skill>        opt a skill out of sync
    artemis sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    artemis sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  artemis sync status            what is synced, and from where\n"
            "  artemis sync enable my-skill   include a skill in your sync\n"
            "  artemis sync now               pull, then push\n"
            "  artemis sync propose my-skill  share a skill with your team\n"
        ),
