...
Pure display functions with no ArtemisCLI state dependency.
"""
import json
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `artemis ... > file`) raises
...
ARTEMIS_AGENT_LOGO = """[bold #5EC8E3] █████╗ ██████╗ ████████╗███████╗███╗   ███╗██╗███████╗[/]
[bold #5EC8E3]██╔══██╗██╔══██╗╚══██╔══╝██╔════╝████╗ ████║██║██╔════╝[/]
[#3BA8C9]███████║██████╔╝   ██║   █████╗  ██╔████╔██║██║███████╗[/]
[#3BA8C9]██╔══██║██╔══██╗   ██║   ██╔══╝  ██║╚██╔╝██║██║╚════██║[/]
[#2A7A94]██║  ██║██║  ██║   ██║   ███████╗██║ ╚═╝ ██║██║███████║[/]
...
# Legacy Artemis logo kept for skins that still reference it
    ...
    try:
        if cache_file.exists():
            ...
            if (
                ...
                and cached.get("src") == "artemis-desktop"
            ):
                return cached.get("behind")
    except Exception:
        pass

    def _parse_ver(value: str):
        parts = []
        for chunk in str(value).lstrip("v").split("."):
            num = ""
            for ch in chunk:
                if ch.isdigit():
                    num += ch
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Artemis git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ARTEMIS_HOME/artemis-agent/`` (or legacy ``artemis-agent/``)
    may be a stale copy carried over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
        artemis_home = get_artemis_home()
        preferred = artemis_home / "artemis-agent"
        legacy = artemis_home / "artemis-agent"
        if (preferred / ".git").exists():
            repo_dir = preferred
        elif (legacy / ".git").exists():
            repo_dir = legacy
        else:
            repo_dir = preferred
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``artemis_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from artemis_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Artemis checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/artemis-agent repo (forks don't get a link).
    """
...
def _format_update_notice(behind: int) -> str:
    ...
    from artemis_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built artemis; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_artemis_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_artemis_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ARTEMIS_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ARTEMIS_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Artemis[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or artemis setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Artemis[/]")

    if os.getenv("ARTEMIS_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from artemis_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from artemis_cli.codex_runtime_switch import get_current_runtime
        from artemis_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ARTEMIS_AGENT_LOGO
        console.print(_logo)
        console.print()
