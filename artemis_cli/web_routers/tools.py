...
late-binding seam in :mod:`artemis_cli.web_deps` (``late`` for callables,
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from artemis_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``artemis tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `artemis tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``artemis tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.artemis/.env`` via ``save_env_value`` —
    the same store ``artemis tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``artemis tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``artemis -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ARTEMIS_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``artemis computer-use permissions grant`` as a background action.
