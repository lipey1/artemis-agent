"""``artemis doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from artemis_cli.doctor import (
    _section,
    check_fail,
    check_info,
    check_ok,
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from artemis_cli.doctor import ARTEMIS_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (ARTEMIS_HOME / "node" / "bin",
                          ARTEMIS_HOME / "node",
                          ARTEMIS_HOME / "node_modules" / ".bin"):
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``artemis mcp test``.
    """
