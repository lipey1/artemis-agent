...
Pure logic: walks a Artemis config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `artemis doctor` and a future `artemis migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Artemis config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Artemis config and return retirement issues.
...
def apply_migration(
    ...
    from artemis_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `artemis migrate xai` reports nothing to migrate
