"""Cross-process mutual exclusion for in-flight Artemis updates.

Three different surfaces can start an update of the same install tree:

* ``artemis update`` from a terminal,
* the dashboard's Update button (``POST /api/artemis/update`` →
  ``_spawn_artemis_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``artemis-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``artemis update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ARTEMIS_HOME>/.artemis-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``artemis update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Artemis is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``artemis-setup`` binary under ``~/.artemis`` is only refreshed by a full
  installer run (``copy_self_to_artemis_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Artemis is still running") on every
  GUI update forever, with no Artemis process actually running.
"""
...
MARKER_NAME = ".artemis-update-in-progress"

# Set by an orchestrating updater (the Tauri `artemis-setup --update` flow) to
# its own pid before spawning `artemis update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ARTEMIS_UPDATE_HANDOFF_PID"
...
# to show "Artemis is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Artemis home (never the context-local profile override):
    the Rust updater resolves ``$ARTEMIS_HOME`` or the platform default, and the
    ...
    """
    from artemis_constants import get_process_artemis_home

    return get_process_artemis_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``artemis update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``artemis-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Artemis update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `artemis update` as a stage): we run under ITS claim
