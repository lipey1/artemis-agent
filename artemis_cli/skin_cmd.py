"""``artemis skin`` — list, switch, and tweak skins from the CLI.
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    if path.exists():
        ...
        from artemis_cli.skin_engine import load_skin
    ...
    print(f"✓ {key} = {value} in {display_artemis_home()}/skins/{target}.yaml (live within ~1s)")
    return 0
