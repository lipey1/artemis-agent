"""``artemis logs`` — view and filter Artemis log files.
...
Usage examples::
    ...
    artemis logs                    # last 50 lines of agent.log
    artemis logs -f                 # follow agent.log in real time
    artemis logs errors             # last 50 lines of errors.log
    artemis logs gateway -n 100    # last 100 lines of gateway.log
    artemis logs gui -f            # follow gui.log (dashboard/pty/ws)
    artemis logs desktop -f        # follow desktop.log (Electron app boot/backend)
    artemis logs --level WARNING    # only WARNING+ lines
    artemis logs --session abc123   # filter by session ID substring
    artemis logs --component tools  # only tool-related lines
    artemis logs --since 1h         # lines from the last hour
    artemis logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_artemis_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Artemis runs — try 'artemis chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_artemis_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_artemis_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_artemis_home()}/logs/")
        return

    print(f"Log files in {display_artemis_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'artemis chat' to generate logs)")
