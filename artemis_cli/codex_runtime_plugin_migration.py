"""Migrate Artemis' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Artemis' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Artemis mcp_servers.<n>.command/args/env  → codex stdio transport
  Artemis mcp_servers.<n>.url/headers       → codex streamable_http transport
  Artemis mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Artemis mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Artemis-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Artemis' own
...
MIGRATION_MARKER = (
    "# managed by artemis-agent — `artemis codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end artemis-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Artemis config.")
...
# Artemis keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Artemis' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, artemis_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Artemis MCP server config to the codex inline-table dict
    ...
    if not isinstance(artemis_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(artemis_cfg.get("command"))
    has_url = bool(artemis_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ARTEMIS_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Artemis)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Artemis' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Artemis' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Artemis
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="artemis-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ARTEMIS_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed artemis-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.artemis`` symlink that happens to live under ``/private/var/folders``
...
def _build_artemis_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Artemis' own
    ...
    so a artemis installed under /opt/, /usr/local/, or a venv all work.
    ARTEMIS_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ARTEMIS_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_artemis_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ARTEMIS_HOME its launcher sets
    ...
    # — that would override the launcher's ARTEMIS_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ARTEMIS_HOME", tmp_path) would
    ...
    artemis_home = os.environ.get("ARTEMIS_HOME") or ""
    if artemis_home and _looks_like_test_tempdir(artemis_home):
        artemis_home = ""
    if artemis_home:
        env["ARTEMIS_HOME"] = artemis_home
    # PYTHONPATH passes through so a worktree-launched artemis finds the
    ...
    env["ARTEMIS_QUIET"] = "1"
    env["ARTEMIS_REDACT_SECRETS"] = env.get("ARTEMIS_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.artemis_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_artemis_tools: bool = True,
) -> MigrationReport:
    """Translate Artemis mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        artemis_config: full ~/.artemis/config.yaml dict
        ...
        expose_artemis_tools: when True (default), register Artemis' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Artemis for tools
    ...
    artemis_servers = (artemis_config or {}).get("mcp_servers") or {}
    if not isinstance(artemis_servers, dict):
        report.errors.append(
            "mcp_servers in Artemis config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in artemis_servers.items():
    ...
    # Inject Artemis' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Artemis for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/artemis_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_artemis_tools:
        translated["artemis-tools"] = _build_artemis_tools_mcp_entry()
        if "artemis-tools" not in report.migrated:
            report.migrated.append("artemis-tools")
