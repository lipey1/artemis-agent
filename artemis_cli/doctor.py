"""
Doctor command for artemis CLI.

Diagnoses issues with Artemis Agent setup.
"""
...
from artemis_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
ARTEMIS_HOME = get_artemis_home()
_DHH = display_artemis_home()  # user-facing display path (e.g. ~/.artemis or ~/.artemis/profiles/coder)

# Load environment variables from ~/.artemis/.env so API key checks work
_env_path = get_env_path()
load_artemis_dotenv(artemis_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from artemis_cli.colors import Colors, color
from artemis_cli.models import _ARTEMIS_USER_AGENT
from artemis_cli.vercel_auth import describe_vercel_auth
from artemis_constants import OPENROUTER_MODELS_URL
...
from artemis_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Artemis containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `artemis update`"
    return (
...
def _artemis_database_paths(artemis_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Artemis-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from artemis_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, artemis_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((artemis_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(artemis_home)), board_db))
    return entries
...
def _format_db_size(db_path: Path) -> str:
    ...
    from artemis_cli.backup import _format_size
...
def _report_database_journal_modes(
    artemis_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from artemis_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = artemis_home if artemis_home is not None else ARTEMIS_HOME
    try:
        databases = _artemis_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Artemis databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.artemis/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ARTEMIS_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from artemis_cli.auth import get_minimax_oauth_auth_status
...
from artemis_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'artemis sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. ARTEMIS_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ARTEMIS_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ARTEMIS_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches artemis_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``artemis --version`` reporting a stale version while
    ...
    """
    try:
        from artemis_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != artemis_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'artemis update', or set "
            "artemis_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-artemis and dashboard static services are up
    ...
    for static in ("main-artemis", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `artemis profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `artemis doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from artemis_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ARTEMIS_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ARTEMIS_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ARTEMIS_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `artemis`.
    os.environ.setdefault("ARTEMIS_INTERACTIVE", "1")

    # Handle `artemis doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.artemis/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Artemis Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`artemis doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from artemis_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Artemis already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and artemis_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.artemis/.env (primary location for user config)
    env_path = ARTEMIS_HOME / '.env'
    if env_path.exists():
        ...
        # matches artemis_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'artemis setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'artemis setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'artemis setup' to create one")
                issues.append("Run 'artemis setup' to create .env")
    
    # Check ~/.artemis/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ARTEMIS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from artemis_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from artemis_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'artemis config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from artemis_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.artemis/.env or run 'artemis setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'artemis setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'artemis config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from artemis_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from artemis_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'artemis setup' to migrate config")
                else:
                    issues.append("Run 'artemis doctor --fix' or 'artemis setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from artemis_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'artemis doctor --fix'")
        except Exception:
            pass

        # Detect stale ARTEMIS_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ARTEMIS_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from artemis_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("ARTEMIS_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ARTEMIS_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `artemis setup` run)",
                )
                if should_fix:
                    if remove_env_value("ARTEMIS_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ARTEMIS_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ARTEMIS_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ARTEMIS_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ARTEMIS_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'artemis doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from artemis_cli.config import load_env as _load_env_depr
            from artemis_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Artemis' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = artemis_home / subdir_name
    ...
    soul_path = artemis_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Artemis a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Artemis Agent Persona\n\n"
                "<!-- Edit this file to customize how Artemis communicates. -->\n\n"
                "You are Artemis, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = artemis_home / "memories"
    ...
    state_db_path = artemis_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from artemis_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'artemis doctor --fix' "
                        "(or 'artemis sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from artemis_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'artemis doctor --fix' "
                        "(or 'artemis sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Artemis
        ...
        try:
            from artemis_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'artemis sessions optimize-storage' "
    ...
    wal_path = artemis_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'artemis doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "artemis"
        ...
        _cmd_link = _cmd_link_dir / "artemis"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(artemis not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/artemis → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/artemis points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/artemis → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/artemis — run 'artemis doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/artemis exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/artemis not found",
                    "(artemis command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/artemis → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/artemis symlink — run 'artemis doctor --fix'")
    ...
    try:
        from artemis_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'artemis-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'artemis-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `artemis acp --setup-browser` installs agent-browser into the
        # Artemis-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_artemis_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(ARTEMIS_HOME / "node" / "bin")
            or _which_in(ARTEMIS_HOME / "node")
        )
        _legacy_ab = _which_in(ARTEMIS_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `artemis update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `artemis doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ARTEMIS_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Artemis one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'artemis config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _ARTEMIS_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``artemis doctor``.
    ...
    # never the legitimate source for `artemis doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'artemis setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ARTEMIS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: artemis skills list)")
    ...
    try:
        from artemis_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = ARTEMIS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: artemis memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: artemis memory setup",
                    "No Honcho API key — run 'artemis memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run artemis memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: artemis memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: artemis memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from artemis_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "artemis -p" in content:
                            _m = _re.search(r"artemis -p (\S+)", content)
    ...
    # `artemis doctor --live` (real network calls; bounded + read-only).
    try:
        from artemis_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'artemis doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
