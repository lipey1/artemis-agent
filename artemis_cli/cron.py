"""
Cron subcommand for artemis CLI.
...
# commands at execution time when ``_ARTEMIS_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: artemis gateway install", Colors.DIM))
    print(color("                    sudo artemis gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  artemis cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'artemis cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: artemis gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec artemis "
                        "artemis cron ...`). Fix ownership to match the "
        ...
        print("    artemis gateway install    # Install as a user service")
        print("    sudo artemis gateway install --system  # Linux servers: boot-time system service")
        print("    artemis gateway            # Or run in foreground")
...
def cron_notepad(args) -> int:
    """Handle ``artemis cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: artemis cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: artemis cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: artemis cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: artemis cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
