"""
Configuration management for Artemis Agent.

Config files are stored in ~/.artemis/ for easy access:
- ~/.artemis/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.artemis/.env         - API keys and secrets

This module provides:
- artemis config          - Show current configuration
- artemis config edit     - Open config in editor
- artemis config get      - Print a resolved configuration value
- artemis config set      - Set a specific value
- artemis config unset    - Remove a user configuration value
- artemis config wizard   - Re-run setup wizard
"""
...
from artemis_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``artemis config set`` (which rewrites
    ...
    ``config.yaml`` in place: artemis never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.artemis/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``artemis logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  artemis config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Artemis makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Artemis itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``artemis update``, the TUI build.
...
#   on every plugin install / ``artemis update``.
...
# * ``ARTEMIS_HOME`` / ``ARTEMIS_PROFILE`` / ``ARTEMIS_CONFIG`` /
#   ``ARTEMIS_ENV`` — Artemis runtime location flags. Writing these into
...
#
# IMPORTANT: ``ARTEMIS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ARTEMIS_LANGFUSE_PUBLIC_KEY,
# ARTEMIS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Artemis runtime location — never via dashboard env writer.
    # NOT a ARTEMIS_* blanket: integration credentials (ARTEMIS_GEMINI_*,
    # ARTEMIS_LANGFUSE_*, ARTEMIS_SPOTIFY_*, ...) ARE allowed.
    "ARTEMIS_HOME", "ARTEMIS_PROFILE", "ARTEMIS_CONFIG", "ARTEMIS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Artemis runtime location "
            "(ARTEMIS_HOME, ARTEMIS_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.artemis/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ARTEMIS_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `artemis plugins enable
    # observability/langfuse` or `artemis tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ARTEMIS_LANGFUSE_ENV",
    "ARTEMIS_LANGFUSE_RELEASE",
    "ARTEMIS_LANGFUSE_SAMPLE_RATE",
    "ARTEMIS_LANGFUSE_MAX_CHARS",
    "ARTEMIS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "ARTEMIS_ACP_AUTH_METHOD",
    "ARTEMIS_ACP_AUTO_APPROVE",
    "ARTEMIS_COPILOT_ACP_COMMAND",
    "ARTEMIS_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from artemis_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ARTEMIS_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ARTEMIS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_artemis_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Artemis is running in package-manager-managed mode.

    Two signals: the ARTEMIS_MANAGED env var (set by the systemd service),
    or a .managed marker file in ARTEMIS_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Artemis through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``artemis_cli/`` — i.e. the git checkout for source
    installs, ``/opt/artemis`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ARTEMIS_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Artemis was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ARTEMIS_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ARTEMIS_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.artemis``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ARTEMIS_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.artemis:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ARTEMIS_HOME``, a
    ...
    ``docker`` and ``artemis update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/artemis-agent`` image bakes a ``docker``
        stamp into ``/opt/artemis`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ARTEMIS_HOME.
    try:
    ...
    #    host install whose shared $ARTEMIS_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``artemis update``.
    try:
        method = (
            (get_artemis_home() / ".install_method")
    ...
    # detect Nix installs that don't set ARTEMIS_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``artemis_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ARTEMIS_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/artemis`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/artemis-agent:latest"
    return "artemis update"
...
# Long-form text for ``artemis update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# artemis_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Artemis, it doesn't update
...
✗ ``artemis update`` doesn't apply inside the Docker container.

Artemis Agent runs as a published image (nousresearch/artemis-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/artemis-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate artemis-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/artemis-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/artemis-agent/tags
  • Your config and session history live under ``$ARTEMIS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``artemis update`` inside Docker.
...
def format_managed_message(action: str = "modify this Artemis installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ARTEMIS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Artemis installation is managed by NixOS "
            f"(ARTEMIS_MANAGED={env_hint}).\n"
            "Edit services.artemis-agent.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Artemis installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Artemis."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ARTEMIS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, artemis_bin
    or None if container mode is not active, we're already inside the
    container, or ARTEMIS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ARTEMIS_DEV") == "1":
        return None
    ...
    container_mode_file = get_artemis_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "artemis-agent")
    exec_user = info.get("exec_user", "artemis")
    artemis_bin = info.get("artemis_bin", "/data/current-package/bin/artemis")
...
# Re-export from artemis_constants — canonical definition lives there.
from artemis_constants import get_artemis_home, get_process_artemis_home  # noqa: F811,E402
...
def get_env_path() -> Path:
    ...
    return get_artemis_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_artemis_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ARTEMIS_UID / ARTEMIS_GID env vars set by Docker deployments.

    Docker containers running Artemis commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ARTEMIS_HOME once, but
    subdirectories created at runtime by ``ensure_artemis_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ARTEMIS_UID", "").strip()
    gid_str = os.environ.get("ARTEMIS_GID", "").strip()
    try:
...
def _chown_to_artemis_uid(path) -> None:
    """Chown ``path`` to ``ARTEMIS_UID:ARTEMIS_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_artemis_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_artemis_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the artemis group can
    share state with the gateway service.

    The mode can be overridden via the ARTEMIS_HOME_MODE environment variable
    (e.g. ARTEMIS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ARTEMIS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ARTEMIS_UID``/``ARTEMIS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ARTEMIS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_artemis_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Artemis runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ARTEMIS_CONTAINER") or os.environ.get("ARTEMIS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ARTEMIS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ARTEMIS_HOME, upgrading legacy empty templates.
...
# ensure_artemis_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ARTEMIS_HOME_ENSURED: set = set()


def ensure_artemis_home():
    """Ensure ~/.artemis directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ARTEMIS_HOME`` we only re-run
    ...
    ``get_artemis_home()`` and therefore re-run for the new path.
    """
    home = get_artemis_home()
    key = str(home)

    if key in _ARTEMIS_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``artemis profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ARTEMIS_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_artemis_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _ARTEMIS_HOME_ENSURED.add(key)


def _ensure_artemis_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ARTEMIS_HOME {home} does not exist. "
...
from artemis_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `artemis config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.artemis.config`` entries, then checks
    ...
    try:
        ...
        # should never break `artemis update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Artemis' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Artemis versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``artemis_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (artemis_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (artemis_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (artemis_cli/setup.py)
    "known_plugin_toolsets", # written/read by artemis_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'artemis setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Artemis won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, artemis send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'artemis doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_artemis_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "artemis update / artemis -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from artemis_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ artemis config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in artemis_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from artemis_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from artemis_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: artemis config set <key> <value>")
    ...
    # `artemis update`, but only the version bump is persisted.
    ...
    # metadata.artemis.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.artemis/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_artemis_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``artemis config set``
    ...
    backends) — while display paths (``artemis status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _artemis_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in artemis_cli.personality.
from artemis_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`artemis_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from artemis_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from artemis_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``ARTEMIS_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`artemis_cli.personality` (single owner).
    """
    from artemis_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.artemis/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_artemis_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``ARTEMIS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Artemis host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/artemis/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_artemis_dotenv()
...
#   openai-codex (OAuth — artemis auth) — OpenAI Codex
#   nous         (OAuth — artemis auth) — Nous Portal
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Artemis writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.artemis/.env.
    ...
    interactive menu render (`artemis tools`, `artemis setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `artemis tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.artemis/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.artemis/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ARTEMIS_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ARTEMIS_CUSTOM_{slug}_API_KEY" if slug else "ARTEMIS_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.artemis/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.artemis/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Artemis — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Artemis vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.artemis/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.artemis/.env`` over ``os.environ``.

    Used for Artemis-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Artemis Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from artemis_cli.auth import get_anthropic_key
    ...
    # Warn on stale ARTEMIS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ARTEMIS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ARTEMIS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'artemis doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from artemis_cli.personality import active_personality_name
    ...
    print(color("  artemis config edit     # Edit config file", Colors.DIM))
    print(color("  artemis config set <key> <value>", Colors.DIM))
    print(color("  artemis setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `artemis cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``artemis config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``artemis config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``artemis config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (artemis config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``artemis config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``artemis config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    artemis config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    artemis config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `artemis config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_artemis_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `artemis config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `artemis config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: artemis config get <key> [--json]")
            print()
            print("Examples:")
            print("  artemis config get model")
            print("  artemis config get terminal.backend")
            print("  artemis config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: artemis config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  artemis config set model anthropic/claude-sonnet-4")
            print("  artemis config set terminal.backend docker")
            print("  artemis config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: artemis config unset <key>")
            print()
            print("Examples:")
            print("  artemis config unset model")
            print("  artemis config unset terminal.backend")
            print("  artemis config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'artemis config migrate' to add them")
        ...
        print("  artemis config           Show current configuration")
        print("  artemis config edit      Open config in editor")
        print("  artemis config get <key>          Print a resolved config value")
        print("  artemis config set <key> <value>   Set a config value")
        print("  artemis config unset <key>        Remove a config value")
        print("  artemis config check     Check for missing/outdated config")
        print("  artemis config migrate   Update config with new options")
        print("  artemis config path      Show config file path")
        print("  artemis config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``artemis config`` UI so users
