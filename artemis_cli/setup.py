"""
Interactive setup wizard for Artemis Agent.
...
from artemis_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://artemis-agent.nousresearch.com/docs"
...
# display_artemis_home imported lazily at call sites (stale-module safety during artemis update)
...
from artemis_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from artemis_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Artemis Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Artemis using environment variables or config commands:")
    print_info("  artemis config set model.provider custom")
    print_info("  artemis config set model.base_url http://localhost:8080/v1")
    print_info("  artemis config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'artemis setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``ARTEMIS_NONINTERACTIVE=1`` (see ``artemis_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ARTEMIS_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ARTEMIS_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from artemis_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'artemis setup')")


def _print_setup_summary(config: dict, artemis_home):
    ...
    try:
        from artemis_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Artemis cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    artemis model            (pick any provider/model)")
        print_info("    artemis setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'artemis setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from artemis_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `artemis tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from artemis_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'artemis setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'artemis setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'artemis tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'artemis setup terminal'"))
    ...
    # Spotify (OAuth via artemis auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'artemis setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {artemis_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('artemis setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('artemis setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('artemis setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('artemis setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('artemis setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('artemis config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('artemis config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('artemis config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('artemis', Colors.GREEN)}              Start chatting")
    print(f"   {color('artemis gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('artemis doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `artemis tools` and `artemis setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``artemis model``)
    ...
    provider added to ``artemis model`` is automatically available here.
    ...
    # Delegate to the shared artemis model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from artemis_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: artemis model")
    ...
    # on demand via `artemis auth add`, `artemis setup` vision, and
    # `artemis setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from artemis_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``artemis model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from artemis_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.artemis/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Artemis auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via artemis setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'artemis setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Artemis runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `artemis setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `artemis setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `artemis egress setup` then `artemis egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `artemis egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'artemis-agent[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Artemis installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'artemis-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ARTEMIS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ARTEMIS_MAX_ITERATIONS")
    ...
    print_info("  Run `artemis setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ARTEMIS_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.artemis/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from artemis_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_artemis_home(Path(get_artemis_home()))
    except Exception:
        pass
...
def _profile_name_from_artemis_home(artemis_home) -> str | None:
    """Return the active profile name when ARTEMIS_HOME is a profile dir."""
    if artemis_home.parent.name == "profiles":
        return artemis_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Artemis to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from artemis_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://artemis-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://artemis-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  artemis config edit")
    print_info("   Open config in your editor:  artemis config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from artemis_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Artemis from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'artemis setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     artemis config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_artemis_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: artemis gateway install --system")
                print_info("  Or run in foreground:  artemis gateway")
        else:
            from artemis_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   artemis gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   artemis gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `artemis setup tools` and `artemis tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Artemis profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``artemis_cli.auth`` — lists every supported
    ...
    """
    try:
        from artemis_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from artemis_cli.config
    ...
    if section_key == "model":
        ...
        from artemis_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_artemis.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_artemis migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_artemis", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Artemis.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Artemis to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Artemis at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Artemis at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Artemis at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Artemis at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Artemis equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Artemis config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Artemis.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Artemis's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(artemis_home: Path) -> bool:
    ...
    print_info("Artemis can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: artemis claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=artemis_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: artemis claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Artemis configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Artemis config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Artemis (use artemis claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``artemis setup --portal`` and ``artemis portal``. This is the
    ...
    Artemis session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``artemis setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``artemis model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``artemis portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Artemis Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `artemis portal` == quick setup's Nous step.
    try:
        from artemis_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `artemis portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `artemis portal`: %s", exc)
        print()
    ...
    print_info("  Run `artemis portal info` to inspect routing.")
    print_info("  Run `artemis` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      artemis setup           — full or quick (auto-detected)
      artemis setup model     — just model/provider
      artemis setup tts       — just text-to-speech
      artemis setup terminal  — just terminal backend
      artemis setup gateway   — just messaging platforms
      artemis setup tools     — just tool configuration
      artemis setup telemetry — just local shared metrics
      artemis setup agent     — just agent settings
    """
    from artemis_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Artemis Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Artemis Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Artemis Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, artemis_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Artemis configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'artemis setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(artemis_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Artemis?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, artemis_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, artemis_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {artemis_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'artemis config edit'")
    ...
    # Tune later with `artemis setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, artemis_home)


def _run_first_time_quick_setup(config: dict, artemis_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``artemis setup <section>``
    or switch providers with ``artemis model``.
    """
    from artemis_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'artemis setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    artemis setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  artemis setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``artemis tools`` (which
    ...
    try:
        ...
        from artemis_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``artemis setup agent`` /
    ``artemis config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, artemis_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `artemis update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        artemis tools")
        print_info("  Seed skills:         artemis skills opt-in --sync")
        print_info("  Add MCP servers:     artemis mcp add")
        print_info("  Enable plugins:      artemis plugins")
        print_info("  Tune agent settings: artemis setup agent")
        print()
        _print_setup_summary(config, artemis_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, artemis_home)


def _blank_slate_walkthrough(config: dict, artemis_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`artemis update` runs from re-injecting them. Opt back in any")
            print_info("time with `artemis skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `artemis tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `artemis plugins list` / `artemis plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `artemis plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `artemis mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `artemis mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   artemis tools")
    print_info("  Seed skills:         artemis skills opt-in --sync")
    print_info("  Add MCP servers:     artemis mcp add")
    print_info("  Tune agent settings: artemis setup agent")
    print()
...
def _run_quick_setup(config: dict, artemis_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'artemis setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Artemis to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'artemis setup gateway'.")
