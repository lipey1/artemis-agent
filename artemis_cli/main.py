...
Usage:
    artemis                     # Interactive chat (default)
    artemis chat                # Interactive chat
    artemis gateway             # Run gateway in foreground
    artemis gateway start       # Start gateway as service
    artemis gateway stop        # Stop gateway service
    artemis gateway status      # Show gateway status
    artemis gateway install     # Install gateway service
    artemis gateway uninstall   # Uninstall gateway service
    artemis setup               # Interactive setup wizard
    artemis logout              # Clear stored authentication
    artemis status              # Show status of all components
    artemis cron                # Manage cron jobs
    artemis cron list           # List cron jobs
    artemis cron status         # Check if cron scheduler is running
    artemis doctor              # Check configuration and dependencies
    artemis honcho setup                    # Configure Honcho AI memory integration
    artemis honcho status                   # Show Honcho config and connection status
    artemis honcho sessions                 # List directory → session name mappings
    artemis honcho map <name>               # Map current directory to a session name
    artemis honcho peer                     # Show peer names and dialectic settings
    artemis honcho peer --user NAME         # Set user peer name
    artemis honcho peer --ai NAME           # Set AI peer name
    artemis honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    artemis honcho mode                     # Show current memory mode
    artemis honcho mode [hybrid|honcho|local]  # Set memory mode
    artemis honcho tokens                   # Show token budget settings
    artemis honcho tokens --context N       # Set session.context() token cap
    artemis honcho tokens --dialectic N     # Set dialectic result char cap
    artemis honcho identity                 # Show AI peer identity representation
    artemis honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    artemis honcho migrate                  # Step-by-step migration guide: OpenClaw native → Artemis + Honcho
    artemis version             Show version
    artemis update              Update to latest version
    artemis uninstall           Uninstall Artemis Agent
    artemis acp                 Run as an ACP server for editor integration
    artemis sessions browse     Interactive session picker with search

    artemis claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: artemis_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``artemis_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``artemis update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``artemis_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, artemis crashes on import and the user can't run
# ``artemis update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import artemis_bootstrap  # noqa: F401
...
from artemis_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python artemis_cli/main.py`` (script
# mode — sys.path[0] is artemis_cli/, not the repo root) can import the
# canonical helpers; everything else lives in artemis_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from artemis_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``artemis update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from artemis_cli.env_loader import ...`` / ``from artemis_cli.config import
...
# package directory, so if IT can't import, nothing else in artemis_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from artemis_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from artemis_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'artemis' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``artemis tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``artemis.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("artemis")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"artemis", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"artemis")
        # Windows: the .exe name is already ``artemis.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* artemis_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ARTEMIS_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".artemis", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ARTEMIS_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``artemis … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("ARTEMIS_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``ARTEMIS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ARTEMIS_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`artemis --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(artemis_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(artemis_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``artemis --version`` before config/logging imports."""
...
from artemis_cli.sessions_cmd import cmd_sessions  # noqa: F401
from artemis_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from artemis_cli.subcommands.cron import build_cron_parser
from artemis_cli.subcommands.sync import build_sync_parser
from artemis_cli.subcommands.gateway import build_gateway_parser
from artemis_cli.subcommands.profile import build_profile_parser
from artemis_cli.subcommands.model import build_model_parser
from artemis_cli.subcommands.setup import build_setup_parser

from artemis_cli.subcommands.whatsapp import build_whatsapp_parser
from artemis_cli.subcommands.slack import build_slack_parser
from artemis_cli.subcommands.login import build_login_parser
from artemis_cli.subcommands.logout import build_logout_parser
from artemis_cli.subcommands.auth import build_auth_parser
from artemis_cli.subcommands.status import build_status_parser
from artemis_cli.subcommands.pause import build_pause_parser
from artemis_cli.subcommands.webhook import build_webhook_parser
from artemis_cli.subcommands.hooks import build_hooks_parser
from artemis_cli.subcommands.doctor import build_doctor_parser
from artemis_cli.subcommands.verify import build_verify_parser
from artemis_cli.subcommands.security import build_security_parser
from artemis_cli.subcommands.approvals import build_approvals_parser
from artemis_cli.subcommands.dump import build_dump_parser
from artemis_cli.subcommands.debug import build_debug_parser
from artemis_cli.subcommands.backup import build_backup_parser
from artemis_cli.subcommands.import_cmd import build_import_cmd_parser
from artemis_cli.subcommands.import_agent import build_import_agent_parser
from artemis_cli.subcommands.config import build_config_parser
from artemis_cli.subcommands.skin import build_skin_parser
from artemis_cli.subcommands.console import build_console_parser
from artemis_cli.subcommands.version import build_version_parser
from artemis_cli.subcommands.update import build_update_parser
from artemis_cli.subcommands.uninstall import build_uninstall_parser
from artemis_cli.subcommands.dashboard import build_dashboard_parser
from artemis_cli.subcommands.gui import build_gui_parser
from artemis_cli.subcommands.logs import build_logs_parser
from artemis_cli.subcommands.prompt_size import build_prompt_size_parser
from artemis_cli.subcommands.memory import build_memory_parser
from artemis_cli.subcommands.acp import build_acp_parser
from artemis_cli.subcommands.tools import build_tools_parser
from artemis_cli.subcommands.insights import build_insights_parser
from artemis_cli.subcommands.monitoring import build_monitoring_parser
from artemis_cli.subcommands.skills import build_skills_parser
from artemis_cli.subcommands.pairing import build_pairing_parser
from artemis_cli.subcommands.plugins import build_plugins_parser
from artemis_cli.subcommands.mcp import build_mcp_parser
from artemis_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (artemis tools, artemis setup, artemis model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'artemis {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any artemis module import.
#
# Many modules cache ARTEMIS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ARTEMIS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.artemis/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ARTEMIS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `artemis mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Artemis' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo artemis -p <name>` against the invoking user's home.
        ...
        candidate = home / ".artemis" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`artemis chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors artemis_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ARTEMIS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.artemis/profiles/coder or
    # /opt/data/profiles/coder).  If ARTEMIS_HOME points to the artemis root
    # instead (e.g. systemd hardcodes ARTEMIS_HOME=/root/.artemis), we must
    # still read active_profile — the user may have switched profiles via
    # `artemis profile use` and the gateway should honour that choice.
    # See issue #22502.
    artemis_home_env = os.environ.get("ARTEMIS_HOME", "")
    if profile_name is None and artemis_home_env:
        if Path(artemis_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the artemis root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ARTEMIS_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``artemis gateway run`` to mean
    # "the root ARTEMIS_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ARTEMIS_S6_SUPERVISED_CHILD"):
        try:
            from artemis_constants import get_default_artemis_root

            active_path = get_default_artemis_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ARTEMIS_HOME
    if profile_name is not None:
        try:
            from artemis_cli.profiles import resolve_profile_env

            artemis_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            artemis_home = _resolve_sudo_user_profile_env(profile_name)
            if not artemis_home:
            ...
            # A bug in profiles.py must NEVER prevent artemis from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ARTEMIS_HOME"] = artemis_home
...
# Load .env from ~/.artemis/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from artemis_cli.config import get_artemis_home
from artemis_cli.env_loader import load_artemis_dotenv

load_artemis_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ARTEMIS_REDACT_SECRETS env
# var BEFORE artemis_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves artemis_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from artemis_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_artemis_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from artemis_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ARTEMIS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ARTEMIS_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `artemis` subcommands
...
try:
    from artemis_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from artemis_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if artemis_constants not importable yet
...
from artemis_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to artemis_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (artemis_cli.main._model_flow_*) keep resolving unchanged.
from artemis_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_artemis_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ARTEMIS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ARTEMIS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`artemis_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from artemis_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from artemis_cli.config import get_env_path, get_artemis_home, load_config
    from artemis_cli.auth import get_auth_status

    # Determine whether Artemis itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from artemis_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_artemis_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from artemis_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_artemis_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Artemis has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Artemis to use their tokens.
    if _has_artemis_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``artemis -c`` from repo A continues repo A's last session rather than the
    ...
    db = None
    try:
        from artemis_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, artemis_bin
        cli_args: the original CLI arguments (everything after 'artemis')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    artemis_bin = container_info["artemis_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo artemis {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo artemis {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  artemis --tui --resume {target}")
    if title:
        print(f'  artemis --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @artemis/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@artemis" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/artemis-ink/package.json",
    "packages/artemis-ink/index.js",
    "packages/artemis-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ARTEMIS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ARTEMIS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ARTEMIS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ARTEMIS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    artemis_home = str(get_artemis_home())
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ARTEMIS_HOME": artemis_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(artemis_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(artemis_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if artemis_cli_dir is None:
        artemis_cli_dir = Path(__file__).parent
    bundled = artemis_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``artemis update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ARTEMIS_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Artemis checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `artemis update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Artemis checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `artemis --tui`\n"
        "If the checkout is still inconsistent, run `artemis update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ARTEMIS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ARTEMIS_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $ARTEMIS_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Artemis installed,
        # and would pick a system Node over the managed one when both exist.
        from artemis_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from artemis_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ARTEMIS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ARTEMIS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ARTEMIS_TUI_DIR (e.g. `unset ARTEMIS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # artemis_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from artemis_constants import with_artemis_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_artemis_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Artemis-managed
            ...
            from artemis_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @artemis/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @artemis/ink resolves through
        # packages/artemis-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "artemis-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from artemis_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("ARTEMIS_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ARTEMIS_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ARTEMIS_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ARTEMIS_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ARTEMIS_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ARTEMIS_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["ARTEMIS_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a artemis process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from artemis_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="artemis-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ARTEMIS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ARTEMIS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["ARTEMIS_MODEL"] = model
        env["ARTEMIS_INFERENCE_MODEL"] = model
    if provider:
        env["ARTEMIS_TUI_PROVIDER"] = provider
        env["ARTEMIS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ARTEMIS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ARTEMIS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ARTEMIS_TUI_SKILLS"] = value
    if query:
        env["ARTEMIS_TUI_QUERY"] = query
    if image:
        env["ARTEMIS_TUI_IMAGE"] = image
    if checkpoints:
        env["ARTEMIS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ARTEMIS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ARTEMIS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ARTEMIS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ARTEMIS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ARTEMIS_ACCEPT_HOOKS"] = "1"
    ...
    # ARTEMIS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `artemis --tui` try to
    ...
    # still set ARTEMIS_TUI_RESUME themselves.
    env.pop("ARTEMIS_TUI_RESUME", None)
    if resume_session_id:
        env["ARTEMIS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `artemis update` so
    ...
    if code == 42:
        from artemis_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ARTEMIS_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``artemis kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``artemis kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ARTEMIS_KANBAN_BOARD"):
        return
    try:
        from artemis_cli.kanban_db import get_current_board

        os.environ["ARTEMIS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.artemis/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Artemis — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Artemis still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``artemis … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("ARTEMIS_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'artemis sessions list' to see available sessions.")
            sys.exit(1)
    ...
    try:
        from artemis_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from artemis_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'artemis doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Artemis isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  artemis setup")
        print()

        from artemis_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'artemis setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from artemis_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    # rare launch right after `artemis update` changed a bundled skill.
    ...
    if getattr(args, "yolo", False):
        os.environ["ARTEMIS_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.artemis/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ARTEMIS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ARTEMIS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ARTEMIS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from artemis_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from artemis_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from artemis_cli.config import get_env_value, save_env_value
    from artemis_constants import find_node_executable, with_artemis_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Artemis?")
        print()
    ...
    # has no creds.json.  Every subsequent `artemis gateway` then paid a 30s
    ...
    session_dir = get_artemis_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: artemis gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `artemis gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  artemis gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Artemis Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  artemis gateway")
        ...
        print()
        print("  Or install as a service: artemis gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'artemis whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``artemis whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``artemis_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from artemis_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from artemis_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from artemis_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``artemis model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from artemis_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from artemis_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from artemis_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("ARTEMIS_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from artemis_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'artemis model' for "
                "available providers, or run 'artemis doctor' to diagnose config "
                "issues."
            )
    ...
    from artemis_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        _PROVIDER_ALIASES,
        group_providers,
    ...
    # PROVIDER_GROUPS in artemis_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``artemis model``
    ...
    # matching against artemis_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    # a leftover OPENAI_BASE_URL in ~/.artemis/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.artemis/.env if the active provider is not 'custom'.
    ...
    """
    from artemis_cli.config import get_env_value, save_env_value, load_config
...
#
# Artemis uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `artemis model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `artemis model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`artemis_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from artemis_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from artemis_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Artemis only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``artemis model`` flow, then route aux tasks
    to them here.
    """
    from artemis_cli.config import load_config
    from artemis_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from artemis_cli.auth import _prompt_model_selection
    from artemis_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from artemis_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from artemis_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from artemis_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Artemis URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from artemis_cli.curses_ui import curses_radiolist
...
# downstream call sites read `artemis_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `artemis` invocation — including
# fast paths like `artemis --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from artemis_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``artemis setup`` / ``artemis model``.
    ...
    recover from a malformed paste without editing ``~/.artemis/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from artemis_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from artemis_cli.config import save_env_value
    from artemis_cli.secret_prompt import masked_secret_prompt
    ...
    from artemis_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `artemis setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from artemis_constants import display_artemis_home as _dhh_fn

            print(
                f"    Artemis will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Artemis CLI with a provider."""
    from artemis_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from artemis_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from artemis_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from artemis_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from artemis_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.artemis/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `artemis sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set ARTEMIS_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "ARTEMIS_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="artemis sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="artemis sync now")
...
def cmd_webhook(args):
    ...
    from artemis_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``artemis slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: artemis slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `artemis slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from artemis_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from artemis_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from artemis_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from artemis_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from artemis_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from artemis_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `artemis security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from artemis_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `artemis approvals <subcmd>`."""
    from artemis_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from artemis_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from artemis_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from artemis_cli.config import config_command
...
def cmd_skin(args):
    ...
    from artemis_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Artemis home directory to a zip file."""
    if getattr(args, "quick", False):
        from artemis_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from artemis_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Artemis backup from a zip file."""
    from artemis_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from artemis_cli.config import detect_install_method
    from artemis_cli.slash_exec import CommandContext, execute_command
    ...
    try:
        from artemis_cli.banner import check_for_updates
        from artemis_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Artemis Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from artemis_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from artemis_cli.uninstall import run_gui_uninstall
    ...
    from artemis_cli.uninstall import run_uninstall
...
# Update pipeline extracted to artemis_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (artemis_cli.main._cmd_update_impl,
# artemis_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from artemis_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in ARTEMIS_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``artemis update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``artemis update``
    ...
    ``artemis`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``artemis update``
    ...
    ``artemis_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "artemis_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ARTEMIS_HOME."""
    from artemis_constants import get_artemis_home
    return get_artemis_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``artemis`` launcher present but ``artemis_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the artemis venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — artemis venv python3, no nix-shell overhead
    ...
    # a artemis venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``artemis update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Artemis-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_artemis_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "artemis_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``artemis web``).
    ...
    # build path usable on a stock `py -m artemis_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_artemis_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (artemis web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``artemis update`` can unconditionally call ``artemis desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``artemis desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ARTEMIS_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ARTEMIS_HOME."""
    from artemis_constants import get_artemis_home
    return get_artemis_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``artemis update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Artemis.app/Contents/MacOS/Artemis"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Artemis.exe",
            release_dir / "win-ia32-unpacked" / "Artemis.exe",
            release_dir / "win-arm64-unpacked" / "Artemis.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "artemis",
            release_dir / "linux-unpacked" / "Artemis",
            release_dir / "linux-arm64-unpacked" / "artemis",
            release_dir / "linux-arm64-unpacked" / "Artemis",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Artemis.exe
...
#
# The desktop self-update chain (Desktop → artemis-setup --update →
# `artemis update` → `artemis desktop --build-only` → relaunch) rebuilds
# Artemis.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Artemis.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Artemis.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 artemis-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Artemis.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Artemis.exe from backup.")
        print("    Your existing version was kept and still works. Run `artemis desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `artemis desktop --force-build` to rebuild, or re-run the Artemis")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Artemis'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_artemis_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Artemis.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Artemis.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Artemis.exe: Access is
    ...
    an unrelated "Artemis" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Artemis.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Artemis is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Artemis.app/Contents/MacOS/Artemis  ->  app bundle = .../Artemis.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``artemis desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Artemis Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Artemis Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ARTEMIS_DESKTOP_DISABLE_GPU env var the Electron app
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Artemis Desktop (Linux only, best-effort).
    ...
    shell. ``artemis uninstall --gui`` removes it.
    """
    try:
        from artemis_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from artemis_logging import setup_logging as _setup_logging_gui
    ...
    # with_artemis_node_path() copies os.environ when called with no arg.
    env = with_artemis_node_path()
    if getattr(args, "fake_boot", False):
        env["ARTEMIS_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ARTEMIS_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "artemis_root", None):
        env["ARTEMIS_DESKTOP_ARTEMIS_ROOT"] = str(Path(args.artemis_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ARTEMIS_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ARTEMIS_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ARTEMIS_DESKTOP_DISABLE_GPU=... artemis desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ARTEMIS_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ARTEMIS_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  artemis gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Artemis-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → artemis-setup →
            # artemis update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `artemis update` path).
            nixos_env = with_artemis_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Artemis.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Artemis.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Artemis.exe, close any")
                    print("  running Artemis desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> artemis desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Artemis is
                ...
                # success on a Artemis.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Artemis shows up
    ...
    if source_mode:
        print("→ Launching Artemis Desktop from source build...")
    ...
    print(f"→ Launching packaged Artemis Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers extracted to artemis_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on artemis_cli.main.<name> keep resolving unchanged.
from artemis_cli.dashboard_procs import (  # noqa: F401
    _detect_concurrent_artemis_instances,
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "artemis dashboard",
            "artemis_cli.main dashboard",
            "artemis_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "artemis serve",
            "artemis_cli.main serve",
            "artemis_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "artemis-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Artemis installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``artemis-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/artemis-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but artemis launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``artemis update``.
    ...
    log_path = get_artemis_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `artemis update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $ARTEMIS_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``artemis update``.
    ...
    protocol stream (``artemis acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `artemis update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``artemis.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_artemis_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from artemis.exe; applying package-only first aid, "
    ...
    try:
        ...
        if self_locked:
            print(
                "  Artemis is still running from the launcher that needs "
                "replacing. Close other Artemis windows, restart from a "
...
def _windows_running_artemis_launcher_locked() -> bool:
    """True when a venv ``artemis*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _artemis_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``artemis update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from artemis_constants import venv_bin_dir

    scripts = venv_bin_dir(venv_dir, windows=_is_windows())
    return scripts if scripts.is_dir() else None


def _artemis_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_artemis_exe(
    ...
    when ``artemis update`` runs, ``artemis.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``artemis.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next artemis invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Artemis Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Artemis Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _artemis_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Artemis Desktop, exit other `artemis` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `artemis update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_artemis_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``artemis.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``artemis.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``artemis`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_artemis_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``artemis.exe.old.*`` left by prior updates.

    Called early on every artemis invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``artemis.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``artemis.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `artemis update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``artemis.exe`` / ``artemis-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_artemis_exe`` for the rationale.
    """
    ...
    # hours later inside ``artemis update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``artemis.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``artemis.exe`` shim is locked during ``artemis update``, or when uv/distlib
    skips a launcher write. The symptom is ``artemis-agent.exe`` and
    ``artemis-acp.exe`` present but ``artemis.exe`` missing, so ``artemis`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `artemis update --force` after "
            "closing other artemis processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m artemis_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``artemis update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``artemis.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``artemis`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `artemis update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `artemis update --force` after closing other artemis processes."
        )
        return
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from artemis_constants import venv_python_path
...
class _UpdateOutputStream:
    """Stream wrapper used during ``artemis update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.artemis/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``artemis update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``artemis update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.artemis/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``artemis update --gateway``) the update is already
    ...
    try:
        ...
        # artemis_cli.config.get_artemis_home to simulate setup failure.
        from artemis_cli.config import get_artemis_home as _get_artemis_home

        logs_dir = _get_artemis_home() / "logs"
...
def cmd_update(args):
    """Update Artemis Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Artemis Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `artemis update --branch=<x>` would actually pull.
    ...
    from artemis_cli.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``artemis -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from artemis_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    if action is None:
        # Bare `artemis profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_artemis_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → artemis -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.artemis)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  artemis profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     artemis -p {name} chat")
                else:
        ...
        from artemis_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from artemis_constants import get_artemis_home as _hh
        ...
        from artemis_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `artemis profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → artemis -p {name}  ({wrapper})")
        print()
        ...
        from artemis_cli.profiles import profile_exists, validate_alias_name
        ...
        from artemis_cli.profiles import rename_profile
        ...
        from artemis_cli.profiles import export_profile
        ...
        from artemis_cli.profiles import import_profile
        ...
        import tempfile
        from artemis_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="artemis_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  artemis -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      artemis -p {plan.manifest.name} chat")
        ...
        from artemis_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `artemis profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"artemis -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from artemis_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("artemis_requires"):
            print(f"Requires:     Artemis {data['artemis_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from artemis_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.artemis_requires:
        print(f"  Requires: Artemis {mf.artemis_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No artemis dashboard processes running.")
        return 0

    print(f"{len(live)} artemis dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``artemis dashboard register`` for OAuth.
    ...
    try:
        from artemis_cli.web_server import should_require_auth
    ...
    try:
        from artemis_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `artemis dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    artemis dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "ARTEMIS_DASHBOARD_OAUTH_CLIENT_ID into ~/.artemis/.env for you.\n"
            "  Docs: https://artemis-agent.nousresearch.com/docs/"
    ...
    try:
        from artemis_cli.config import load_config, save_config
        from artemis_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
    ...
    try:
        from artemis_cli.plugins import discover_plugins
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from artemis_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.artemis/desktop-ssh: a
    # literal "~/.artemis/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of ARTEMIS_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_artemis_home(): a non-default sticky profile (or any ARTEMIS_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_artemis_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".artemis" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``ARTEMIS_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``artemis dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No artemis dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `artemis update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with artemis serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with ARTEMIS_DESKTOP=1 plus
    # ARTEMIS_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # ARTEMIS_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `artemis dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited ARTEMIS_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # ARTEMIS_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (ARTEMIS_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets ARTEMIS_SERVE_HEADLESS below.
    if os.environ.get("ARTEMIS_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("ARTEMIS_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("ARTEMIS_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("ARTEMIS_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → ARTEMIS_HOME points into
    ...
    try:
        from artemis_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ARTEMIS_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "artemis_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: ARTEMIS_HOME is explicitly pinned to the
        ...
        # ARTEMIS_HOME.  We must resolve the root explicitly instead of just
        # dropping ARTEMIS_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ARTEMIS_HOME=/opt/data`), so an unset
        # ARTEMIS_HOME falls back to $HOME/.artemis = /opt/data/.artemis — an
        ...
        # update-button guard also misfires).  get_default_artemis_root()
        # returns the root for both layouts: ~/.artemis for a standard install
        ...
        try:
            from artemis_constants import get_default_artemis_root
            env["ARTEMIS_HOME"] = str(get_default_artemis_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ARTEMIS_HOME) rather than block the reroute.
            env.pop("ARTEMIS_HOME", None)
    ...
    from artemis_cli.resource_limits import apply_nofile_soft_limit
    ...
    # the same logs directory as every other Artemis surface.
    try:
        from artemis_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["ARTEMIS_SERVE_HEADLESS"] = "1"
    elif "ARTEMIS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ARTEMIS_WEB_DIST"])
            if "ARTEMIS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "artemis_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ARTEMIS_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "ARTEMIS_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ARTEMIS_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ARTEMIS_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ARTEMIS_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ARTEMIS_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ARTEMIS_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ARTEMIS_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ARTEMIS_WEB_DIST: {_dist_root}")
    ...
    try:
        from artemis_cli.mcp_startup import start_background_mcp_discovery
    ...
    from artemis_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from artemis_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from artemis_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from artemis_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from artemis_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Artemis log files."""
    from artemis_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Artemis command console."""
    from artemis_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from artemis_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``artemis -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``artemis_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``artemis -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``artemis`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``artemis <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``artemis photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ARTEMIS_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ARTEMIS_YOLO_MODE is set before ANY
    ...
    try:
        from artemis_cli.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["ARTEMIS_SAFE_MODE"] = "1"
    os.environ["ARTEMIS_IGNORE_USER_CONFIG"] = "1"
    os.environ["ARTEMIS_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    ``artemis`` / ``artemis -w -s foo --yolo`` / ``artemis chat`` don't need the
    ...
    """
    if os.environ.get("ARTEMIS_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from artemis_cli.config import get_container_exec_info
    ...
    from artemis_cli._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("ARTEMIS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ARTEMIS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ARTEMIS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ARTEMIS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `artemis --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from artemis_constants import get_artemis_home, display_artemis_home
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_artemis_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_artemis_home()}/memories/\n")
    else:
        from artemis_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Artemis Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from artemis_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from artemis_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from artemis_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: artemis-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from artemis_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from artemis_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from artemis_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from artemis_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from artemis_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from artemis_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``artemis-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``ARTEMIS_AGENT`` is the Artemis-specific marker. setdefault: never
    clobber an outer harness (e.g. Artemis running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "artemis-agent")
    os.environ.setdefault("ARTEMIS_AGENT", "true")


def main():
    """Main entry point for artemis CLI."""
    # Cosmetic: make the process show up as 'artemis' instead of 'python3.11'
    ...
    try:
        from artemis_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``artemis.exe.old.*`` quarantine files left by previous
    # ``artemis update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_artemis_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (artemis update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``artemis update``
    ...
    # ``artemis skills install update``) merely defers recovery one launch;
    # under-matching (missing ``artemis -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in artemis_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from artemis_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from artemis_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://artemis-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `artemis model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.artemis/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://artemis-agent.nousresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from artemis_cli import secrets_cli as _secrets_cli
    from artemis_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `artemis proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://artemis-agent.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from artemis_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``artemis proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from artemis_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in artemis_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in artemis_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in artemis_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `artemis whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in artemis_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from artemis_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in artemis_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in artemis_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in artemis_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in artemis_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in artemis_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in artemis_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in artemis_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from artemis_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from artemis_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from artemis_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in artemis_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in artemis_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in artemis_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in artemis_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in artemis_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in artemis_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in artemis_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in artemis_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.artemis/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo artemis uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from artemis_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in artemis_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: artemis_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from artemis_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in artemis_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in artemis_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in artemis_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in artemis_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in artemis_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from artemis_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in artemis_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``artemis --help``, ``artemis version``, ``artemis logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from artemis_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from artemis_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Artemis shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from artemis_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from artemis_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in artemis_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in artemis_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `artemis computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `artemis tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `artemis computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Artemis app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from artemis_cli.tools_config import install_cua_driver
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from artemis_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: artemis computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: artemis computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: artemis computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: artemis computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: artemis computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: artemis computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in artemis_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <artemis home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in artemis_cli/sessions_cmd.py (main.py decomposition).
    ...
    # =========================================================================
    # insights command  (parser built in artemis_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in artemis_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in artemis_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in artemis_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in artemis_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in artemis_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in artemis_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in artemis_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Artemis-Setup.exe success screen tells users to
    # run `artemis desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in artemis_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in artemis_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in artemis_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``artemis -c Pokemon Agent Dev`` → ``artemis -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'artemis -c model' where 'model' is consumed as the
    ...
    # --yolo: set ARTEMIS_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (artemis hooks list, cron
    ...
    # ``artemis egress start`` refusing when credential_source=bitwarden
