"""artemis memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``artemis memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_artemis_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as artemis tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``artemis update`` heals the active memory provider
    ...
    # data volume (ARTEMIS_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'artemis memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(artemis_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.artemis/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like artemis-cli).
    ...
    if provider_name:
        ...
        if provider:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.artemis/plugins/")
