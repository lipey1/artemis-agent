...
- Keep data: Remove code but keep ~/.artemis/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Artemis PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing artemis-agent or artemis PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Artemis Agent" comment and following line
                if '# Artemis Agent' in line or '# artemis-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('artemis' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing artemis
                if 'artemis' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Artemis-owned file, and
...
def remove_wrapper_script():
    """Remove the artemis wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "artemis",
        Path.home() / ".local" / "bin" / "artemis-acp",
        Path.home() / ".local" / "bin" / "artemis-agent",
        Path("/usr/local/bin/artemis"),
        Path("/usr/local/bin/artemis-acp"),
        Path("/usr/local/bin/artemis-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains artemis_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'artemis_cli' in content or 'artemis-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(artemis_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``artemis`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Artemis home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (artemis_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``artemis gateway run`` processes
    ...
    try:
        from artemis_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from artemis_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ARTEMIS_HOME`` and ``ARTEMIS_GIT_BASH_PATH``
...
#   4. On the ``artemis dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\artemis\gateway-service\`` and sometimes
...
def _artemis_path_markers(artemis_home: Path) -> list[str]:
    """Path-entry substrings that identify Artemis-owned User-PATH entries."""
    ...
    # backslash-artemis as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(artemis_home: Path) -> list[str]:
    """Strip Artemis-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _artemis_path_markers(artemis_home)
            kept: list[str] = []
...
def remove_artemis_env_vars_windows() -> list[str]:
    """Delete ARTEMIS_HOME and ARTEMIS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ARTEMIS_HOME", "ARTEMIS_GIT_BASH_PATH"):
                try:
...
def _is_default_artemis_home(artemis_home: Path) -> bool:
    """Return True when ``artemis_home`` points at the default (non-profile) root."""
    try:
        from artemis_constants import get_default_artemis_root
        return artemis_home.resolve() == get_default_artemis_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ARTEMIS_HOME directory.

    We shell out to ``artemis -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ARTEMIS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m artemis_cli.main` so we don't depend on a `artemis`
    #    wrapper that may be half-removed mid-uninstall.
    artemis_invocation = [_sys.executable, "-m", "artemis_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                artemis_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's ARTEMIS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``artemis uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ARTEMIS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from artemis_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(artemis_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Artemis Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Artemis Chat GUI installation was found.")
        print(f"  Checked: {artemis_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Artemis agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(artemis_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Artemis agent at {artemis_home / 'artemis-agent'}")
        print(f"  • Your config, sessions, and secrets under {artemis_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Artemis agent is still installed. Run 'artemis' to use the CLI,")
    print("or 'artemis uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.artemis/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.artemis/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ARTEMIS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_artemis_home(artemis_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.artemis data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Artemis Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {artemis_home / 'config.yaml'}")
    print(f"  Secrets: {artemis_home / '.env'}")
    print(f"  Data:    {artemis_home / 'cron/'}, {artemis_home / 'sessions/'}, {artemis_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ARTEMIS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Artemis data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Artemis code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, artemis_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Artemis PATH entries from shell configs / Windows User PATH")
    print("  • Artemis wrapper scripts and Artemis-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Artemis config/data: {artemis_home}")
        if _is_default_artemis_home(artemis_home):
        ...
        print(f"  • Keep Artemis config/data: {artemis_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``artemis`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ARTEMIS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in artemis_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\artemis\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(artemis_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Artemis-owned PATH entries in User environment")

        log_info("Removing ARTEMIS_HOME / ARTEMIS_GIT_BASH_PATH User env vars...")
        removed_env = remove_artemis_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Artemis-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing artemis command...")
    ...
    #     (only when they still point into this Artemis home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Artemis-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(artemis_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Artemis-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(artemis_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ARTEMIS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from artemis_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(artemis_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.artemis/, just remove the artemis-agent subdir
            if artemis_home in project_root.parents or project_root.parent == artemis_home:
    ...
    #     under ARTEMIS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(artemis_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(artemis_home)
    ...
    # 5. Optionally remove ~/.artemis/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ARTEMIS_HOME dirs live under
        ...
        try:
            if artemis_home.exists():
                shutil.rmtree(artemis_home)
                log_success(f"Removed {artemis_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {artemis_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {artemis_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {artemis_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/lipey1/artemis-desktop/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/lipey1/artemis-desktop/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Artemis Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m artemis_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import artemis_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``artemis_constants`` + ``artemis_cli.colors``
    (and lazily ``artemis_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m artemis_cli.uninstall")
    parser.add_argument(
        "--mode",
