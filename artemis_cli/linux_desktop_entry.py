"""Install and remove the Linux desktop entry (``artemis.desktop``).

``artemis desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "artemis.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``artemis.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``artemis desktop``.

    Prefer the real ``artemis`` executable (argv[0] or PATH). When Artemis
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from artemis_cli.relaunch import resolve_artemis_bin

    bin_path = resolve_artemis_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "artemis_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Artemis\n"
        "GenericName=Artemis Desktop\n"
        "Comment=Launch Artemis Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Artemis desktop entry. Return its path.
