"""artemis webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    artemis webhook subscribe <name> [options]
    artemis webhook list
    artemis webhook remove <name>
    artemis webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.artemis/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _artemis_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: artemis gateway run
"""
...
def webhook_command(args):
    """Entry point for 'artemis webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: artemis webhook {subscribe|list|remove|test}")
        print("Run 'artemis webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (artemis gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: artemis webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from artemis webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (artemis gateway run)")
