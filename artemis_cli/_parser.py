"""
Top-level argparse construction for the artemis CLI.
...
# argparse runs (it sets ``ARTEMIS_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``artemis_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    artemis                        Start interactive chat
    artemis chat -q "Hello"        Single query mode
    artemis --tui                  Launch the modern TUI (or set display.interface: tui)
    artemis --cli                  Force the classic REPL (overrides display.interface: tui)
    artemis -c                     Resume the most recent session
    artemis -c "my project"        Resume a session by name (latest in lineage)
    artemis --resume <session_id>  Resume a specific session by ID
    artemis --resume latest        Resume the most recent session (same as -c)
    artemis --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    artemis setup                  Run setup wizard
    artemis logout                 Clear stored authentication
    artemis auth add <provider>    Add a pooled credential
    artemis auth list              List pooled credentials
    artemis auth remove <p> <t>    Remove pooled credential by index, id, or label
    artemis auth reset <provider>  Clear exhaustion status for a provider
    artemis model                  Select default model
    artemis fallback [list]        Show fallback provider chain
    artemis fallback add           Add a fallback provider (same picker as `artemis model`)
    artemis fallback remove        Remove a fallback provider from the chain
    artemis config                 View configuration
    artemis config edit            Edit config in $EDITOR
    artemis config set model gpt-4 Set a config value
    artemis gateway                Run messaging gateway
    artemis -s artemis-agent-dev,github-auth
    artemis -w                     Start in isolated git worktree
    artemis gateway install        Install gateway background service
    artemis sessions list          List past sessions
    artemis sessions browse        Interactive session picker
    artemis sessions rename ID T   Rename/title a session
    artemis logs                   View agent.log (last 50 lines)
    artemis logs -f                Follow agent.log in real time
    artemis logs errors            View errors.log
    artemis logs --since 1h        Lines from the last hour
    artemis debug share             Upload debug report for support
    artemis console                Open the safe Artemis command console
    artemis update                 Update to latest version
    artemis dashboard              Start web UI dashboard (port 9119)
    artemis dashboard --stop       Stop running dashboard processes
    artemis dashboard --status     List running dashboard processes

For more help on a command:
    artemis <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="artemis",
        description="Artemis Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `artemis chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ARTEMIS_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `artemis setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to ARTEMIS_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.artemis/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Artemis Agent",
    )
    ...
    # top-level parser: when the user writes `artemis -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/artemis_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also ARTEMIS_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.artemis/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Artemis itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
