"""Dependency-light venv recovery that runs BEFORE artemis_cli.main's imports.

The ``artemis`` console entry point is ``artemis_cli.main:main``.  Importing
``artemis_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``artemis_cli.env_loader``, ``yaml`` via ``artemis_cli.config``, ...).  In
...
a corrupted venv.  ``artemis_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``artemis_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``artemis acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``artemis_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``artemis_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``artemis update`` exists AND an import probe
