"""
Backup and import commands for artemis CLI.

`artemis backup` creates a zip archive of the entire ~/.artemis/ directory
(excluding the artemis-agent repo and transient files).

`artemis import` restores from a backup zip, overlaying onto the current
ARTEMIS_HOME root.
"""
...
from artemis_constants import get_default_artemis_root, get_artemis_home, display_artemis_home

# Shared formatter; the private alias is kept because claw.py and the backup
# tests import ``_format_size`` from this module.
from artemis_cli.sizefmt import format_bytes as _format_size
...
# ``artemis-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/artemis-agent/``
...
# ARTEMIS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "artemis-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ARTEMIS_HOME) —
...
# File names that ``artemis import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ARTEMIS_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Artemis backup slot."""
...
@contextmanager
def _backup_operation_lock(artemis_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = artemis_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Artemis backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ARTEMIS_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to artemis root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``artemis-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/artemis-agent/`` — must be preserved.
        if part == "artemis-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ARTEMIS_HOME.
...
# to the user as a hung `artemis update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # artemis_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Artemis home directory."""
    artemis_root = get_default_artemis_root()

    if not artemis_root.is_dir():
        print(f"Error: Artemis home directory not found at {artemis_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(artemis_root):
            _run_backup_locked(args, artemis_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, artemis_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"artemis-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"artemis-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_artemis_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(artemis_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(artemis_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``artemis-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "artemis-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(artemis_root)
    ...
    # outside ARTEMIS_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_artemis_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: artemis import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Artemis backup.
    ...
    # Look for telltale files that a artemis home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Artemis backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.artemis/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a artemis dir name
        if prefix in {".artemis", "artemis"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_artemis_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (artemis_root / "config.yaml").exists()
        has_env = (artemis_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Artemis configuration.")
        ...
        artemis_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ARTEMIS_HOME.
            ...
            target = artemis_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(artemis_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_artemis_home()}."
            )
        ...
        profiles_dir = artemis_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # artemis_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: artemis profile list  (after installing artemis)")

        # Guidance
        print()
        if not (artemis_root / "artemis-agent").is_dir():
            print("Note: The artemis-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: artemis update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  artemis -p {pname} gateway install")

        print("Done. Your Artemis configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and artemis backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ARTEMIS_HOME).
...
# — `artemis update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ARTEMIS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ARTEMIS_HOME and the entry is silently
...
def _quick_snapshot_root(artemis_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: artemis snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(artemis_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ARTEMIS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``artemis update``.
    ...
    Args:
        ...
        artemis_home: Override for the Artemis home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(artemis_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for artemis backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_artemis_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, artemis_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(artemis_root):
            return _write_full_zip_backup_locked(out_path, artemis_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, artemis_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``artemis_root`` to ``out_path``.
...
def _pre_update_backup_dir(artemis_home: Optional[Path] = None) -> Path:
    home = artemis_home or get_artemis_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ARTEMIS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ARTEMIS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``artemis update``) should continue even if the backup fails.
    """
    artemis_root = artemis_home or get_default_artemis_root()
    if not artemis_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(artemis_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, artemis_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `artemis claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ARTEMIS_HOME under ``backups/`` before a
    ``artemis claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``artemis import <archive>``.  Writes to
    ``<ARTEMIS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `artemis import` and the
