...
``<root>`` is the **shared Artemis root** (the parent of any active
...
``artemis -p <profile>`` joins the same board as the dispatcher that
...
the profile's Artemis home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ARTEMIS_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ARTEMIS_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``artemis kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.artemis``. In Docker / custom
deployments where ``ARTEMIS_HOME`` points outside ``~/.artemis`` (e.g.
``/opt/artemis``), ``<root>`` is ``ARTEMIS_HOME``. Legacy env-var
overrides still work:

* ``ARTEMIS_KANBAN_DB`` — pin the database file path directly.
* ``ARTEMIS_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ARTEMIS_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ARTEMIS_KANBAN_DB``,
``ARTEMIS_KANBAN_WORKSPACES_ROOT``, and ``ARTEMIS_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/artemis-kanban-v1-spec.pdf`` for the full
...
from artemis_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``artemis_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from artemis_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("ARTEMIS_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active ARTEMIS_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from artemis_cli.lifecycle import invoke_hook
...
# ``ARTEMIS_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ARTEMIS_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ARTEMIS_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ARTEMIS_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ARTEMIS_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ARTEMIS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "ARTEMIS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "artemis_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``artemis-agent``
...
def kanban_home() -> Path:
    """Return the shared Artemis root that anchors the kanban board.

    Resolution order:

    1. ``ARTEMIS_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_artemis_root()``, which already returns ``<root>``
       when ``ARTEMIS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ARTEMIS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ARTEMIS_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ARTEMIS_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from artemis_constants import get_default_artemis_root
    return get_default_artemis_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``artemis kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ARTEMIS_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``artemis kanban boards
    ...
    env = os.environ.get("ARTEMIS_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``artemis kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ARTEMIS_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ARTEMIS_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ARTEMIS_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ARTEMIS_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ARTEMIS_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("ARTEMIS_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``artemis kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``artemis_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``ARTEMIS_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (artemis_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ARTEMIS_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ARTEMIS_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``artemis_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from artemis_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``artemis gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from artemis_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see artemis_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from artemis_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Artemis treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``artemis kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ARTEMIS_KANBAN_DB`` env → ``ARTEMIS_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `artemis kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from artemis_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from artemis_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See artemis_state._WAL_INCOMPAT_MARKERS for detection logic.
                from artemis_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``artemis kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ARTEMIS_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from artemis_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"artemis_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``artemis --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from artemis_cli import projects_db as _pdb
    ...
    # `artemis --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``artemis kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `artemis kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``artemis kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``artemis kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``artemis kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``artemis kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``artemis kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ARTEMIS_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Artemis' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Artemis-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``artemis kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Artemis checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Artemis treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Artemis creates/reuses that linked worktree. With
      no ``workspace_path``, Artemis anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Artemis uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``ARTEMIS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Artemis
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``artemis kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``artemis kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Artemis profile.
    ...
    if not rows:
        return False
    try:
        from artemis_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Artemis ships the ``sdlc-review`` skill and the
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from artemis_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Artemis profile.
        # `_default_spawn` invokes ``artemis -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Artemis
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `artemis kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Artemis bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from artemis_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_artemis_argv() -> list[str]:
    """Return the interpreter-bound Artemis CLI invocation."""
    # ``artemis_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``artemis`` package — there is no
    # ``artemis`` package to import.
    return [sys.executable, "-m", "artemis_cli.main"]


def _absolute_artemis_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Artemis shim."""
...
def _artemis_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Artemis executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_artemis_argv()
    return [_absolute_artemis_path(path)]


def _resolve_artemis_argv() -> list[str]:
    """Resolve the ``artemis`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ARTEMIS_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("artemis")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\artemis.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m artemis_cli.main`` — fallback for setups where
       Artemis is launched from a venv and the ``artemis`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_artemis_bin`` for the same reason. Kept
    local (not imported from gateway) because ``artemis_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ARTEMIS_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _artemis_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _artemis_path_argv(resolved_env_bin)
        return _module_artemis_argv()

    artemis_bin = _safe_which_no_cwd("artemis") if _IS_WINDOWS else shutil.which("artemis")
    if artemis_bin:
        return _artemis_path_argv(artemis_bin)
    return _module_artemis_argv()
...
def _resolve_worker_cli_toolsets(artemis_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``ARTEMIS_KANBAN_TASK`` is set.
    """
    if not artemis_home:
        return None
    try:
        ...
        token = set_artemis_home_override(artemis_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for ARTEMIS_HOME=%r (%s)",
            artemis_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``artemis -p <profile> chat -q ...`` subprocess.
    ...
    ``ARTEMIS_KANBAN_DB`` / ``ARTEMIS_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ARTEMIS_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `artemis -p <name>` the
    # _apply_profile_override() runs *before* artemis_constants is imported.
    # If ARTEMIS_HOME is absent from the child's env, get_artemis_home() falls
    # back to Path.home() / ".artemis" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from artemis_cli.profiles import resolve_profile_env
    try:
        env["ARTEMIS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ARTEMIS_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ARTEMIS_HOME never had profiles created.
        pass
    if task.tenant:
        env["ARTEMIS_TENANT"] = task.tenant
    env["ARTEMIS_KANBAN_TASK"] = task.id
    env["ARTEMIS_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `artemis kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["ARTEMIS_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ARTEMIS_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ARTEMIS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ARTEMIS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["ARTEMIS_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ARTEMIS_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`artemis -p <name>`
    # rewrites ARTEMIS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_artemis_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ARTEMIS_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ARTEMIS_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ARTEMIS_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ARTEMIS_KANBAN_BOARD"] = resolved_board
    # ARTEMIS_PROFILE is the author the kanban_comment tool defaults to.
    # `artemis -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ARTEMIS_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited ARTEMIS_TUI=1
    ...
    # older artemis builds on PATH that predate the flag's precedence.
    env.pop("ARTEMIS_TUI", None)

    cmd = [
        *_resolve_artemis_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped ARTEMIS_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("ARTEMIS_HOME"))
    if worker_toolsets:
    ...
    # `artemis kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`artemis` executable not found on PATH. "
            "Install Artemis Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``artemis kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ARTEMIS_PROFILE values like "artemis-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Artemis root exists

    Reads profile paths directly so this module has no import dependency on
    ``artemis_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from artemis_constants import get_default_artemis_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``artemis kanban assignees`` for the terminal.
