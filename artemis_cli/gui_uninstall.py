"""
Artemis Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``artemis desktop`` / ``artemis gui``)
     Built inside the agent checkout under ``$ARTEMIS_HOME/artemis-agent/``:
       ...
       - ``$ARTEMIS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Artemis.app`` or ``~/Applications/Artemis.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Artemis`` (NSIS per-user)
...
the app name ("Artemis"), separate from ``$ARTEMIS_HOME``:
  - macOS:   ``~/Library/Application Support/Artemis``
  - Windows: ``%APPDATA%\\Artemis``
  - Linux:   ``$XDG_CONFIG_HOME/Artemis`` (default ``~/.config/Artemis``)
...
``artemis uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(artemis_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return artemis_home / "artemis-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Artemis"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Artemis"
...
def source_built_gui_artifacts(artemis_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``artemis desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``artemis-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(artemis_home)
    ...
    return [
        ...
        artemis_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Artemis" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Artemis.app"),
            home / "Applications" / "Artemis.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Artemis).
            local_base / "Programs" / "Artemis",
            # Older / alternate layout some builds used.
            local_base / "artemis-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Artemis")
    else:
        ...
        # ``uninstall_gui``.
        from artemis_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `artemis desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Artemis.desktop",
        ]
    return paths


def agent_is_installed(artemis_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ARTEMIS_HOME.
    ...
    if (agent_root / "artemis_cli").is_dir():
        return True
...
def gui_is_installed(artemis_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(artemis_home):
        if p.exists():
            return True
...
def gui_install_summary(artemis_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = artemis_home if artemis_home is not None else get_artemis_home()
...
def uninstall_gui(artemis_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``artemis-agent/artemis_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ARTEMIS_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Artemis disappears from the launcher.
        try:
            from artemis_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove artemis' or "
            "'sudo dnf remove artemis'). AppImage builds are a single file you "
