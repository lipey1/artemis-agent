"""CLI subcommand: ``artemis send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"artemis send: {file_path} is not a text file. --file reads the "
                ...
                f'  artemis send --to telegram "MEDIA:{file_path}"\n'
                f'  artemis send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  artemis send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"artemis send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"artemis send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"artemis send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"artemis send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `artemis send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"artemis send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `artemis gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.artemis/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_artemis_env() -> None:
    """Populate ``os.environ`` from ``~/.artemis/.env`` AND bridge top-level
    ...
    startup that ``artemis send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.artemis/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.artemis/config.yaml`` into
       ...
       via ``artemis config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``artemis send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.artemis/.env and ~/.artemis/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "artemis send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  artemis send --to telegram \"hello\"\n"
            "  artemis send --to discord:#ops --file report.md\n"
            "  artemis send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "artemis send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `artemis send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Artemis "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.artemis/.env + ~/.artemis/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  artemis send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | artemis send --to telegram:-1001234567890\n"
            "  artemis send --to discord:#ops --file /tmp/report.md\n"
            "  artemis send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  artemis send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  artemis send --list                  # all platforms\n"
            "  artemis send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `artemis send --list telegram`.",
    )
