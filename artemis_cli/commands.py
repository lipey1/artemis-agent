"""Slash command definitions and autocomplete for the Artemis CLI.
...
from artemis_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``artemis_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``artemis_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Artemis config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Artemis works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Artemis' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Artemis is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.artemis/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Artemis Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Artemis version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`artemis_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/artemis`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Artemis ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Artemis
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``artemis skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``artemis skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``artemis skills list`` and
...
# unrelated command landed. These claim their slots right after /artemis,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /artemis <command>).
...
# native slot, the alias spelling stays reachable via /artemis reset).
...
# ``/artemis <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/artemis" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /artemis topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /artemis moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /artemis debug on Slack.
#   - egress: Docker-only proxy status; reachable as /artemis egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /artemis init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /artemis version on
...
#   - diff: git working-tree diff; reached via /artemis diff on Slack so it
...
#     /artemis update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /artemis heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /artemis refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /artemis pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
_SLACK_VIA_ARTEMIS_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/artemis <verb>`` subcommand.
    ...
    can still reach them via ``/artemis <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/artemis`` is always reserved as the first entry so the
    legacy ``/artemis <subcommand>`` form keeps working for anything that
    ...
    # Reserve /artemis as the catch-all top-level command.
    entries.append(("artemis", "Talk to Artemis or run a subcommand", "[subcommand] [args]"))
    seen.add("artemis")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ARTEMIS_ONLY:
            # Intentionally Slack-via-/artemis only (see _SLACK_VIA_ARTEMIS_ONLY).
            return
    ...
    # everything except /artemis, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://artemis-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /artemis handler.

    Maps both canonical names and aliases so /artemis bg do stuff works
    the same as /artemis background do stuff.

    Plugin-registered slash commands are included so ``/artemis <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from artemis_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via artemis_cli.personality."""
        try:
