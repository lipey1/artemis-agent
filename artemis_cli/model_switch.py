...
- ``artemis_cli.providers``        -- canonical provider identity + overlays
- ``artemis_cli.model_normalize``  -- per-provider name formatting
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``artemis model`` wizard — not a
    ...
    while ``artemis model`` still live-probed the full ``/v1/models`` list.
...
_ARTEMIS_MODEL_WARNING = (
    "Nous Research Artemis 3 & 4 models are NOT agentic and are not designed "
    "for use with Artemis Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Artemis 3 / Artemis 4 chat families.
# The previous substring check (`"artemis" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``artemis-brain:qwen3-14b-ctx16k`` that just
# happen to carry "artemis" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Artemis-3-Llama-3.1-70B, artemis-4-405b, openrouter/artemis3:70b
# Negative examples it must NOT match:
#   artemis-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ARTEMIS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])artemis[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_artemis_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Artemis 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ARTEMIS_NON_AGENTIC_RE.search(model_name))


def _check_artemis_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Artemis 3/4 chat model."""
    if is_nous_artemis_non_agentic(model_name):
        return _ARTEMIS_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``artemis config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from artemis_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``artemis_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'artemis model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'artemis doctor' — config issues detected:"
        ...
        from artemis_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from artemis_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from artemis_cli.config import load_config
            from artemis_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from artemis_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from artemis_cli.providers import nous_api_mode
    ...
    # artemis_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from artemis_cli.models import normalize_opencode_base_url
    ...
    artemis_warn = _check_artemis_model_warning(new_model)
    if artemis_warn:
        warnings.append(artemis_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from artemis_cli.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from artemis_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against artemis_id / mdev_id (section 1), pid / artemis_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from artemis_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by artemis provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://artemis-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Artemis release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from artemis_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from artemis_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Artemis-mapped providers ---
    from artemis_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from artemis_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for artemis_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # same artemis_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if artemis_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(artemis_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ARTEMIS_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Artemis may not route yet.
        ...
        from artemis_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(artemis_id):
            continue
        ...
        if not has_creds:
            try:
                from artemis_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(artemis_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        artemis_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `artemis model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(artemis_id)
        if not model_ids:
            model_ids = curated.get(artemis_id, [])
            if artemis_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(artemis_id, model_ids)
        ...
        results.append({
            ...
            "is_current": (
                ...
                or artemis_id == current_provider
    ...
    # --- 2. Check Artemis-only providers (nous, openai-codex, copilot, opencode-go) ---
    from artemis_cli.providers import ARTEMIS_OVERLAYS
    from artemis_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Artemis provider ID.
    # ARTEMIS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Artemis IDs ("copilot").
    _mdev_to_artemis = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ARTEMIS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Artemis slug — e.g. "github-copilot" → "copilot"
        artemis_slug = _mdev_to_artemis.get(pid, pid)
        if artemis_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or artemis_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(artemis_slug)
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or artemis_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(artemis_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(artemis_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", artemis_slug, exc)
        ...
        # configured.
        if not has_creds and artemis_slug == "anthropic":
            try:
                ...
                if (artemis_creds and artemis_creds.get("accessToken")) or \
        ...
        if artemis_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(artemis_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(artemis_slug)
                model_ids = _ids if _ids else (curated.get(artemis_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(artemis_slug, []) or curated.get(pid, [])
        elif artemis_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `artemis model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from artemis_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(artemis_slug, []) or curated.get(pid, [])
                if artemis_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(artemis_slug, model_ids)
        total = len(model_ids)
        if artemis_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": artemis_slug,
            "name": get_label(artemis_slug),
            "is_current": artemis_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "artemis",
        ...
        seen_slugs.add(artemis_slug.lower())
        _record_builtin_endpoint(artemis_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ARTEMIS_OVERLAYS (keeps /model in sync
    # with `artemis model`).
    try:
        from artemis_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Artemis writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Artemis's own writer uses (mirrors section-4 grouping).
            ...
            # ``artemis model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``artemis model``. Pin with
            ...
            if _discovery_allowed:
                try:
                    from artemis_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Artemis's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, artemis_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``artemis model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from artemis_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`artemis_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from artemis_cli.models import fetch_openrouter_models
