"""``artemis sessions`` command — extracted from ``artemis_cli/main.py``.
...
Helpers that stay in ``artemis_cli.main`` (``get_artemis_home``,
...
``artemis_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_artemis_home():
    return _m().get_artemis_home()
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    artemis sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from artemis_cli.session_recovery import (
    ...
    if action == "list":
        from artemis_state import workspace_key as _ws_key
        ...
        from artemis_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        def _redact(data):
            if not args.redact or data is None:
                return data
            from artemis_cli.session_export_md import redact_session_data
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from artemis_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_artemis_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from artemis_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_artemis_home() / "session-exports"
        ...
        # `artemis sessions prune`: no time window and no filters at all
        ...
        # Launch artemis --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`artemis sessions optimize` later to reclaim freed space.)")
