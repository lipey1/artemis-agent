...
on Japanese locales, etc.).  Artemis's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``artemis_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Artemis is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ARTEMIS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ARTEMIS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Artemis-managed Git install directories so
    ...
    # already-running shells don't see the change — which means artemis
    ...
    # install without Artemis-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Artemis's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Artemis's
    ``artemis config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Artemis (User env var in Windows Settings, or
    export in a PowerShell profile) and Artemis picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Artemis-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\artemis\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``artemis`` from right after install) retain their old PATH.

    Any subprocess Artemis spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Artemis-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Artemis venv Scripts directory — host of the artemis.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes artemis, but harmless to include.
        os.path.join(local_appdata, "artemis", "artemis-agent", "venv", "Scripts"),
