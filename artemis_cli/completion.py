"""Shell completion script generation for artemis CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_artemis_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Artemis Agent bash completion
# Add to ~/.bashrc:
#   eval "$(artemis completion bash)"

_artemis_profiles() {{
    local profiles_dir="$HOME/.artemis/profiles"
...
_artemis_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_artemis_profiles)" -- "$cur"))
        return
    fi
...
complete -F _artemis_completion artemis
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef artemis
# Artemis Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(artemis completion zsh)"

_artemis_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.artemis/profiles" ]]; then
        profiles+=($HOME/.artemis/profiles/*(N/:t))
    fi
...
_artemis() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_artemis_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'artemis command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Artemis Agent fish completion",
        "# Add to your config:",
        "#   artemis completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __artemis_profiles",
        "    echo default",
        "    if test -d $HOME/.artemis/profiles",
        "        for d in $HOME/.artemis/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c artemis -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c artemis -f -s p -l profile"
        " -d 'Profile name' -xa '(__artemis_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__artemis_profiles)' -d 'Profile name'"
                )
