"""Pydantic request/response models for the Artemis dashboard web server.

Extracted verbatim from ``artemis_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from artemis_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``artemis model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `artemis import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`artemis -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ARTEMIS_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under ARTEMIS_HOME.
    output: str = ""
