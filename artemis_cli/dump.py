"""
Dump command for artemis CLI.

Outputs a compact, plain-text summary of the user's Artemis setup
...
from artemis_cli.config import get_artemis_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.artemis/.env.
    ...
    exports. ``artemis debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.artemis_build_sha`` by the Dockerfile's
    ``ARTEMIS_GIT_SHA`` build-arg (see ``artemis_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``artemis dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from artemis_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(artemis_home: Path) -> int:
...
def _cron_summary(artemis_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = artemis_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"artemis_home:      {display_artemis_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.artemis/.env: a managed
        ...
        # A credential added via `artemis auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `artemis auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["artemis-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(artemis_home)}")
    lines.append(f"  skills:             {_count_skills(artemis_home)}")
