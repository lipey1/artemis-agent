...
remains ``artemis cron`` / the ``cronjob`` tool, which runs in isolated
...
def _get_session_db() -> Optional[Any]:
    # Reuse the goals module's per-ARTEMIS_HOME cached SessionDB so both
    # features share one connection instead of thrashing the file.
    try:
        from artemis_cli.goals import _get_session_db as _goals_db
