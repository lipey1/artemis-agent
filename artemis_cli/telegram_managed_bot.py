...
manual BotFather token copy-paste. Artemis talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.artemis-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "ArtemisSetupBot"

DEFAULT_BOT_NAME = "Artemis Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``artemis_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``artemis_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"artemis_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Artemis Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Artemis Telegram onboarding service.")
