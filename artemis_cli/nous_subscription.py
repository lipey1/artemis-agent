...
from artemis_cli.config import get_env_value, load_config
...
# coverage category (artemis_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `artemis tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    from artemis_constants import agent_browser_runnable, with_artemis_node_path
    ...
    # Artemis-managed Node dirs (Windows installer / POSIX $ARTEMIS_HOME/node)
    ...
    managed_path = with_artemis_node_path().get("PATH", "")
    if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("ARTEMIS_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `artemis model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("ARTEMIS_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("ARTEMIS_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from artemis_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`artemis tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``artemis tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``artemis model``'s Nous
    login, this:
