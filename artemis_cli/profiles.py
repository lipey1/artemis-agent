"""
Profile management for multiple isolated Artemis instances.

Each profile is a fully independent ARTEMIS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.artemis/profiles/<name>/`` by default.

The "default" profile is ``~/.artemis`` itself — backward compatible,
zero migration needed.

Usage::

    artemis profile create coder          # fresh profile + bundled skills
    artemis profile create coder --clone  # also copy config, .env, SOUL.md, skills
    artemis profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    artemis -p coder chat                 # or via flag
    artemis profile use coder             # set as sticky default
    artemis profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See artemis_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.artemis``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   artemis-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `artemis backup` archives
...
# Marker file written by `artemis profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `artemis
...
# `artemis skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Artemis infrastructure directories that only the default profile
         (``~/.artemis``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_artemis_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.artemis) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "artemis-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ARTEMIS_HOME equals the
...
# into the export. The set below identifies the *known Artemis profile
# artifacts* at the root of ARTEMIS_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``artemis_constants``.
...
_RESERVED_NAMES = frozenset({
    "artemis", "default", "test", "tmp", "root", "sudo",
})

# Artemis subcommands that cannot be used as profile names/aliases
_ARTEMIS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the artemis root, NOT to the current ARTEMIS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ARTEMIS_HOME points outside
    ``~/.artemis``, profiles live under ``ARTEMIS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_artemis_home() / "profiles"


def _get_default_artemis_home() -> Path:
    """Return the default (pre-profile) ARTEMIS_HOME path.

    In standard deployments this is ``~/.artemis``.
    In Docker/custom deployments where ARTEMIS_HOME is outside ``~/.artemis``
    (e.g. ``/opt/data``), returns ARTEMIS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_artemis_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Artemis installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ARTEMIS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_artemis_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, artemis subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ARTEMIS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a artemis subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            artemis_exe = shutil.which("artemis") or "artemis"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(artemis_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Artemis. If we
    ...
    try:
        ...
        # not be migrated. The next `artemis doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``artemis -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``artemis profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``artemis -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``artemis profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ARTEMIS_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Artemis config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_artemis_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, artemis_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ARTEMIS_HOME. When
    ...
    The returned ``artemis_home`` is the path to pass to
    ``set_artemis_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_artemis_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``artemis update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.artemis)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.artemis/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `artemis -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Artemis profile.\n"
    ...
    if not soul_path.exists():
        try:
            from artemis_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `artemis update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`artemis profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Artemis (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `artemis profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `artemis -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ARTEMIS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``artemis profile create
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_artemis_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Artemis *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Artemis backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ARTEMIS_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `artemis -p <canon> profile
    ...
    artemis_markers = ("artemis_cli.main", "artemis-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Artemis process: either an entrypoint marker in argv, or
            # a resolved executable named `artemis`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_artemis = (
                any(marker in joined for marker in artemis_markers)
                or exe_name == "artemis"
                or exe_name.startswith("artemis")
            )
            if not is_artemis:
                continue
            ...
            # ...or by ARTEMIS_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ARTEMIS_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.artemis).\n"
            "To remove everything, use: artemis uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_artemis_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ARTEMIS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from artemis_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ARTEMIS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ARTEMIS_HOME")
    try:
        os.environ["ARTEMIS_HOME"] = str(profile_dir)
        from artemis_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ARTEMIS_HOME"] = old_home
        elif "ARTEMIS_HOME" in os.environ:
            del os.environ["ARTEMIS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.artemis/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: artemis profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ARTEMIS_HOME.

    Returns ``"default"`` if ARTEMIS_HOME is not set or points to ``~/.artemis``.
    Returns the profile name if ARTEMIS_HOME points into ``~/.artemis/profiles/<name>``.
    Returns ``"custom"`` if ARTEMIS_HOME is set to an unrecognized path.
    """
    from artemis_constants import get_artemis_home
    artemis_home = get_artemis_home()
    resolved = artemis_home.resolve()

    default_resolved = _get_default_artemis_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ARTEMIS_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Artemis file the
      user may have lying alongside ARTEMIS_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``artemis sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``ARTEMIS_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.artemis itself — its parent is ~/ and its
        # directory name is ".artemis", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: artemis profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.artemis itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.artemis). "
            "Specify a different name: artemis profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="artemis_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"artemis_{old_name}"
    legacy_old_host = f"artemis.{old_name}"
    new_host = f"artemis_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_artemis_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("artemis_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ARTEMIS_HOME path string.

    Called early in the CLI entry point, before any artemis modules
    are imported, to set the ARTEMIS_HOME environment variable.
    """
