...
``artemis_cli.config.migrate_config``. Each step is a function
...
``artemis_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``artemis_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``artemis_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``artemis_cli.config`` (e.g.
``patch("artemis_cli.config.read_raw_config", ...)``) keep working, because
...
#: ARTEMIS_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_artemis_home()}/config.yaml and run `artemis setup` to "
...
def _cfg():
    """Return the live ``artemis_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_artemis_home = _c.get_artemis_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$ARTEMIS_HOME/plugins/`` for currently installed user plugins.
        grandfathered: List[str] = []
        try:
            user_plugins_dir = get_artemis_home() / "plugins"
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `artemis plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `artemis curator status` has no
    ...
    #   3. Creates `~/.artemis/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_artemis_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_artemis_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `artemis config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `artemis config set`"
                )
...
# behavior set it to true explicitly via `artemis config set`.
