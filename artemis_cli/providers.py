"""
Single source of truth for provider identity in Artemis Agent.
...
2. **Artemis overlays** — transport type, auth patterns, aggregator flags,
...
# -- Artemis overlay ----------------------------------------------------------
# Artemis-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ArtemisOverlay:
    """Artemis-specific provider metadata layered on top of models.dev."""
...
ARTEMIS_OVERLAYS: Dict[str, ArtemisOverlay] = {
    "moa": ArtemisOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": ArtemisOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": ArtemisOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": ArtemisOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ArtemisOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ArtemisOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ARTEMIS_QWEN_BASE_URL",
    ),
    "lmstudio": ArtemisOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ArtemisOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ArtemisOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ArtemisOverlay(
    ...
    "kimi-for-coding": ArtemisOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ArtemisOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ArtemisOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ArtemisOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ArtemisOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ArtemisOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ArtemisOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ArtemisOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": ArtemisOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": ArtemisOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ArtemisOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": ArtemisOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": ArtemisOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": ArtemisOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": ArtemisOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": ArtemisOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": ArtemisOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": ArtemisOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": ArtemisOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "artemis", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Artemis overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Artemis overlay
    ...
    overlay = ARTEMIS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + artemis extra
    ...
    if overlay is not None:
        # Artemis-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Artemis overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in ARTEMIS_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Artemis provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from artemis_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
