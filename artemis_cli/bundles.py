"""Implementation of the ``artemis bundles`` CLI subcommand.

Mirrors the structure of ``artemis_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `artemis bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  artemis bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``artemis bundles`` argparse tree.

    Called from ``artemis_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `artemis bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``artemis bundles <subcommand>`` to the right handler."""
