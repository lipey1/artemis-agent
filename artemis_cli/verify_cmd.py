"""``artemis verify`` — detect a project's run recipe and smoke-test it.
...
``.artemis/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``artemis verify`` satisfies
