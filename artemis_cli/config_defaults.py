"""Default configuration data for Artemis Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from artemis_cli/config.py. Must not import from artemis_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["artemis-cli"],
    # SQLite journal mode used by every Artemis database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Artemis server processes.
    ...
    "agent": {
        ...
        # the Artemis-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Artemis
        ...
        # identity slot (SOUL.md). Empty by default. The ARTEMIS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Artemis adds a coding operating brief
        ...
        # ``ARTEMIS_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             ARTEMIS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ARTEMIS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Artemis auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Artemis sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Artemis
        ...
        #  "/home/user/.artemis/cache/documents:/output"]
        ...
        # bundled Artemis image, which drops to the `artemis` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Artemis sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Artemis should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``artemis chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.artemis/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: artemis sweeps the checkpoint base at startup
        ...
        # ``artemis checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .artemis.md, or .cursorrules before Artemis applies
    ...
    # Single-query (``artemis -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Artemis truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Artemis' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); artemis =
                                      # Artemis' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Artemis' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Artemis does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``artemis kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``artemis kanban decompose`` and the kanban
        ...
        # ``artemis profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `artemis model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `artemis` (and `artemis chat`) launches by default:
        ...
        # REPL and `--tui` (or ARTEMIS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `artemis --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `artemis -c` muscle memory.  Default off so existing
        # users aren't surprised.  ARTEMIS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `artemis --tui` drops a one-time hint
        ...
        # model — see artemis_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to ARTEMIS_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/artemis`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `artemis pets`. Disabled until
        ...
        "pet": {
            ...
            # get_artemis_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``ARTEMIS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ARTEMIS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ARTEMIS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ARTEMIS_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ARTEMIS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/artemis``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Artemis appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.artemis/cache/piper-voices/
    ...
    # "Hey Artemis" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey artemis",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_artemis" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.artemis/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.artemis/cache/delegation/
    ...
    # response. If not, Artemis feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Artemis auto-pauses the goal and
    ...
    "moa": {
        ...
        # <artemis_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.artemis/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ARTEMIS_SKILL_DIR} and ${ARTEMIS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `artemis curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `artemis curator run --consolidate` overrides
        ...
        # normally restored on every `artemis update`, so pruning them only
        ...
        # skipped), snapshot ~/.artemis/skills/ into
        # ~/.artemis/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `artemis curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for artemis-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Artemis Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ARTEMIS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Artemis' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.artemis/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ARTEMIS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .artemis config) always require human
        ...
        # longer trigger the startup banner. Add via `artemis doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``artemis_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Artemis to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ARTEMIS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ARTEMIS_CRON_MAX_PARALLEL env var.
        ...
        # ARTEMIS_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `artemis -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `artemis` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `artemis kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     artemis-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Artemis tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.artemis/logs/.
    ...
    # update model picker lists without shipping a artemis-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://artemis-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `artemis model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal ARTEMIS_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.artemis/sessions/sessions.json entirely.
        ...
        # the ARTEMIS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.artemis.gateway` that defenses 1-2 don't
        ...
        # `artemis update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``ARTEMIS_GATEWAY_MAX_STARTS`` / ``ARTEMIS_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.artemis/.env,
        ...
        # behavior: files must live under the Artemis cache, under
        ...
        # user. Bridged to ARTEMIS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Artemis
        # cache (~/.artemis/cache/{documents,images,audio,video,screenshots})
        ...
        # to ARTEMIS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ARTEMIS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ARTEMIS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.artemis/state.db.
    ...
    "sessions": {
        ...
        # the default of ``artemis sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.artemis/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `artemis sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `artemis update` prints a one-line notice with
        ...
        # ~/.artemis/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ARTEMIS_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ARTEMIS_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `artemis sessions export`. Checked
    ...
    # ``artemis doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `artemis doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``artemis update`` behaviour.
    "updates": {
        ...
        #     DBs) into <ARTEMIS_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``artemis backup``-style zip
        #     of ARTEMIS_HOME into <ARTEMIS_HOME>/backups/, restorable with
        #     ``artemis import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `artemis update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ARTEMIS_HOME>/lsp/bin/`` on first use.
    ...
    # `artemis tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.artemis/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.artemis/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.artemis/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Artemis may use this encrypted
            ...
            # ~/.artemis/bin/ on first use.  When False you must install
            ...
            # `artemis secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.artemis/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Artemis disables it for our users unless they
        # explicitly opt in here. When false (default), Artemis sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Artemis also calls
    ...
    #
    # Configure with `artemis egress setup`.  Disabled by default — the rest of
    # Artemis works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.artemis/bin/ on
    ...
    # Artemis Desktop (Electron app) launch options. These only affect
    # `artemis desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ARTEMIS_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Artemis mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'artemis model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Artemis auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Artemis' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Artemis, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @artemis:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ARTEMIS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #artemis)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: artemis-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'artemis-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Artemis API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Artemis API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Artemis API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ARTEMIS_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ARTEMIS_TOOL_PROGRESS_MODE for backward
    ...
    # ARTEMIS_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "ARTEMIS_PREFILL_MESSAGES_FILE": {
    ...
    "ARTEMIS_EPHEMERAL_SYSTEM_PROMPT": {
