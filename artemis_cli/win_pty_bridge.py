"""Windows ConPTY bridge for the `artemis dashboard` chat tab.

Drop-in counterpart to ``artemis_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``artemis_cli.web_server`` consumes: ``spawn``, ``read``,
