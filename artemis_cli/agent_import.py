"""artemis import-agent — import Claude Code / Codex CLI setups into Artemis.

Usage:
    artemis import-agent                       # auto-detect ~/.claude or ~/.codex
    artemis import-agent claude-code           # import from ~/.claude
    artemis import-agent codex                 # import from ~/.codex
    artemis import-agent claude-code --dry-run # preview only, no changes
    artemis import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``artemis claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_artemis.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ARTEMIS_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ARTEMIS_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ARTEMIS_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ARTEMIS_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ARTEMIS_HOME/skills/codex-imports/<name>/
...
deliberately via ``artemis setup`` or config.yaml.
"""
...
# Same entry delimiter as the Artemis memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `artemis config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`artemis config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_artemis.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Artemis command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Artemis glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Artemis equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Artemis config")
                continue

            artemis_srv: Dict[str, Any] = {}
            if srv.get("command"):
                artemis_srv["command"] = srv["command"]
                if srv.get("args"):
                    artemis_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    artemis_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    artemis_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                artemis_srv["url"] = srv["url"]
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        artemis_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ARTEMIS_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``artemis import-agent`` (invoked from artemis_cli.main)."""
    from artemis_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: artemis import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: artemis import-agent claude-code   or   artemis import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Artemis — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: artemis import-agent "
    ...
    print_info(f"Target:      {artemis_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'artemis setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: artemis import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'artemis setup' "
               "to configure providers, or add them to ~/.artemis/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from artemis_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'artemis setup' or ~/.artemis/.env.")
        print()
