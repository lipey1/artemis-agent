...
Used by ``artemis profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``artemis_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Artemis Agent kanban board.
...
Rules:
  ...
  - Never write "Artemis Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_artemis_home())
        else:
