...
Used by ``artemis kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``artemis_cli/goals.py`` — same aux
...
ARTEMIS_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("ARTEMIS_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Artemis Agent board.
...
def _profile_author() -> str:
    """Mirror of ``artemis_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ARTEMIS_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=ARTEMIS_KANBAN_SPECIFY_MAX_TOKENS,
