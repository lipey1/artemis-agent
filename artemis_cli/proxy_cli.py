"""CLI handlers for ``artemis egress ...``.
...
The top-level command is ``artemis egress``.  Note that the inbound OAuth
reverse-proxy command (``artemis proxy``) lives elsewhere in
``artemis_cli/main.py`` — different direction, different purpose.
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from artemis_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``artemis_cli.main`` as part of building the top-level
    ``artemis egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``artemis proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`artemis egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `artemis secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.artemis/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `artemis egress setup` finds the same keys the
        ...
        if loaded:
            console.print(
                ...
                f"~/.artemis/.env for discovery.[/dim]"
            )
    ...
    # explicitly requested rotation.  This prevents re-running `artemis
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Artemis sandbox.  They will start "
    ...
    # `artemis egress reload` apply future ruleset changes without a
    ...
    # rotation), running `artemis egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]artemis egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]artemis egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]artemis egress start[/cyan]\n"
        "  Restart: [cyan]artemis egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]artemis egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]artemis egress status[/cyan]\n"
        "  Stop:    [cyan]artemis egress stop[/cyan]\n"
        "  Disable: [cyan]artemis egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `artemis egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `artemis egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`artemis egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `artemis secrets bitwarden setup` to configure the "
                "project, or switch back via `artemis egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``artemis egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `artemis egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `artemis egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `artemis egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]artemis egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `artemis egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.artemis/.env`` into ``os.environ``.

    ``artemis egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.artemis/.env`` (which the agent
