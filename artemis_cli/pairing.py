...
Usage:
    artemis pairing list              # Show all pending + approved users
    artemis pairing approve <platform> <request-id|code>  # Approve a pairing request
    artemis pairing revoke <platform> <user_id> # Revoke user access
    artemis pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle artemis pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: artemis pairing {list|approve|revoke|clear-pending}")
        print("Run 'artemis pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: artemis pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.artemis/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'artemis pairing list' to see pending requests.\n")
