"""On-demand supply-chain audit for Artemis Agent installs.

Scans three surfaces a Artemis user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Artemis venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.artemis/plugins``
...
def _discover_plugins(artemis_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.artemis/plugins``.
    ...
    """
    plugins_dir = artemis_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = artemis_home or Path(get_artemis_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `artemis security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, artemis_home=home
    )
