"""
artemis fallback — manage the fallback provider chain.
...
Subcommands:
  artemis fallback [list]   Show the current fallback chain (default when no subcommand)
  artemis fallback add      Pick provider + model via the same picker as `artemis model`,
                           then append the selection to the chain
  artemis fallback remove   Pick an entry to delete from the chain
  artemis fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.artemis/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from artemis_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `artemis model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  artemis fallback add")
        print()
        return
    ...
    print("  Docs: https://artemis-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `artemis model`, then append the selection to the chain."""
    from artemis_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `artemis model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `artemis fallback list` to view, or `artemis fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``artemis fallback [subcommand]``."""
