...
The provider list shown by ``artemis model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`artemis_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``artemis model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`artemis_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``artemis model`` shows.
"""
...
# artemis_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `artemis model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``artemis model`` universe.
    ...
    # Artemis overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = ARTEMIS_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Artemis overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
