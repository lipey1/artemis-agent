"""`artemis checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.artemis/checkpoints/``.  Actions:

    artemis checkpoints               # same as `status`
    artemis checkpoints status        # total size, project count, breakdown
    artemis checkpoints list          # per-project checkpoint counts + workdir
    artemis checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    artemis checkpoints clear [-f]    # nuke the entire base (asks first)
    artemis checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    artemis checkpoints
    artemis checkpoints prune --retention-days 3 --max-size-mb 200
    artemis checkpoints clear -f
...
from artemis_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: artemis checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``artemis checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `artemis checkpoints` → status
