"""Profile distributions — shareable, packaged Artemis profiles via git.

A distribution is a Artemis profile published as a git repository (or
...
* ``artemis profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``artemis skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``artemis profile``, not a parallel tree):

    artemis profile install <source> [--name N] [--alias] [--force] [--yes]
    artemis profile update  <name>  [--force-config] [--yes]
    artemis profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "artemis_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".artemis_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "artemis-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            artemis_requires=str(data.get("artemis_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.artemis_requires:
            out["artemis_requires"] = self.artemis_requires
        if self.author:
...
def check_artemis_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Artemis {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Artemis distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``artemis profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Artemis profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Artemis distribution."
        )

    # Version check up-front so we fail fast
    check_artemis_requires(manifest.artemis_requires, artemis_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.artemis).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="artemis_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `artemis profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`artemis profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="artemis_dist_update_") as tmp:
        plan = plan_install(
