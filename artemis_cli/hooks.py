"""artemis hooks — inspect and manage shell-script hooks.

Usage::

    artemis hooks list
    artemis hooks test <event> [--for-tool X] [--payload-file F]
    artemis hooks revoke <command>
    artemis hooks doctor

Consent records live under ``~/.artemis/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.artemis/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``artemis hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: artemis hooks {list|test|revoke|doctor}")
        print("Run 'artemis hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.artemis/config.yaml.")
        print("See `artemis hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.artemis/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `artemis hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `artemis hooks test` and `artemis hooks doctor`
...
def _print_parsed(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Artemis wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `artemis hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `artemis hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `artemis hooks doctor`.")
