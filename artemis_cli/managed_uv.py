"""Artemis-managed uv and Python runtime repair.

Artemis owns its own uv binary at ``$ARTEMIS_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ARTEMIS_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Artemis
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.artemis-runtime/python``. A
...
from artemis_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".artemis-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Artemis keeps *its* uv binary.

    ``$ARTEMIS_HOME/bin/uv`` on POSIX, ``$ARTEMIS_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Artemis-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Artemis keeps databases "
            "out of WAL mode on this SQLite build. The next `artemis update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``artemis update`` runs
    the call site from the *old*, already-imported ``artemis_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``artemis update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under ARTEMIS_HOME
    caches the last successful self-update time.
    """
    try:
        from artemis_constants import get_artemis_home

        stamp = get_artemis_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``artemis update`` so the managed copy stays current.
...
def _reload_artemis_constants():
    """Re-execute ``artemis_constants`` from disk and return the fresh module.

    ``artemis update`` imports ``artemis_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'artemis_constants'
        (~/.artemis/artemis-agent/artemis_constants.py)
    ...
    """
    import artemis_constants

    return importlib.reload(artemis_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Artemis generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("artemis_cli.main")
    ...
    if holders:
        ...
        return True, f"other Artemis processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Artemis-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``artemis update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``artemis update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$ARTEMIS_HOME/bin/`` instead of ``~/.local/bin/``.
    """
