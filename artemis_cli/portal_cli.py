"""``artemis portal`` — the human-readable entry point for Nous Portal.

Running ``artemis portal`` with no subcommand performs the one-shot Portal
...
``artemis auth add nous --type oauth`` (which still works), is identical to
``artemis setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``artemis auth`` or ``artemis tools``. It's the onboarding + discovery
...
DOCS_URL = "https://artemis-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `artemis model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `artemis portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `artemis auth add nous --type
    oauth`. It reuses the exact wiring behind `artemis setup --portal` (which in
    ...
    """
    from artemis_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `artemis portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `artemis portal` is the
        # human-readable alias for `artemis auth add nous --type oauth` /
        # `artemis setup --portal`.
    ...
    print("Run `artemis portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `artemis portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `artemis portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `artemis auth add "
            "nous --type oauth`, identical to `artemis setup --portal`). "
