...
This module is imported by ``artemis_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no artemis_cli.config,
...
``artemis_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(artemis_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ARTEMIS_HOME?"""
    active_profile = os.path.join(artemis_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    artemis_home = os.environ.get("ARTEMIS_HOME", "").strip()
    if artemis_home:
        return artemis_home
    return os.path.join(os.path.expanduser("~"), ".artemis")
...
def container_mode_may_be_active() -> bool:
    ...
    artemis_home = os.environ.get("ARTEMIS_HOME", "").strip()
    if artemis_home:
        if os.path.exists(os.path.join(artemis_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(artemis_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(artemis_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".artemis")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ARTEMIS_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    print("Run 'artemis version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``artemis --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the ARTEMIS_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("ARTEMIS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
