...
``artemis_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import artemis_state (cycle) — shared
module-level constants live in artemis_state_common.
"""
...
from artemis_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_CJK_STALE_KEY,
    FTS_STALE_KEY,
    FTS_SQL,
...
# Moved methods logged under the "artemis_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("artemis_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``artemis_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `artemis update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._artemis_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._artemis_fts5_probe")
            return True
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from artemis_constants import get_artemis_home
            cache_path = get_artemis_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `artemis sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `artemis db optimize`) must keep its EXISTING
