---
name: artemis-agent-skill-authoring
description: "Author in-repo SKILL.md files: frontmatter and structure."
version: 2.0.0
author: Artemis Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  artemis:
    tags: [skills, authoring, artemis-agent, conventions, skill-md]
    related_skills: [plan, requesting-code-review]
---

# Authoring Artemis-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.artemis/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the artemis-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with artemis-agent
...
- Don't use for: personal skills in `~/.artemis/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `artemis skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Artemis Agent
license: MIT
...
- Credit the **human first**, then "Artemis Agent" as secondary collaborator: `Ben Barclay (benbarclay), Artemis Agent`.
- Never `author: Artemis Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Artemis Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.artemis/skills/`.
...
|---|---|
| Artemis tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.artemis/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Artemis Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.artemis.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Artemis tools
