...
- A long-running process (artemis gateway, tui_gateway) misbehaves and you can't restart it
...
The artemis test runner and pytest both support this:
...
For long-lived processes: Artemis gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <artemis-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Artemis you have two practical options:
...
```json
{
  "name": "Attach to Artemis",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<artemis-agent-repo>" }
  ]
}
```
...
## Debugging Artemis-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `artemis` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `artemis --tui`)
...
debugpy.wait_for_client()
```
Start `artemis --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Artemis subagents, debug one process at a time.
