...
Ledger path resolution (first wins):
  --ledger PATH
  $ARTEMIS_CITATION_LEDGER
  $ARTEMIS_HOME/cache/citations/ledger.json
"""
...
from _artemis_home import get_artemis_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ARTEMIS_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_artemis_home() / "cache" / "citations" / "ledger.json"
