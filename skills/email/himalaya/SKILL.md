...
This skill is separate from the Artemis Email gateway adapter. The gateway
adapter lets people email the agent and uses Artemis' built-in IMAP/SMTP
...
To reply non-interactively from Artemis, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Artemis)** — pipe the message via stdin:
