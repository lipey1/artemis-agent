#!/usr/bin/env python3
"""Google Workspace API CLI for Artemis Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Artemis-facing JSON contract and falls back to the Python client
...
from _artemis_home import get_artemis_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ARTEMIS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Artemis Agent")
