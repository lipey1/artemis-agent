...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `artemis kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `artemis kanban show <task-id>` for completion
