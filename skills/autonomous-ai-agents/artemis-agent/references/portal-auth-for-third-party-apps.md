...
## Layer 1 — Is this thing a Artemis plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Artemis process**. Its LLM calls go through Artemis's already-configured provider. | Already uses Portal if user's Artemis is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Artemis. That LLM call is already authenticated
via Artemis's provider config. The plugin's own server auth (e.g.
...
What Artemis Agent has that *feels* like OAuth — `artemis login --provider nous`
opening a browser, user signs in, token lands in `~/.artemis/auth.json` — is a
**Artemis-specific browser flow**. Under the hood it produces a credential
Artemis uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Artemis's existing Portal credential out of `~/.artemis/auth.json`.
...
rides on the credential Artemis already holds.

Where this could live in Artemis:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Artemis itself.
...
1. First decide: Artemis plugin (runs inside Artemis) or separate app? If plugin,
   it already uses Portal via Artemis's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Artemis agent itself") is fully supported. Artemis treats agent
...
pitch in the Tool Gateway announcement is specifically about Artemis Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Artemis is configured to use Portal as its provider. It is
