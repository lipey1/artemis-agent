# Themes / Skins — Author a Artemis Color Theme

Author a Artemis **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`artemis_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.artemis/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Artemis itself.
...
- Write access to the Artemis home dir — `~/.artemis` by default, or `$ARTEMIS_HOME`
  / the active profile's dir. Skins live in `<artemis-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `artemis config set`).
...
3. `write_file` it to `<artemis-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `artemis config set display.skin default` (or they can `/skin default`).
...
```
artemis skin set <key> <hex>      # e.g. artemis skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `artemis skin set background "#08201f"` changes
only the background; `artemis skin use <name>` / `artemis skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.artemis`** when a profile is active — resolve the real home
  from `$ARTEMIS_HOME` first, falling back to `~/.artemis`.
...
- **Never hand-edit `config.yaml` to activate.** Use `artemis config set
...
- **You apply it, not the user.** `artemis config set display.skin <name>` is
...
- `read_file` the written `<artemis-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `artemis config get display.skin` and confirm it reports `<name>`.
