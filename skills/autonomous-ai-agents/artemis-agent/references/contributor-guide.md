...
For occasional contributors and PR authors. Full developer docs: https://artemis-agent.nousresearch.com/docs/developer-guide/
...
├── cli.py                # Interactive CLI (ArtemisCLI)
├── artemis_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── artemis_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.artemis/config.yaml` (settings), `~/.artemis/.env` (API keys) — both under `$ARTEMIS_HOME` when it is set.
...
`_ARTEMIS_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_artemis_home()` for paths,
never hardcode `~/.artemis`. For custom/local-only tools, write a plugin in
`~/.artemis/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `artemis_cli/commands.py`
...
- Tests auto-redirect `ARTEMIS_HOME` to temp dirs — never touch real `~/.artemis/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/artemis_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_artemis_home()` from `artemis_constants` for all paths (profile-safe)
