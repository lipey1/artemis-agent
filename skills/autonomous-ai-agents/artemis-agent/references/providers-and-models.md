...
Set via `artemis model` (picker) or `artemis setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://artemis-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `artemis auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `artemis auth add openai-codex` |
| qwen-oauth | OAuth | `artemis auth add qwen-oauth` |
| minimax-oauth | OAuth | `artemis auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`artemis auth`).
Fallback chain when the primary fails: `artemis fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`artemis_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   artemis config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
