...
Author widget apps for the Artemis TUI (`artemis --tui`): glanceable ambient
...
- The TUI must be in use (`artemis --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.artemis/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.artemis/logs/tui_gateway_crash.log` if a widget never appears.
