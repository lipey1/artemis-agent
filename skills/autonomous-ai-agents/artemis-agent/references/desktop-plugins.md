...
Write plugins for the Artemis desktop app: statusbar items, layout panes,
...
system (`~/.artemis/plugins/`) is otherwise documented separately.
...
- The Artemis desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ARTEMIS_HOME/desktop-plugins/` (usually
  `~/.artemis/desktop-plugins/`).

## How to Run

1. Create `$ARTEMIS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.artemis/...` by default, or `~/.artemis/profiles/<profile>/...` under a
...
The ONLY import surface is `@artemis/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Artemis (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.artemis/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@artemis/plugin-sdk`, `react`, and
