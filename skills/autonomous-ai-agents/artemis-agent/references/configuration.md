...
Edit with `artemis config edit` or `artemis config set section.key value`.
Full reference: https://artemis-agent.nousresearch.com/docs/user-guide/configuration
...
`artemis config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `artemis tools` (interactive) or `artemis tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ARTEMIS_CORE_TOOLS` is the default bundle most platforms inherit).
