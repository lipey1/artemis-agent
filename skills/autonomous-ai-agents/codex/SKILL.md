...
metadata:
  artemis:
    ...
    related_skills: [claude-code, artemis-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Artemis terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Artemis itself, `model.provider: openai-codex` uses Artemis-managed Codex
OAuth from `~/.artemis/auth.json` after `artemis auth add openai-codex`. For the
...
## Artemis Gateway Caveat

When invoking the Codex CLI from a Artemis gateway/service context (for example,
