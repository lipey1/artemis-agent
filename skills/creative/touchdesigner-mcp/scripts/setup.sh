...
ARTEMIS_HOME_DIR="${ARTEMIS_HOME:-$HOME/.artemis}"
ARTEMIS_CFG="${ARTEMIS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Artemis config has twozero_td MCP entry ──
if [[ ! -f "$ARTEMIS_CFG" ]]; then
    echo -e " ${FAIL} Artemis config not found at ${ARTEMIS_CFG}"
    manual_steps+=("Create ${ARTEMIS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ARTEMIS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Artemis config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Artemis config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ARTEMIS_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ARTEMIS_CFG} manually"); }
    manual_steps+=("Restart Artemis session to pick up config change")
fi
