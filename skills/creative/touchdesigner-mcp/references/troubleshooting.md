...
## 5. Artemis Configuration

### Config location

`$ARTEMIS_HOME/config.yaml` (defaults to `~/.artemis/config.yaml` when `ARTEMIS_HOME` is unset)
...
Restart the Artemis session for changes to take effect. The MCP connection is
