#!/usr/bin/env bash
# GitHub environment detection helper for Artemis Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _artemis_env="${ARTEMIS_HOME:-$HOME/.artemis}/.env"; [ -f "$_artemis_env" ] && grep -q "^GITHUB_TOKEN=" "$_artemis_env" 2>/dev/null; then
