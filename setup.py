...
Artemis Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ARTEMIS_NIX_BUILD=1`` on the
Artemis package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ARTEMIS_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for artemis-agent is not supported.\n"
    "Artemis is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://artemis-agent.nousresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Artemis Nix derivation sets ARTEMIS_NIX_BUILD=1. If it does, file a bug."
)
