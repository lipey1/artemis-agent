...
_ARTEMIS_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ARTEMIS_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `artemis tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``artemis tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ARTEMIS_KANBAN_TASK env "
    ...
    # Coding posture (base Artemis — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in artemis_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Artemis toolsets (CLI + messaging platforms)
    #
    ...
    # the `artemis send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "artemis-cli": {
        ...
        "tools": _ARTEMIS_CORE_TOOLS,
        "includes": []
    },

    "artemis-cron": {
        # Mirrors artemis-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `artemis tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as artemis-cli; gated by `artemis tools`",
        "tools": _ARTEMIS_CORE_TOOLS,
        "includes": []
    },
    ...
    "artemis-email": {
        "description": "Email bot toolset - interact with Artemis via email (IMAP/SMTP)",
        "tools": _ARTEMIS_CORE_TOOLS,
        "includes": []
    },
    ...
    "artemis-sms": {
        "description": "SMS bot toolset - interact with Artemis via SMS (Twilio)",
        "tools": _ARTEMIS_CORE_TOOLS,
        "includes": []
    },
    ...
    "artemis-webhook": {
        ...
        "tools": _ARTEMIS_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "artemis-gateway": {
        ...
        "tools": [],
        "includes": ["artemis-telegram", "artemis-discord", "artemis-whatsapp", "artemis-slack", "artemis-signal", "artemis-bluebubbles", "artemis-homeassistant", "artemis-email", "artemis-sms", "artemis-mattermost", "artemis-matrix", "artemis-dingtalk", "artemis-feishu", "artemis-wecom", "artemis-wecom-callback", "artemis-weixin", "artemis-qqbot", "artemis-webhook", "artemis-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``artemis-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ARTEMIS_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``artemis-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (artemis-<name>).
        # Gives them _ARTEMIS_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("artemis-"):
            platform_name = name[len("artemis-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ARTEMIS_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
