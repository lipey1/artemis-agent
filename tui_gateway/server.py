...
try:
    from artemis_cli.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("ARTEMIS_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ARTEMIS_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ARTEMIS_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("artemis_gateway_runtime_session_record", default=None)
)
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # gateway's launch ARTEMIS_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["ARTEMIS_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            artemis_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"ARTEMIS_HOME": str(profile_home)} if profile_home else None,
        )
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from artemis_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `artemis --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("ARTEMIS_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `artemis --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from artemis_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from artemis_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from artemis_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its ARTEMIS_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s ARTEMIS_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_artemis_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_artemis_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from artemis_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``artemis dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ARTEMIS_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ARTEMIS_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_artemis_home_override(profile_home)
                try:
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``artemis`` / ``artemis --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from artemis_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from artemis_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from artemis_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``artemis_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``artemis_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    artemis_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read ARTEMIS_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # ARTEMIS_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ARTEMIS_GATEWAY_SESSION"] = "1"
    os.environ["ARTEMIS_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from artemis_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``artemis config set display.skin``) OR edited the active skin's colors in
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ARTEMIS_MODEL", "")
        or os.environ.get("ARTEMIS_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from artemis_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ARTEMIS_DESKTOP=1`` and ``ARTEMIS_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ARTEMIS_DESKTOP_TERMINAL=1`` → "tui"
        (``artemis --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``artemis --tui``.)
    """
    if is_truthy_value(os.environ.get("ARTEMIS_DESKTOP")) and not is_truthy_value(
        os.environ.get("ARTEMIS_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ARTEMIS_MODEL /
    ARTEMIS_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`artemis --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads ARTEMIS_MODEL /
    # ARTEMIS_INFERENCE_MODEL, which `artemis --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("ARTEMIS_TUI_PROVIDER", "").strip()
    ...
    try:
        from artemis_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ARTEMIS_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from artemis_cli.runtime_provider import (
...
def _load_approval_mode() -> str:
    ...
    canonical ``artemis_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`artemis_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ARTEMIS_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("ARTEMIS_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ARTEMIS_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from artemis_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ARTEMIS_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ARTEMIS_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ARTEMIS_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ARTEMIS_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from artemis_cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from artemis_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (ARTEMIS_MODEL /
    # ARTEMIS_INFERENCE_MODEL / ARTEMIS_TUI_PROVIDER / ARTEMIS_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `artemis --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # ARTEMIS_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ARTEMIS_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from artemis_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from artemis_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from artemis_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from artemis_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from artemis_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via artemis_cli.personality (single owner)."""
    from artemis_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as artemis_cli.personality.resolve_personality — (name,
    ...
    """
    from artemis_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to artemis_cli.personality (single owner).
    """
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ARTEMIS_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ARTEMIS_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _resolve_runtime_with_fallback(
    ...
    """
    from artemis_cli.auth import AuthError
    from artemis_cli.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from artemis_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses artemis_cli.mcp_startup; TUI stdio keeps
    ...
    cfg = _load_cfg()
    from artemis_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ARTEMIS_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ARTEMIS_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ARTEMIS_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ARTEMIS_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped ARTEMIS_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / ARTEMIS_DIRECTIVE_RE)
...
def _pet_gen_root():
    ...
    root = get_artemis_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ARTEMIS_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $ARTEMIS_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_artemis_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=ARTEMIS_SESSION_KEY`` (see
    ...
    # DB when ARTEMIS_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``ARTEMIS_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ARTEMIS_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_artemis_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from artemis_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from artemis_cli.voice import is_continuous_active
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: artemis_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific ARTEMIS_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile ARTEMIS_HOME
    override, so ``get_artemis_home()`` here would return the gateway's launch
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile ARTEMIS_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from artemis_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from artemis_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ARTEMIS_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["ARTEMIS_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("ARTEMIS_YOLO_MODE", None)
                    nv = "0"
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through artemis_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from artemis_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under ARTEMIS_HOME (~/.artemis by
    ...
    artemis_home = os.path.realpath(str(get_artemis_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == artemis_home
        or real.startswith(artemis_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ARTEMIS_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: ARTEMIS_HOME itself and the dirs in
    ...
    artemis_home = os.path.normcase(os.path.realpath(str(get_artemis_home())))
    return real in _non_workspace_dirs() or real == artemis_home
...
def _discover_repos_payload(
    ...
    repos even with zero artemis sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (artemis home subtree + bare
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if a0 == "gateway":
        return "`artemis gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`artemis sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`artemis config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from artemis_cli.inventory import load_picker_context
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ARTEMIS_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ARTEMIS_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``artemis_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["ARTEMIS_VOICE"] = "0"
                    os.environ["ARTEMIS_VOICE_TTS"] = "0"
                    try:
                        from artemis_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via artemis_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from artemis_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ARTEMIS_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["ARTEMIS_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from artemis_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from artemis_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import artemis_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from artemis_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from artemis_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
