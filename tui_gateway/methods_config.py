...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from artemis_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_artemis_home), "display": display_artemis_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _artemis_home / "config.yaml"
        try:
...
def _(rid, params: dict) -> dict:
    try:
        from artemis_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Artemis provider is configured.",
                },
            )
